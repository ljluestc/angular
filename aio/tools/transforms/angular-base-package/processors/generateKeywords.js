...
async $process(docs) {
  const {stemmer: stem} = await import('stemmer');  const dictionary = new Map();
  ...
  log.debug('Doc types to ignore', docTypesToIgnore);  const filteredDocs = docs
  ...
  .filter(doc => !doc.internal && !doc.privateExport);  for (const doc of filteredDocs) {
