...
import {UpgradeAppType} from '../src/util';describe('downgradeInjectable', () => {
...
mockNg1Injector.has.and.callFake((key: string) => mockDependencies.hasOwnProperty(key));const mockNg2Injector = jasmine.createSpyObj<Injector>(['get']);
mockNg2Injector.get.and.returnValue('service value');const mockDependencies: {[key: string]: any} = {
...
expect((factory as any).$inject).toEqual([$INJECTOR]);const {mockNg1Injector, mockNg2Injector} = setupMockInjectors();
...
expect(factory).toEqual(jasmine.any(Function));
expect((factory as any).$inject).toEqual([$INJECTOR]);const {mockNg1Injector, mockNg2Injector} = setupMockInjectors('someModule');
expect(factory(mockNg1Injector)).toEqual('service value');
expect(mockNg2Injector.get).toHaveBeenCalledWith('someToken');
