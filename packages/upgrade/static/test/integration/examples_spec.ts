...
afterEach(() => destroyPlatform());it('should have AngularJS loaded',
   () => expect(angular.getAngularJSGlobal().version.major).toBe(1));it('should verify UpgradeAdapter example', waitForAsync(() => {
 ...
 class Ng1Component extends UpgradeComponent {
   @Input() title: string = '';   constructor(elementRef: ElementRef, injector: Injector) {
...
complaining */
 ...
 // AngularJS
 .directive('ng2', downgradeComponent({component: Ng2Component})); // This is the (AngularJS) application bootstrap element
 // Notice that it is actually a downgraded Angular component
 const element = html('<ng2 name="World">project</ng2>'); // Let's use a helper function to make this simpler
