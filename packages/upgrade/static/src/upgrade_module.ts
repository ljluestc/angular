 ...
 */import {Injector, NgModule, NgZone, PlatformRef, Testability} from '@angular/core';import {ɵangular1, ɵconstants, ɵutil} from '../common';import {angular1Providers, setTempInjectorRef} from './angular1_providers';
...
export class UpgradeModule {
  ...
  public injector: Injector;  constructor(
...
const INIT_MODULE_NAME = ɵconstants.UPGRADE_MODULE_NAME + '.init';// Create an ng1 module to bootstrap
ɵangular1
.module_(INIT_MODULE_NAME, []).constant(ɵconstants.UPGRADE_APP_TYPE_KEY, ɵutil.UpgradeAppType.Static).value(ɵconstants.INJECTOR_KEY, this.injector).factory(
ɵconstants.LAZY_MODULE_REF,
[ɵconstants.INJECTOR_KEY, (injector: Injector) => ({injector} as ɵutil.LazyModuleRef)]).config([
...
setTimeout(() => {
  ...
  .forEach(prop => (wrappedInterval as any)[prop] = intervalDelegate[prop]);  // the `flush` method will be present when ngMocks is used
...
this.injector.get(ɵconstants.$INJECTOR);// Put the injector on the DOM, so that it can be "required"
ɵangular1.element(element).data!
(ɵutil.controllerKey(ɵconstants.INJECTOR_KEY), this.injector);// Destroy the AngularJS app once the Angular `PlatformRef` is destroyed.
// This does not happen in a typical SPA scenario, but it might be useful for
// other use-cases where disposing of an Angular/AngularJS app is necessary
// (such as Hot Module Replacement (HMR)).
// See https://github.com/angular/angular/issues/39935.
this.platformRef.onDestroy(() => ɵutil.destroyApp($injector));// Wire up the ng1 rootScope to run a digest cycle whenever the zone settles
...
ɵangular1.module_(ɵconstants.UPGRADE_MODULE_NAME, [INIT_MODULE_NAME].concat(modules));// Make sure resumeBootstrap() only exists if the current bootstrap is deferred
...
const returnValue =
this.ngZone.run(() => ɵangular1.bootstrap(element, [upgradeModule.name], config));// Patch resumeBootstrap() to run inside the ngZone
