...
import {BehaviorSubject, ReplaySubject, Subject} from 'rxjs';describe('toSignal()', () => {
  it('should reflect the last emitted value of an Observable', test(() => {
 const counter$ = new BehaviorSubject(0);
 const counter = toSignal(counter$); expect(counter()).toBe(0);
 counter$.next(1);
 ...
 const counter = toSignal(counter$); expect(counter()).toBe(1); counter$.next(2);
 expect(counter()).toBe(2);
     }));  it('should propagate an error returned by the Observable', test(() => {
 const counter$ = new BehaviorSubject(1);
 const counter = toSignal(counter$); expect(counter()).toBe(1); counter$.error('fail');
 ...
 const counter = runInInjectionContext(injector, () => toSignal(counter$)); expect(counter()).toBe(0);
 counter$.next(1);
 expect(counter()).toBe(1); // Destroying the injector should unsubscribe the Observable.
 injector.destroy(); // The signal should have the last value observed.
 expect(counter()).toBe(1); // And this value should no longer be updating (unsubscribed).
 counter$.next(2);
 expect(counter()).toBe(1);
     }));  it('should unsubscribe when an explicitly provided injector is destroyed', test(() => {
 const counter$ = new BehaviorSubject(0);
 const injector = Injector.create({providers: []}) as EnvironmentInjector;
 const counter = toSignal(counter$, {injector}); expect(counter()).toBe(0);
 counter$.next(1);
 expect(counter()).toBe(1); // Destroying the injector should unsubscribe the Observable.
 injector.destroy(); // The signal should have the last value observed.
 expect(counter()).toBe(1); // And this value should no longer be updating (unsubscribed).
 ...
 expect(counter()).toBe(1);
    ...
    const injector = Injector.create({providers: []}) as EnvironmentInjector;
    const counter =
  runInInjectionContext(injector, () => toSignal(counter$, {manualCleanup: true}));    injector.destroy();    // Destroying the injector should not have unsubscribed the Observable.
    counter$.next(1);
    expect(counter()).toBe(1);    counter$.complete();    // The signal should have the last value observed before the observable completed.
    expect(counter()).toBe(1);
  });  describe('with no initial value', () => {
    it('should return `undefined` if read before a value is emitted', test(() => {
nter$ = new Subject<number>();
nter = toSignal(counter$);unter()).toBeUndefined();
next(1);
unter()).toBe(1);
 }));    it('should not throw if a value is emitted before called', test(() => {
nter$ = new Subject<number>();
nter = toSignal(counter$);next(1);
 => counter()).not.toThrow();
 }));
  });  describe('with requireSync', () => {
    it('should throw if created before a value is emitted', test(() => {
nter$ = new Subject<number>();
 => toSignal(counter$, {requireSync: true})).toThrow();
 }));    it('should not throw if a value emits synchronously on creation', test(() => {
nter$ = new ReplaySubject<number>(1);
next(1);
nter = toSignal(counter$);
unter()).toBe(1);
 }));
  });  describe('with an initial value', () => {
    it('should return the initial value if called before a value is emitted', test(() => {
nter$ = new Subject<number>();
nter = toSignal(counter$, {initialValue: null});unter()).toBeNull();
next(1);
unter()).toBe(1);
 }));    it('should not return the initial value if called after a value is emitted', test(() => {
nter$ = new Subject<number>();
nter = toSignal(counter$, {initialValue: null});next(1);
unter()).not.toBeNull();
 }));
  });
