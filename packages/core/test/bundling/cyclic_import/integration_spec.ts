...
import * as path from 'path';const PACKAGE = 'angular/packages/core/test/bundling/cyclic_import';describe('treeshaking with uglify', () => {
  let content: string;
...
ort(path.join(PACKAGE, 'bundle.js'));
ndow as any).appReady;
cument.body.textContent).toEqual('dep');
 }));    it('should render hello world when debug minified',
 withBody('<trigger></trigger>', async () => {
ort(path.join(PACKAGE, 'bundle.debug.min.js'));
ndow as any).appReady;
cument.body.textContent).toEqual('dep');
 }));    it('should render hello world when fully minified',
 withBody('<trigger></trigger>', async () => {
ort(path.join(PACKAGE, 'bundle.min.js'));
ndow as any).appReady;
cument.body.textContent).toEqual('dep');
 }));
  });
