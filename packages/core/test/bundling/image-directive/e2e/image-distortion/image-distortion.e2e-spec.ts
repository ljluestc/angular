...
import {logging} from 'selenium-webdriver';import {collectBrowserLogs} from '../browser-logs-util';describe('NgOptimizedImage directive', () => {
  it('should not warn if there is no image distortion', async () => {
    ...
    const logs = await collectBrowserLogs(logging.Level.WARNING);    expect(logs.length).toEqual(7);
...
expect(logs.some((log) => {
 ...
 '\\nTo fix this, update the width and height attributes.');    expectErrorMessageInLogs(
  logs,
  'The NgOptimizedImage directive (activated on an \\u003Cimg> element ' +
 'with the \`ngSrc=\\"/e2e/b.png\\"`) has detected that ' +
 'the aspect ratio of the image does not match the aspect ratio indicated by the width and height attributes. ' +
 ...
 '\\nTo fix this, update the width and height attributes.');    // Images with incorrect styling
    expectErrorMessageInLogs(
  logs,
  'The NgOptimizedImage directive (activated on an \\u003Cimg> element ' +
 'with the \`ngSrc=\\"/e2e/b.png\\"`) has detected that ' +
 ...
 'adding \\"height: auto\\" or \\"width: auto\\" to the image styling will fix this issue.');    expectErrorMessageInLogs(
  logs,
  'The NgOptimizedImage directive (activated on an \\u003Cimg> element ' +
 'with the \`ngSrc=\\"/e2e/b.png\\"`) has detected that ' +
 'the aspect ratio of the rendered image does not match the image\'s intrinsic aspect ratio. ' +
 ...
 '\\nThis issue can occur if \\"width\\" and \\"height\\" attributes are added to an image ' +
 'without updating the corresponding image styling. To fix this, adjust image styling. In most cases, ' +
 'adding \\"height: auto\\" or \\"width: auto\\" to the image styling will fix this issue.');    // Padding is used on <img>
    expectErrorMessageInLogs(
  logs,
  'The NgOptimizedImage directive (activated on an \\u003Cimg> element ' +
 ...
 '\\nThis issue can occur if \\"width\\" and \\"height\\" attributes are added to an image ' +
 'without updating the corresponding image styling. To fix this, adjust image styling. In most cases, ' +
 'adding \\"height: auto\\" or \\"width: auto\\" to the image styling will fix this issue.');    // Image with incorrect width/height attributes AND incorrect styling
