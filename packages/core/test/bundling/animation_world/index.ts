...
import {BrowserModule, platformBrowser} from '@angular/platform-browser';@Directive({
...
class MakeColorGreyDirective {
  ...
  @HostBinding('style.color') private _textColor: string|null = null;  on() {
...
class BoxWithOverriddenStylesComponent {
  public active = false;  @HostBinding('style') styles = {};  constructor() {
    this.onInActive();
...
<div
  ...
  *ngFor="let item of items"
  ...
  {{ item.value }}
    ...
    </div>    <hr>    <box-with-overridden-styles
style="display:block"
...
class AnimationWorldComponent {
  @HostBinding('class') classVal = 'border';  items: any[] = [
  ...
  public styles: {[key: string]: any}|null = null;  constructor(element: ElementRef) {
