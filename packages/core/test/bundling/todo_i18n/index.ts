...
import {BrowserModule, platformBrowser} from '@angular/platform-browser';import {translations} from './translations';class Todo {
  editing: boolean;  get title() {
    return this._title;
  }
...
class ToDoAppComponent {
  newTodoText = '';  constructor(public todoStore: TodoStore) {
    (window as any).todoAppComponent = this;
  }  cancelEditingTodo(todo: Todo) {
    todo.editing = false;
    detectChanges(this);
  }  finishUpdatingTodo(todo: Todo, editedTitle: string) {
    editedTitle = editedTitle.trim();    if (editedTitle.length === 0) {
this.remove(todo);
...
class ToDoAppModule {
}loadTranslations(translations);(window as any).appReady = platformBrowser().bootstrapModule(ToDoAppModule, {ngZone: 'noop'});
