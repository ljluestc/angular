...
import {isDOMParserAvailable} from '../../src/sanitization/inert_body';function sanitizeHtml(defaultDoc: any, unsafeHtmlInput: string): string {
  return _sanitizeHtml(defaultDoc, unsafeHtmlInput).toString();
}{
  describe('HTML sanitizer', () => {
    let defaultDoc: any;
    let originalLog: (msg: any) => any = null!;
    let logMsgs: string[];    beforeEach(() => {
defaultDoc = document;
logMsgs = [];
...
.toEqual('<img srcset="/foo.png 400px, javascript:evil() 23px">');// Verify that complex `srcset` with URLs that contain commas are retained as is.
...
const dangerousAttrs = ['id', 'name', 'style'];for (const attr of dangerousAttrs) {
  it(`${attr}`, () => {
