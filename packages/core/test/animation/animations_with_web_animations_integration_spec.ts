...
import {BrowserAnimationsModule} from '@angular/platform-browser/animations';(function() {
...
if (isNode) return;describe('animation integration tests using web animations', function() {
  beforeEach(() => {
...
[
 ...
 transition('* => *', animate(1000))
    ...
    fixture.detectChanges();    expect(engine.players.length).toEqual(1);
    let webPlayer =
        (engine.players[0] as TransitionAnimationPlayer).getRealPlayer() as ɵWebAnimationsPlayer;    expect(webPlayer.keyframes).toEqual([
      new Map<string, string|number>([['height', '0px'], ['offset', 0]]),
      new Map<string, string|number>([['height', '100px'], ['offset', 1]])
    ]);    webPlayer.finish();    cmp.exp = false;
    fixture.detectChanges();
    engine.flush();    expect(engine.players.length).toEqual(1);
    webPlayer =
        (engine.players[0] as TransitionAnimationPlayer).getRealPlayer() as ɵWebAnimationsPlayer;    expect(webPlayer.keyframes).toEqual([
...
[transition('* => *', [style({height: '!'}), animate(1000, style({height: '*'}))])])]
    ...
    let webPlayer = (player as TransitionAnimationPlayer).getRealPlayer() as ɵWebAnimationsPlayer;    expect(webPlayer.keyframes).toEqual([
      new Map<string, string|number>([['height', '0px'], ['offset', 0]]),
      new Map<string, string|number>([['height', '100px'], ['offset', 1]])
    ...
    player.destroy();    cmp.exp = 2;
    cmp.items = [0, 1, 2, 6];
    fixture.detectChanges();
    engine.flush();    expect(engine.players.length).toEqual(1);
    player = engine.players[0];
    webPlayer = (player as TransitionAnimationPlayer).getRealPlayer() as ɵWebAnimationsPlayer;    expect(webPlayer.keyframes).toEqual([
      new Map<string, string|number>([['height', '100px'], ['offset', 0]]),
...
[
 ...
 transition('* => *', animate('500ms')),
    ...
    const fixture = TestBed.createComponent(Cmp);
    const cmp = fixture.componentInstance;    cmp.exp = true;
    fixture.detectChanges();    let player = engine.players[0];
    ...
    player.finish();    cmp.exp = false;
    fixture.detectChanges();    player = engine.players[0];
...
animations: [
 ...
 query(':enter', [
   ...
   animate(500, style({height: '*'}))
        ...
        class Cmp {
y[] = []; {
return this.items.length ? 'full' : 'empty';this.items = [];this.items = [0, 1, 2, 3, 4];       }       TestBed.configureTestingModule({declarations: [Cmp]});       const engine = TestBed.inject(ɵAnimationEngine);
       const fixture = TestBed.createComponent(Cmp);
       const cmp = fixture.componentInstance;       cmp.empty();
       fixture.detectChanges();
       let player = engine.players[0] as TransitionAnimationPlayer;
       player.finish();       cmp.full();
       fixture.detectChanges();       player = engine.players[0] as TransitionAnimationPlayer;
       let queriedPlayers =
((player as TransitionAnimationPlayer).getRealPlayer() as AnimationGroupPlayer).players;
       expect(queriedPlayers.length).toEqual(5);       let i = 0;
       for (i = 0; i < queriedPlayers.length; i++) {
r = queriedPlayers[i] as ɵWebAnimationsPlayer;
ayer.keyframes).toEqual([
new Map<string, string|number>([['height', '0px'], ['offset', 0]]),
new Map<string, string|number>([['height', '50px'], ['offset', 1]]),nish();
       }       cmp.empty();
       fixture.detectChanges();       player = engine.players[0] as TransitionAnimationPlayer;
       queriedPlayers =
((player as TransitionAnimationPlayer).getRealPlayer() as AnimationGroupPlayer).players;
       expect(queriedPlayers.length).toEqual(5);       for (i = 0; i < queriedPlayers.length; i++) {
r = queriedPlayers[i] as ɵWebAnimationsPlayer;
ayer.keyframes).toEqual([
...
<div [@myAnimation]="exp" style="background-color: blue;">...</div>
 ...
 [
   ...
   transition('* => b', [animate('1s', style({opacity: 0}))]),
    ...
    fixture.detectChanges();    const player1 = engine.players[0];
    const webPlayer1 =
        (player1 as TransitionAnimationPlayer).getRealPlayer() as ɵWebAnimationsPlayer;
    webPlayer1.setPosition(0.5);    cmp.exp = 'b';
    fixture.detectChanges();    const player2 = engine.players[0];
    const webPlayer2 =
...
ems: any[] = [];
       ...
       const fixture = TestBed.createComponent(Cmp);
       const cmp = fixture.componentInstance;       cmp.exp = 'full';
       cmp.items = [0, 1, 2, 3, 4];
       fixture.detectChanges();       let player = engine.players[0];
       let groupPlayer =
(player as TransitionAnimationPlayer).getRealPlayer() as AnimationGroupPlayer;
 ...
 players[i] as ɵWebAnimationsPlayer;
tion(0.5);
       }       cmp.exp = 'empty';
       cmp.items = [];
       fixture.detectChanges();       player = engine.players[0];
       groupPlayer = (player as TransitionAnimationPlayer).getRealPlayer() as AnimationGroupPlayer;
       players = groupPlayer.players;       expect(players.length).toEqual(5);
       for (let i = 0; i < players.length; i++) {
 players[i] as ɵWebAnimationsPlayer;
proximate(parseFloat(p.keyframes[0].get('width') as string), 250))
  .toBeLessThan(0.05);
proximate(parseFloat(p.keyframes[0].get('height') as string), 500))
  .toBeLessThan(0.05);
       }
...
[
  ...
  transition(
      ...
      [
       ...
       class Cmp {
d('elm', {static: true}) public element: any;AnimationExp = '';
       }       TestBed.configureTestingModule({declarations: [Cmp]});       const engine = TestBed.inject(ɵAnimationEngine);
       const fixture = TestBed.createComponent(Cmp);
       const cmp = fixture.componentInstance;       const elm = cmp.element.nativeElement;
       expect(elm.style.getPropertyValue('display')).toEqual('table');
       expect(elm.style.getPropertyValue('position')).toEqual('fixed');       cmp.myAnimationExp = 'go';
       fixture.detectChanges();       expect(elm.style.getPropertyValue('display')).toEqual('inline');
       expect(elm.style.getPropertyValue('position')).toEqual('absolute');       const player = engine.players.pop()!;
       player.finish();
       player.destroy();       expect(elm.style.getPropertyValue('display')).toEqual('inline-block');
...
<div #elm [@myAnimation]="myAnimationExp" style="width: 100%; font-size: 2rem"></div>
 ...
 [
   ...
   transition('* => go', [animate('1s')])
    ...
    class Cmp {
      @ViewChild('elm', {static: true}) public element: any;      public myAnimationExp = '';
    }    TestBed.configureTestingModule({declarations: [Cmp]});    const engine = TestBed.inject(ɵAnimationEngine);
    const fixture = TestBed.createComponent(Cmp);
    const cmp = fixture.componentInstance;    const elm = cmp.element.nativeElement;
    expect(elm.style.getPropertyValue('width')).toEqual('100%');
    expect(elm.style.getPropertyValue('font-size')).toEqual('2rem');    cmp.myAnimationExp = 'go';
    fixture.detectChanges();    const player = engine.players.pop()!;
    player.finish();
    player.destroy();    expect(elm.style.getPropertyValue('width')).toEqual('300px');
...
'background-color': 'red',
       ...
       class Cmp {
atus: 'active'|'inactive' = 'inactive';
       }       TestBed.configureTestingModule({declarations: [Cmp]});       const engine = TestBed.inject(ɵAnimationEngine);
       const fixture = TestBed.createComponent(Cmp);
       const cmp = fixture.componentInstance;
       fixture.detectChanges();       cmp.status = 'active';
       fixture.detectChanges();
       engine.flush();       expect(engine.players.length).toEqual(2);
       const [camelCaseWebPlayer, kebabCaseWebPlayer] = engine.players.map(
player => (player as TransitionAnimationPlayer).getRealPlayer() as ɵWebAnimationsPlayer);       [camelCaseWebPlayer, kebabCaseWebPlayer].forEach(webPlayer => {
bPlayer.keyframes).toEqual([
