...
import {inject} from '@angular/core/testing';import {DEFAULT_LOCALE_ID} from '../src/i18n/localization';
...
afterEach(() => restoreLocale());it('should set the ivy locale to `$localize.locale` value if it is defined', () => {
  // Injecting `LOCALE_ID` should also initialize the ivy locale
  const locale = TestBed.inject(LOCALE_ID);
  expect(locale).toEqual('de');
  expect(getLocaleId()).toEqual('de');
});it('should set the ivy locale to an application provided LOCALE_ID even if `$localize.locale` is defined',TestBed.configureTestingModule({providers: [{provide: LOCALE_ID, useValue: 'fr'}]});
const locale = TestBed.inject(LOCALE_ID);
expect(locale).toEqual('fr');
...
let originalLocale: string;function initLocale(locale: string) {
