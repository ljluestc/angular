...
import {compileComponent} from '../../src/render3/jit/directive';describe('resource_loading', () => {
  afterEach(clearResolutionOfComponentResourcesQueue);  describe('error handling', () => {
...
Did you run and wait for 'resolveComponentResources()'?`.trim());
 ...
 () => {
omponent: ComponentType<any> = (class MyComponent {}) as any;
mponent(
  MyComponent, {templateUrl: 'someUrl', styleUrls: ['someUrl1', 'someUrl2']});
 => MyComponent.ɵcmp).toThrowError(`
Component 'MyComponent' is not resolved:
 - templateUrl: someUrl
 - styleUrls: ["someUrl1","someUrl2"]
Did you run and wait for 'resolveComponentResources()'?`.trim());
...
return URLS[url] || Promise.reject('NOT_FOUND: ' + url);
    ...
    beforeEach(() => resourceFetchCount = 0);    it('should resolve template', async () => {
const MyComponent: ComponentType<any> = (class MyComponent {}) as any;
...
const MyComponent: ComponentType<any> = (class MyComponent {}) as any;
const MyComponent2: ComponentType<any> = (class MyComponent {}) as any;compileComponent(MyComponent, {template: ''});
expect(isComponentResourceResolutionQueueEmpty()).toBe(true);compileComponent(MyComponent2, {templateUrl: 'test://template'});
...
compileComponent(MyComponent, metadata);expect(() => resolveComponentResources(testResolver))
