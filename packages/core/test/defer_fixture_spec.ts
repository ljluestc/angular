...
import {expect} from '@angular/platform-browser/testing/src/matchers';@Component({
  selector: 'second-deferred-comp',
  standalone: true,
  template: `<div class="more">More Deferred Component</div>`,
})
class SecondDeferredComp {
}const COMMON_PROVIDERS = [{provide: PLATFORM_ID, useValue: PLATFORM_BROWSER_ID}];describe('DeferFixture', () => {
  beforeEach(() => setEnabledBlockTypes(['defer']));
  afterEach(() => setEnabledBlockTypes([]));  it('should start in manual behavior mode', async () => {
    @Component({
selector: 'defer-comp',
standalone: true,
...
deferBlockBehavior: DeferBlockBehavior.Playthrough,
    ...
    const el = componentFixture.nativeElement as HTMLElement;
    expect(el.querySelectorAll('.more').length).toBe(0);    componentFixture.componentInstance.shouldLoad = true;
    componentFixture.detectChanges();    await componentFixture.whenStable();  // await loading of deps    expect(el.querySelector('.more')).toBeDefined();
...
providers: COMMON_PROVIDERS,
    ...
    const el = componentFixture.nativeElement as HTMLElement;
    expect(el.querySelectorAll('.more').length).toBe(0);    componentFixture.componentInstance.shouldLoad = true;
    componentFixture.detectChanges();    await componentFixture.whenStable();  // await loading of deps    expect(el.querySelectorAll('.more').length).toBe(0);
