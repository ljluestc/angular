...
import {getElementClasses, getElementStyles} from '@angular/core/testing/src/styling';import {clearFirstUpdatePass, enterViewWithOneDiv, rewindBindingIndex} from './shared_spec';describe('styling', () => {
  beforeEach(enterViewWithOneDiv);
  afterEach(leaveView);  let div!: HTMLElement;
  beforeEach(() => div = getNativeByIndex(HEADER_OFFSET, getLView()) as HTMLElement);  it('should do set basic style', () => {
    ...
    expectStyle(div).toEqual({color: 'blue'});    clearFirstUpdatePass();    rewindBindingIndex();
    ɵɵstyleProp('color', 'red');
    ɵɵstyleProp('color', undefined);
    ɵɵstyleProp('color', undefined);
    expectStyle(div).toEqual({color: 'red'});
    ...
    ngDevModeResetPerfCounters();    clearFirstUpdatePass();
    rewindBindingIndex();
    ...
    ngDevModeResetPerfCounters();    rewindBindingIndex();
    ...
    expectClass(div).toEqual({foo: true});    rewindBindingIndex();
...
ngDevModeResetPerfCounters();clearFirstUpdatePass();rewindBindingIndex();
...
expectStyle(div).toEqual({color: 'blue', width: '100px'});clearFirstUpdatePass();rewindBindingIndex();
ɵɵstyleProp('color', 'red');
...
it('should append suffix', () => {
  ...
  expectStyle(div).toEqual({width: '100px'});  clearFirstUpdatePass();  rewindBindingIndex();
  ɵɵstyleProp('width', 200, 'px');
...
it('should capture static values in TStylingKey', () => {
  givenTemplateAttrs([AttributeMarker.Styles, 'content', '"TEMPLATE"']);  ɵɵstyleProp('content', '"dynamic"');
  expectTStylingKeys('style').toEqual([
['', 'content', 'content', '"TEMPLATE"'], 'prev',  // contains statics
null  // residual
  ]);
  expectStyle(div).toEqual({content: '"dynamic"'});  ɵɵstyleProp('content', undefined);
  expectTStylingKeys('style').toEqual([
...
[AttributeMarker.Styles, 'content', '"middle"'],  // 1
  ...
  expectStyle(div).toEqual({content: '"middle"'});  activateHostBindings(0);
  ɵɵstyleProp('content', '"dynamic"');
  expectTStylingKeys('style').toEqual([
['', 'content', 'content', '"lowest"'], 'both',  // 1: contains statics
['content', '"middle"'],// residual
  ]);
  expectStyle(div).toEqual({content: '"middle"'});  // second binding should not get statics
...
['content', '"middle"']// residual
  ...
  expectStyle(div).toEqual({content: '"middle"'});  activateHostBindings(1);
...
[AttributeMarker.Styles, 'content', '"middle"', 'width', '100px'],  // 1
  ...
  givenTemplateAttrs([AttributeMarker.Styles, 'content', '"TEMPLATE"', 'color', 'red']);  // TEMPLATE
...
null
  ...
  expectStyle(div).toEqual({content: '"TEMPLATE"', color: 'red', width: '100px'});  // Directive 0
...
null
  ...
  expectStyle(div).toEqual(
 {content: '"TEMPLATE"', color: 'red', width: '100px', height: '50px'});  // Directive 1
...
[AttributeMarker.Styles, 'content', '"middle"', 'width', '100px'],  // 1
  ...
  givenTemplateAttrs([AttributeMarker.Styles, 'content', '"TEMPLATE"', 'color', 'red']);  // Directive 1
  activateHostBindings(1);
  ɵɵstyleProp('color', 'white');
  expectTStylingKeys('style').toEqual([
// Host Binding 0 + 1
...
expect(toStylingKeyValueArray(keyValueArraySet, null!, {X: 'x', A: 'a'})).toEqual([
 'A', 'a', 'X', 'x'
    ...
    const MAX_VALUE = StylingRange.UNSIGNED_MASK;    it('should throw on negative values', () => {
...
}class MockDir {}function givenDirectiveAttrs(tAttrs: TAttributes[]) {
  const tNode = getTNode();
...
if (mode == AttributeMarker.ImplicitAttributes) {
  ...
  tStylingKeys.push(
(isClassBased ? tNode.residualClasses : tNode.residualStyles) as null | string[]);  return expect(tStylingKeys);
}
