...
import {withBody} from '@angular/private/testing';describe('effects', () => {
  beforeEach(destroyPlatform);
  afterEach(destroyPlatform);  it('should run effects in the zone in which they get created',
...
}); } await bootstrapApplication(Cmp); expect(log).not.toEqual(['angular', 'angular']);
     }));  it('should propagate errors to the ErrorHandler', () => {
    let run = false;    let lastError: any = null;
...
cleanupCount++;
    ...
    expect(cleanupCount).toBe(0);    fixture.componentInstance.counter.set(5);
    fixture.detectChanges();
    await fixture.whenStable();
    expect(counterLog).toEqual([0, 5]);
    expect(cleanupCount).toBe(1);    fixture.destroy();
    expect(counterLog).toEqual([0, 5]);
    expect(cleanupCount).toBe(2);
  });  it('should run effects created in ngAfterViewInit', async () => {
    let didRun = false;    @Component({
selector: 'test-cmp',
standalone: true,
template: '',
    })
    class Cmp implements AfterViewInit {
injector = inject(Injector);ngAfterViewInit(): void {
  effect(() => {
didRun = true;
  }, {injector: this.injector});
}
    }    const fixture = TestBed.createComponent(Cmp);
    fixture.detectChanges();    // Effects don't run during change detection.
    expect(didRun).toBeFalse();    TestBed.flushEffects();    expect(didRun).toBeTrue();
...
effect(() => {
  expect(() => this.counter.set(1)).toThrow();
    ...
    const counter = signal(0);    effect(() => counter.set(1), {allowSignalWrites: true, injector: TestBed.inject(Injector)});
...
@Input() in : string|undefined;ngOnChanges(changes: SimpleChanges): void {
  if (changes['in']) {
...
state = signal('property initializer');constructor() {
...
state = signal('property initializer');@Input()
...
template: '{{items().length}}',
    ...
    class WithQuery {
items = signal<unknown[]>([]);@ContentChildren('item')
...
constructor(environmentInjector: EnvironmentInjector) {
  ...
  // code defensively in the framework.  // Here we want to specifically verify that an effect is _not_ re-run if a signal read
...
this.noOfCmpCreated++;
    ...
    fixture.detectChanges();    expect(fixture.componentInstance.noOfCmpCreated).toBe(1);    state.set('changed');
    fixture.detectChanges();    expect(fixture.componentInstance.noOfCmpCreated).toBe(1);
...
counter$ = toObservable(signal(0));
    ...
    fixture.detectChanges();
    fixture.detectChanges();    expect(fixture.nativeElement.textContent).toBe('0');
  });
});
