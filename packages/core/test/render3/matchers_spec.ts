...
import {TViewType} from '@angular/core/src/render3/interfaces/view';import {isShapeOf, ShapeOf} from './is_shape_of';
import {matchDomElement, matchDomText, matchObjectShape, matchTNode, matchTView} from './matchers';describe('render3 matchers', () => {
  const fakeMatcherUtil = {equals: (a: any, b: any) => a === b} as jasmine.MatchersUtil;  describe('matchObjectShape', () => {
    interface MyShape {
propA: any;
propB: any;
...
expect({node: tView}).toEqual({node: matchTView({type: TViewType.Root})});
  ...
  describe('matchTNode', () => {
    const tView = createTView(TViewType.Root, null, null, 2, 3, null, null, null, null, null, null);
    const tNode = createTNode(tView, null, TNodeType.Element, 0, 'tagName', []);    it('should match', () => {
