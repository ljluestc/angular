...
import {TestBed} from '@angular/core/testing';import {ChangeDetectionStrategy, Component, Injector, Input, NgModuleRef, OnChanges, Output, RendererType2, SimpleChanges, ViewChild, ViewContainerRef, ViewEncapsulation} from '../../src/core';
import {ComponentFactory} from '../../src/linker/component_factory';
import {RendererFactory2} from '../../src/render/api';
import {Sanitizer} from '../../src/sanitization/sanitizer';import {MockRendererFactory} from './instructions/mock_renderer_factory';const THROWING_RENDERER_FACTOR2_PROVIDER = {
...
throw new Error('Incorrect injector for Renderer2');
  ...
  const cfr = new ComponentFactoryResolver();  describe('constructor()', () => {
...
class TestComponent {
}const cf = cfr.resolveComponentFactory(TestComponent);expect(cf.selector).toBe('test[foo],bar');
...
class TestComponent {
  @Input() in1: unknown;  @Input('input-attr-2') in2: unknown;  @Input({alias: 'input-attr-3', transform: transformFn}) in3: unknown;  @Output() out1: unknown;  @Output('output-attr-2') out2: unknown;
}const cf = cfr.resolveComponentFactory(TestComponent);expect(cf.componentType).toBe(TestComponent);
expect(cf.ngContentSelectors).toEqual(['*', 'a', 'b']);
expect(cf.selector).toBe('test[foo],bar');expect(cf.inputs).toEqual([
...
expect(cf.outputs).toEqual([
  ...
  {propName: 'out2', templateName: 'output-attr-2'},
    ...
    let cf: ComponentFactory<any>;    beforeEach(() => {
@Component({
  selector: 'test',
  template: '...',
  host: {
...
providers: [
 {provide: RendererFactory2, useValue: rendererFactorySpy},
  ...
  });  cf.create(injector);  expect(rendererFactorySpy.wasCalled).toBeTrue();
...
providers: [
 ...
 {provide: Sanitizer, useFactory: sanitizerFactorySpy, deps: []},
  ...
  });  cf.create(injector);  expect(sanitizerFactorySpy).toHaveBeenCalled();
});
...
providers: [
 {provide: RendererFactory2, useValue: rendererFactorySpy},
  ...
  const mInjector = Injector.create({providers: [THROWING_RENDERER_FACTOR2_PROVIDER]});  cf.create(injector, undefined, undefined, {injector: mInjector} as NgModuleRef<any>);  expect(rendererFactorySpy.wasCalled).toBeTrue();
});it('should retrieve `RendererFactory2` from the `ngModuleRef` if not provided by the injector',const injector = Injector.create({providers: []});
const mInjector = Injector.create({
  providers: [
{provide: RendererFactory2, useValue: rendererFactorySpy},
  ]
});cf.create(injector, undefined, undefined, {injector: mInjector} as NgModuleRef<any>);expect(rendererFactorySpy.wasCalled).toBeTrue();it('should retrieve `Sanitizer` from the specified injector first', () => {
...
providers: [
 ...
 {provide: Sanitizer, useFactory: mSanitizerFactorySpy, deps: []},
  ...
  });  cf.create(injector, undefined, undefined, {injector: mInjector} as NgModuleRef<any>);  expect(iSanitizerFactorySpy).toHaveBeenCalled();
  expect(mSanitizerFactorySpy).not.toHaveBeenCalled();
});it('should retrieve `Sanitizer` from the `ngModuleRef` if not provided by the injector',const injector = Injector.create({providers: []});const mSanitizerFactorySpy =
jasmine.createSpy('NgModuleRef#sanitizerFactory').and.returnValue({});
const mInjector = Injector.create({
  providers: [
{provide: RendererFactory2, useValue: rendererFactorySpy},
...
});cf.create(injector, undefined, undefined, {injector: mInjector} as NgModuleRef<any>);expect(mSanitizerFactorySpy).toHaveBeenCalled();    });    describe('(when the factory is bound to a `ngModuleRef`)', () => {
it('should retrieve `RendererFactory2` from the specified injector first', () => {
  const injector = Injector.create({
providers: [
 {provide: RendererFactory2, useValue: rendererFactorySpy},
...
};cf.create(injector);expect(rendererFactorySpy.wasCalled).toBeTrue();it('should retrieve `Sanitizer` from the specified injector first', () => {
  const iSanitizerFactorySpy =
 jasmine.createSpy('Injector#sanitizerFactory').and.returnValue({});
  const injector = Injector.create({
providers: [
...
injector: Injector.create({
 providers: [
  ...
  };  cf.create(injector);  expect(iSanitizerFactorySpy).toHaveBeenCalled();
  expect(mSanitizerFactorySpy).not.toHaveBeenCalled();
});it('should retrieve `Sanitizer` from the `ngModuleRef` if not provided by the injector',const injector = Injector.create({providers: []});const mSanitizerFactorySpy =
jasmine.createSpy('NgModuleRef#sanitizerFactory').and.returnValue({});
(cf as any).ngModule = {
...
};cf.create(injector);expect(mSanitizerFactorySpy).toHaveBeenCalled();    });    it('should ensure that rendererFactory is called after initial styling is set', () => {
...
const inputChangesLog: string[] = [];@Component({template: `{{in}}`})
...
}const fixture = TestBed.createComponent(DynamicCmp);fixture.detectChanges();
expect(fixture.nativeElement.textContent).toBe('');
expect(inputChangesLog).toEqual([]);fixture.componentRef.setInput('in', 'first');
fixture.detectChanges();
expect(fixture.nativeElement.textContent).toBe('first');
expect(inputChangesLog).toEqual(['undefined:first:true']);fixture.componentRef.setInput('in', 'second');
fixture.detectChanges();
...
expect(fixture.nativeElement.textContent).toBe('');fixture.componentRef.setInput('publicName', 'in value');
fixture.detectChanges();
expect(fixture.nativeElement.textContent).toBe('in value');fixture.componentRef.setInput('in', 'should not change');
fixture.detectChanges();
...
fixture.detectChanges();spyOn(console, 'error');
fixture.componentRef.setInput('doesNotExist', '');const msgL1 =
...
expect(fixture.nativeElement.textContent).toBe('');fixture.componentRef.setInput('in', 'pushed');
fixture.detectChanges();
...
class Wrapper {
  ...
  componentRef?: ComponentRef<DynamicCmp>;  create() {
...
fixture.detectChanges();
fixture.componentInstance.create();fixture.componentInstance.setInput('1');
fixture.detectChanges();
expect(fixture.nativeElement.innerText).toBe('1');fixture.componentInstance.setInput('2');
fixture.detectChanges();
