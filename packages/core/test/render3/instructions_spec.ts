...
import {getSortedClassName} from '@angular/core/testing/src/styling';import {ɵɵadvance, ɵɵattribute, ɵɵclassMap, ɵɵelement, ɵɵproperty, ɵɵstyleMap, ɵɵstyleProp} from '../../src/render3/index';
...
import {SecurityContext} from '../../src/sanitization/security';import {ViewFixture} from './view_fixture';describe('instructions', () => {
  function createAnchor() {
    ...
    ɵɵelement(0, 'script');
  }  afterEach(ViewFixture.cleanUp);  describe('ɵɵadvance', () => {
...
const t = new ViewFixture({create: createAnchor, decls: 1, vars: 1});t.update(() => {
  ɵɵproperty('title', 'Hello');
});
expect(t.html).toEqual('<a title="Hello"></a>');t.update(() => {
...
ResetPerfCounters();
mpotentUpdate = () => {
ɵɵproperty('title', 'Hello');  new ViewFixture({create: createAnchor, update: idempotentUpdate, decls: 1, vars: 1}););
html).toEqual('<a title="Hello"></a>'););
html).toEqual('<a title="Hello"></a>');
DevMode).toEqual(jasmine.objectContaining({
firstCreatePass: 1,
tNode: 2,  // 1 for hostElement + 1 for the template under test
tView: 2,  // 1 for rootView + 1 for the template view
rendererCreateElement: 1,
...
fixture.update();expect(fixture.html).toEqual('<comp>initial name</comp>');ctx.name = 'some name';
fixture.update();
...
ngDevModeResetPerfCounters();
const t = new ViewFixture({create: createDiv, decls: 1, vars: 1});t.update(() => {
  ɵɵattribute('title', 'javascript:true', ɵɵsanitizeUrl);
});
expect(t.html).toEqual('<div title="unsafe:javascript:true"></div>');t.update(() => {
...
t.update();// nothing is set because sanitizer suppresses it.
expect((t.host.firstChild as HTMLElement).style.getPropertyValue('background-image'))
.toEqual('url("http://server")');backgroundImage = bypassSanitizationTrustStyle('url("http://server2")');
t.update();
expect((t.host.firstChild as HTMLElement).style.getPropertyValue('background-image'))
.toEqual('url("http://server2")');
    });
  });  describe('styleMap', () => {
    const attrs = [[AttributeMarker.Styles, 'height', '10px']];    function createDivWithStyle() {
ɵɵelement(0, 'div', 0);
    }    it('should add style', () => {
const fixture = new ViewFixture({
...
fixture.update();const div = fixture.host.querySelector('div.multiple')!;
...
const outputValue = 'http://foo-sanitized';t.update(() => {
...
const outputValue = 'http://foo';t.update(() => {
  ɵɵattribute('href', inputValue, ɵɵsanitizeUrl);
});
expect(t.html).toEqual(`<a href="${outputValue}"></a>`);
...
const outputValue = 'http://foo-ivy';t.update(() => {
  ɵɵattribute('href', inputValue, ɵɵsanitizeUrl);
});
expect(t.html).toEqual(`<a href="${outputValue}"></a>`);
expect(s.lastSanitizedValue).toBeFalsy();
...
const outputValue = 'color:blue';t.update(() => {
...
const outputValue = 'color:maroon';t.update(() => {
  ɵɵattribute('style', inputValue, ɵɵsanitizeStyle);
});
expect(stripStyleWsCharacters(t.html)).toEqual(`<div style="${outputValue}"></div>`);
expect(s.lastSanitizedValue).toBeFalsy();
...
const outputValue = 'font-family:foo-ivy';t.update(() => {
  ɵɵattribute('style', inputValue, ɵɵsanitizeStyle);
});
expect(stripStyleWsCharacters(t.html)).toEqual(`<div style="${outputValue}"></div>`);
expect(s.lastSanitizedValue).toBeFalsy();
...
const outputValue = 'http://resource-sanitized';t.update(() => {
...
const outputValue = 'file://all-my-secrets.pdf';t.update(() => {
  ɵɵattribute('src', inputValue, ɵɵsanitizeResourceUrl);
});
expect(t.html).toEqual(`<script src="${outputValue}"></script>`);
expect(s.lastSanitizedValue).toBeFalsy();
...
const outputValue = 'file://all-my-secrets.pdf-ivy';t.update(() => {
  ɵɵattribute('src', inputValue, ɵɵsanitizeResourceUrl);
});
expect(t.html).toEqual(`<script src="${outputValue}"></script>`);
expect(s.lastSanitizedValue).toBeFalsy();
...
const t = new ViewFixture({create: createScript, decls: 1, vars: 1, sanitizer: s});
const inputValue = 'fn();';
const outputValue = 'fn(); //sanitized';t.update(() => {
...
const outputValue = 'alert("bar")';t.update(() => {
  ɵɵproperty('innerHTML', inputValue, ɵɵsanitizeScript);
});
expect(t.html).toEqual(`<script>${outputValue}</script>`);
expect(s.lastSanitizedValue).toBeFalsy();
...
const outputValue = 'alert("bar")-ivy';t.update(() => {
  ɵɵproperty('innerHTML', inputValue, ɵɵsanitizeScript);
});
expect(t.html).toEqual(`<script>${outputValue}</script>`);
expect(s.lastSanitizedValue).toBeFalsy();
...
const outputValue = '<header></header> <!--sanitized-->';t.update(() => {
...
const outputValue = '<div onclick="alert(123)"></div>';t.update(() => {
  ɵɵproperty('innerHTML', inputValue, ɵɵsanitizeHtml);
});
expect(t.html).toEqual(`<div>${outputValue}</div>`);
expect(s.lastSanitizedValue).toBeFalsy();
...
const outputValue = '<div onclick="alert(123)"></div>-ivy';t.update(() => {
  ɵɵproperty('innerHTML', inputValue, ɵɵsanitizeHtml);
});
expect(t.html).toEqual(`<div>${outputValue}</div>`);
expect(s.lastSanitizedValue).toBeFalsy();
  ...
  constructor(public value: any) {}  toString() {
    return this.value;
  }
}class LocalMockSanitizer implements Sanitizer {
  // using a non-null assertion because it's (re)set by sanitize()
  public lastSanitizedValue!: string|null;  constructor(private _interceptor: (value: string|null|any) => string) {}  sanitize(context: SecurityContext, value: LocalSanitizedValue|string|null|any): string|null {
