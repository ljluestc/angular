...
import {stringifyElement} from '@angular/platform-browser/testing/src/browser_util';import {extractDirectiveDef} from '../../src/render3/definition';
...
import {noop} from '../../src/util/noop';import {getRendererFactory2} from './imported_renderer2';/**
...
leaveView();
  ...
  }  private createFn?: () => void;  private updateFn?: () => void;  private context?: {};  /**
   * DOM element which acts as a host to the `LView`.
   */
  host: HTMLElement;  tView: TView;  lView: LView;  constructor({create, update, decls, vars, consts, context, directives, sanitizer}: {
    create?: () => void,
    update?: () => void,
    decls?: number,
    vars?: number,
    ...
    this.updateFn = update;    const document = ((typeof global == 'object' && global || window) as any).document;
    const rendererFactory = getRendererFactory2(document);    const hostRenderer = rendererFactory.createRenderer(null, null);
...
inlineEffectRunner: null,
  ...
  hostRenderer, null, null, null);    let template = noop;
    if (create) {
...
create();
  ...
  hostRenderer, null, null, null);    if (this.createFn) {
...
function toDefs(
  ...
  if (element) {
    let html = stringifyElement(element);    if (!keepNgReflect) {
