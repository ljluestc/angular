...
template: functionContaining([/element[^(]*\(0,'div'\)/]),
  ...
  ComponentDef<TestClass>;    expectComponentDef(def, {
selectors: [['', 'dir', ''], ['test']],
    });
...
contentQueries: functionContaining([
  ...
  '(ctx.byRef = _t)',  // "byToken" should use `staticContentQuery` with `3`
...
viewQuery: functionContaining([
  ...
  /viewQuery[^(]*\(_c0,4\)/,
  '(ctx.byRef = _t)',  // "byToken" should use `viewQuery` with `3`
  // (`QueryFlags.descendants|QueryFlags.isStatic`) for query flag and `ElementRef` as
  // read token, and bind to the first result in the query result.
...
type: TestClass,
template,
ponentDef<TestClass>;    expectComponentDef(whenTrue, {
...
directives: [TestCmp, TestDir],
 ...
 }) as ComponentDef<TestClass>;    @Directive({selector: '[forward]'})
    class ForwardDir {
...
pipes: [TestPipe],
 ...
 }) as ComponentDef<TestClass>;    @Pipe({name: 'forward'})
    class ForwardPipe {
    }    expectComponentDef(def, {
pipes: [ForwardPipe],
    });
...
.toEqual(expectation.providersResolver);
  ...
  expect(actual.data).withContext('data').toEqual(expectation.data);  const convertNullToEmptyArray = <T extends Type<any>[]|null>(arr: T): T =>
arr ?? ([] as unknown as T);  const directiveDefs =
typeof actual.directiveDefs === 'function' ? actual.directiveDefs() : actual.directiveDefs;
  ...
  expect(convertNullToEmptyArray(directiveTypes)).toEqual(expectation.directives);  const pipeDefs = typeof actual.pipeDefs === 'function' ? actual.pipeDefs() : actual.pipeDefs;
...
}class TestClass {}@Directive({selector: '[dir]'})
class TestDir {
