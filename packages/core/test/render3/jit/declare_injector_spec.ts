 ...
 */import {InjectionToken, ɵɵInjectorDef, ɵɵngDeclareInjector} from '@angular/core';describe('Injector declaration jit compilation', () => {
  it('should compile a minimal Injector declaration', () => {
    const def = ɵɵngDeclareInjector({type: TestClass}) as ɵɵInjectorDef<TestClass>;    expect(def.providers).toEqual([]);
 ...
 }) as ɵɵInjectorDef<TestClass>;    expect(def.providers).toEqual([OtherClass, {provide: TestToken, useValue: testTokenValue}]);
    expect(def.imports).toEqual([]);
  ...
  ɵɵInjectorDef<TestClass>;    expect(def.providers).toEqual([]);
