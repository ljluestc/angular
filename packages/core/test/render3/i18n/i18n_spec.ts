...
import {TNodeType} from '@angular/core/src/render3/interfaces/node';import {ɵɵelementEnd, ɵɵelementStart} from '../../../src/render3/instructions/all';
...
import {ViewFixture} from '../view_fixture';describe('Runtime i18n', () => {
  describe('getTranslationForTemplate', () => {
    it('should crop messages for the selected template', () => {
let message = `simple text`;
expect(getTranslationForTemplate(message, -1)).toEqual(message);message = `Hello �0�!`;
expect(getTranslationForTemplate(message, -1)).toEqual(message);message = `Hello �#2��0��/#2�!`;
expect(getTranslationForTemplate(message, -1)).toEqual(message);// Embedded sub-templates
...
expect(getTranslationForTemplate(message, 2)).toEqual('middle');// Embedded & sibling sub-templates
message =
...
expect(() => getTranslationForTemplate(message, -1)).toThrowError(/Tag mismatch/);
  ...
  });  let tView: TView;  function getOpCodes(
...
index: number): TI18n|I18nUpdateOpCodes {
    ...
    fixture.enterView();
    createTemplate();    // Make `tView` available for tests.
    tView = fixture.tView;
    const opCodes = fixture.tView.data[index] as TI18n | I18nUpdateOpCodes;    ViewFixture.cleanUp();
    return opCodes;
  }  describe('i18nStart', () => {
    it('for text', () => {
...
rt(index, 0);
End();
 }, nbConsts, HEADER_OFFSET + index) as TI18n;expect(opCodes).toEqual({
  create: matchDebug([
...
const opCodes = getOpCodes(message, () => {
  ...
  ɵɵi18nStart(index, 0);
  ɵɵelementEnd();
}, nbConsts, HEADER_OFFSET + index);expect(opCodes).toEqual({
  create: matchDebug([
...
const opCodes = getOpCodes(message, () => {
  ...
  ɵɵi18nStart(index, 0);
  ɵɵelementEnd();
}, nbConsts, HEADER_OFFSET + index);expect((opCodes as any).update.debug).toEqual([`if (mask & 0b1) { (lView[${
HEADER_OFFSET + 2}] as Text).textContent = \`Hello \${lView[i-1]}!\`; }`]);expect(opCodes).toEqual({
  create: matchDebug([
...
// </div>
const message =
`�0� is rendered as: �*2:1��#1:1�before�*2:2��#1:2�middle�/#1:2��/*2:2�after�/#1:1��/*2:1�!`;/**** Root template ****/
...
}, nbConsts, index + HEADER_OFFSET);expect(opCodes).toEqual({
  create: matchDebug([
...
const opCodes = getOpCodes(attrs, () => {
  ...
  ɵɵelementEnd();
}, nbConsts, HEADER_OFFSET + index);expect(opCodes).toEqual(matchDebug([
  `if (mask & 0b1) { (lView[${
...
const str = `[${arr.join('|')}]`;const cases = [
  // empty string
  ['', {}, ''],  // string without any special cases
  ['Foo [1,2,3] Bar - no ICU here', {}, 'Foo [1,2,3] Bar - no ICU here'],  // multi-value cases
  [
...
fixture.enterView();
ɵɵelementStart(0, 'div');
    });    afterEach(ViewFixture.cleanUp);    function i18nRangeOffset(offset: number): number {
