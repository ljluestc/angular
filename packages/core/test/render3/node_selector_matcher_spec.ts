 ...
 */import {createTNode} from '@angular/core/src/render3/instructions/shared';import {AttributeMarker, TAttributes, TNode, TNodeType} from '../../src/render3/interfaces/node';
import {CssSelector, CssSelectorList, SelectorFlags} from '../../src/render3/interfaces/projection';
import {extractAttrsAndClassesFromSelector, getProjectAsAttrValue, isNodeMatchingSelector, isNodeMatchingSelectorList, stringifyCSSSelectorList} from '../../src/render3/node_selector_matcher';function testLStaticData(tagName: string, attrs: TAttributes|null): TNode {
...
it('should match element name only if names are the same', () => {
 ...
 .toBeTruthy(`Selector 'span' should match <span>`);  expect(isMatching('span', null, ['div']))
...
// TODO: do we need to differentiate no value and empty value? that is: title vs. title="" ?it('should match single attribute without value', () => {
  expect(isMatching('span', ['title', ''], [
'', 'title', ''
  ])).toBeTruthy(`Selector '[title]' should match <span title>`);  expect(isMatching('span', ['title', 'my title'], [
'', 'title', ''
  ])).toBeTruthy(`Selector '[title]' should match <span title="my title">`);  expect(isMatching('span', ['name', 'name'], [
'', 'title', ''
  ])).toBeFalsy(`Selector '[title]' should NOT match <span name="name">`);  expect(isMatching('span', null, [
'', 'title', ''
  ])).toBeFalsy(`Selector '[title]' should NOT match <span>`);  expect(isMatching('span', ['title', ''], [
'', 'other', ''
...
xit('should match namespaced attributes', () => {
  expect(isMatching(
    ...
    ['', 'title', '']))
 .toBeTruthy();
});it('should match selector with one attribute without value when element has several attributes',expect(isMatching('span', ['id', 'my_id', 'title', 'test_title'], [
  '', 'title', ''
...
'', 'title', 'my title'
  ])).toBeTruthy(`Selector '[title="My Title"]' should match <span title="My Title">'`);  expect(isMatching('span', ['title', 'My Title'], [
'', 'title', 'other title'
  ...
  expect(isMatching('span', ['title', 'My Title'], [
'div', 'title', ''
  ])).toBeFalsy(`Selector 'div[title]' should NOT match <span title="My Title">`);  expect(isMatching('span', ['title', 'My Title'], [
'div', 'title', 'my title'
  ...
  const selector = ['', 'title', 'title', 'name', 'name'];  // <span title="title" name="name">
  expect(isMatching('span', ['title', 'title', 'name', 'name'], selector))
 .toBeTruthy(
 `Selector '[title=title][name=name]' should NOT match <span title="title" name="name">`);  // <span title="title">
  expect(isMatching('span', ['title', 'title'], selector))
 .toBeFalsy(`Selector '[title=title][name=name]' should NOT match <span title="title">`);  // <span name="name">
 ...
 .toBeFalsy(`Selector '[title=title][name=name]' should NOT match <span name="name">`);
  ...
  // selector: [name=name]
  const selector = ['', 'name', 'name'];  // <span title="name">
  expect(isMatching('span', ['title', 'name'], selector))
 .toBeFalsy(`Selector '[name=name]' should NOT match <span title="name">`);  // <span title="name" name="name">
...
'', 'foo', ''
  ])).toBeTruthy(`Selector '[foo]' should match <span foo>`);  expect(isMatching('span', ['foo', ''], [
'', 'Foo', ''
...
'', 'class', ''
  ])).toBeTruthy(`Selector '[class]' should match <span class="foo">`);  expect(isMatching('span', ['class', 'foo'], [
'', 'class', 'foo'
...
});it('should not match optional binding attribute names if attribute selector has value',expect(isMatching('span', [AttributeMarker.Bindings, 'directive'], [
  '', 'directive', 'value'
])).toBeFalsy(`Selector '[directive=value]' should not match <span [directive]="exp">`);it('should not match optional binding attribute names if attribute selector has value and next name equals to value',expect(isMatching(
 'span', [AttributeMarker.Bindings, 'directive', 'value'],
 ['', 'directive', 'value']))
.toBeFalsy(
    `Selector '[directive=value]' should not match <span [directive]="exp" [value]="otherExp">`);it('should match bound attributes that come after classes', () => {
  expect(isMatching(
    'span',
    [
...
'', SelectorFlags.CLASS, 'foo'
  ])).toBeTruthy(`Selector '.foo' should match <span class="foo bar">`);  expect(isMatching('span', ['class', 'foo bar'], [
'', SelectorFlags.CLASS, 'bar'
  ])).toBeTruthy(`Selector '.bar' should match <span class="foo bar">`);  expect(isMatching('span', ['class', 'foo bar'], [
...
'', SelectorFlags.CLASS, 'foo'
  ])).toBeFalsy(`Selector '.foo' should NOT match <span class="foobar">`);  expect(isMatching('span', ['class', 'foobar'], [
'', SelectorFlags.CLASS, 'bar'
  ])).toBeFalsy(`Selector '.bar' should NOT match <span class="foobar">`);  expect(isMatching('span', ['class', 'foobar'], [
'', SelectorFlags.CLASS, 'ob'
  ])).toBeFalsy(`Selector '.ob' should NOT match <span class="foobar">`);  expect(isMatching('span', ['class', 'foobar'], [
...
'', SelectorFlags.CLASS, 'foo', 'bar'
  ])).toBeTruthy(`Selector '.foo.bar' should match <span class="foo bar">`);  expect(isMatching('span', ['class', 'foo'], [
'', SelectorFlags.CLASS, 'foo', 'bar'
  ])).toBeFalsy(`Selector '.foo.bar' should NOT match <span class="foo">`);  expect(isMatching('span', ['class', 'bar'], [
'', SelectorFlags.CLASS, 'foo', 'bar'
...
'', SelectorFlags.CLASS, 'bar', 'foo'
  ])).toBeTruthy(`Selector '.bar.foo' should match <span class="foo bar">`);  expect(isMatching('span', ['class', 'bar foo'], [
'', SelectorFlags.CLASS, 'foo', 'bar'
  ])).toBeTruthy(`Selector '.foo.bar' should match <span class="bar foo">`);  expect(isMatching('span', ['class', 'bar foo'], [
'', SelectorFlags.CLASS, 'bar', 'foo'
...
'', SelectorFlags.CLASS, 'foo'
  ...
  // selector: '.foo'
  const selector = ['', SelectorFlags.CLASS, 'foo'];  // <div title="title">
  expect(isMatching('div', ['title', 'title'], selector))
 .toBeFalsy(`Selector '.foo' should NOT match <div title="title">`);  // <div>
 ...
 .toBeFalsy(`Selector '.foo' should NOT match <div>`);
  ...
  const selector = ['div', 'title', 'title', SelectorFlags.CLASS, 'foo'];  // <div class="foo" title="title">
  expect(isMatching('div', ['class', 'foo', 'title', 'title'], selector)).toBeTruthy();  // <div title="title">
  expect(isMatching('div', ['title', 'title'], selector)).toBeFalsy();  // <div class="foo">
...
])).toBeFalsy(`Selector ':not(span)' should NOT match <span foo="">`);expect(isMatching('span', ['foo', ''], [
...
const selector = ['', SelectorFlags.NOT | SelectorFlags.ELEMENT, 'span', 'foo', ''];// <span foo="">
expect(isMatching('span', ['foo', ''], selector)).toBeFalsy();// <span bar="">
expect(isMatching('span', ['bar', ''], selector)).toBeTruthy();
    });    it('should not match negative classes', () => {
// selector: ':not(.foo.bar)'
const selector = ['', SelectorFlags.NOT | SelectorFlags.CLASS, 'foo', 'bar'];// <span class="foo bar">
expect(isMatching('span', ['class', 'foo bar'], selector)).toBeFalsy();// <span class="foo">
expect(isMatching('span', ['class', 'foo'], selector)).toBeTruthy();// <span class="bar">
...
expect(isMatching('div', ['class', 'baz'], selector)).toBeTruthy();// <div title="title">
expect(isMatching('div', ['title', 'title'], selector)).toBeTruthy();// <div class="baz" title="title">
...
expect(isMatching('div', ['class', 'baz'], selector)).toBeTruthy();// <div class="baz" title="">
expect(isMatching('div', ['class', 'baz', 'title', ''], selector)).toBeFalsy();// <div class="baz" foo="">
...
expect(isMatching('div', ['foo', ''], selector)).toBeFalsy();// <span bar="">
expect(isMatching('span', ['bar', ''], selector)).toBeFalsy();// <div bar="">
...
expect(isMatching('div', ['name', 'name', 'class', 'foo bar'], selector)).toBeTruthy();// <div name="name" class="foo bar baz">
expect(isMatching('div', ['name', 'name', 'class', 'foo bar baz'], selector)).toBeFalsy();// <div name="name" title class="foo bar">
...
.toBeTruthy(`Selector 'span' should match <span>`);expect(isAnyMatching('span', null, [['div']]))
.toBeFalsy(`Selector 'div' should NOT match <span>`);
...
expect(getProjectAsAttrValue(testTNode(['foo', AttributeMarker.ProjectAs]))).toBe(null);
  ...
  .toBe('button[id="value"][title="other"]');
    ...
    expect(stringifyCSSSelectorList([['button', SelectorFlags.CLASS, 'foo']])).toBe('button.foo');    expect(stringifyCSSSelectorList([['button', SelectorFlags.CLASS, 'foo', 'bar']]))
  .toBe('button.foo.bar');    expect(stringifyCSSSelectorList([
['button', 'id', 'value', 'title', 'other', SelectorFlags.CLASS, 'foo', 'bar']
  ...
  .toBe(':not(.foo.bar)');    expect(stringifyCSSSelectorList([
['button', SelectorFlags.ATTRIBUTE | SelectorFlags.NOT, 'foo', 'bar']
    ])).toBe('button:not([foo="bar"])');    expect(stringifyCSSSelectorList([['', SelectorFlags.ELEMENT | SelectorFlags.NOT, 'foo']]))
  .toBe(':not(foo)');    expect(stringifyCSSSelectorList([
['span', SelectorFlags.CLASS, 'foo', SelectorFlags.CLASS | SelectorFlags.NOT, 'bar', 'baz']
    ])).toBe('span.foo:not(.bar.baz)');    expect(stringifyCSSSelectorList([
['span', 'id', 'value', SelectorFlags.ATTRIBUTE | SelectorFlags.NOT, 'title', 'other']
    ])).toBe('span[id="value"]:not([title="other"])');    expect(stringifyCSSSelectorList([[
'', SelectorFlags.CLASS, 'bar', SelectorFlags.ATTRIBUTE | SelectorFlags.NOT, 'foo', '',
SelectorFlags.ELEMENT | SelectorFlags.NOT, 'div'
    ]])).toBe('.bar:not([foo]):not(div)');    expect(stringifyCSSSelectorList([[
'div', SelectorFlags.ATTRIBUTE | SelectorFlags.NOT, 'foo', '', SelectorFlags.CLASS, 'bar',
SelectorFlags.CLASS | SelectorFlags.NOT, 'baz'
    ]])).toBe('div:not([foo].bar):not(.baz)');    expect(stringifyCSSSelectorList([[
...
['', 'id', ''], ['button', 'id', 'value']
    ])).toBe('[id],button[id="value"]');    expect(stringifyCSSSelectorList([
['', 'id', ''], ['button', 'id', 'value'],
['div', SelectorFlags.ATTRIBUTE | SelectorFlags.NOT, 'foo', '']
    ])).toBe('[id],button[id="value"],div:not([foo])');    expect(stringifyCSSSelectorList([
['', 'id', ''], ['button', 'id', 'value'],
['div', SelectorFlags.ATTRIBUTE | SelectorFlags.NOT, 'foo', ''],
[
  'div', SelectorFlags.ELEMENT | SelectorFlags.NOT, 'p', SelectorFlags.CLASS, 'bar',
