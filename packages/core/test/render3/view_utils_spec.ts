...
import {ViewFixture} from './view_fixture';describe('view_utils', () => {
  it('should verify unwrap methods (isLView and isLContainer)', () => {
    ...
    const lContainer =
  createLContainer(viewFixture.lView, viewFixture.lView, viewFixture.host, tNode);    expect(isLView(viewFixture.lView)).toBe(true);
    expect(isLView(lContainer)).toBe(false);    expect(isLContainer(viewFixture.lView)).toBe(false);
