...
import {ComponentDef, PipeDef,} from '@angular/core/src/render3/interfaces/definition';describe('render3 jit', () => {
  let injector: any;
  beforeAll(() => {
...
selector: 'test-cmp',
    ...
    const SomeCmpAny = SomeCmp as any;    expect(SomeCmpAny.ɵcmp).toBeDefined();
...
selector: 'inner-cmp',
  ...
  rawDirectiveDefs! instanceof Function ? rawDirectiveDefs!() : rawDirectiveDefs!;
    ...
    const ServiceAny = Service as any;    expect(ServiceAny.ɵprov).toBeDefined();
...
get value(): any {
  return null;
    ...
    const ServiceAny = Service as any;    expect(ɵɵinject(Service).value).toBe('test');
...
constructor(public token: Token) {}
    ...
    const instance = factory();    // Since the instance was created outside of an injector using the module, the
    // injection will use the default provider, not the provider from the module.
    expect(instance.token).toBe('default');    const injectorDef: ɵɵInjectorDef<Module> = (Module as any).ɵinj;
...
selector: 'foo',
    ...
    const cmpDef: ComponentDef<Cmp> = (Cmp as any).ɵcmp;    expect(cmpDef.directiveDefs).toBeNull();    @NgModule({
declarations: [Cmp],
    })
    class Module {
    }    const moduleDef: NgModuleDef<Module> = (Module as any).ɵmod;
...
host: {
  ...
  '(click)': 'onClick()',
    ...
    class Cmp {
@HostBinding('class.green') green: boolean = false;@HostListener('change', ['$event'])
onChange(event: any): void {
}
    }    const cmpDef = (Cmp as any).ɵcmp as ComponentDef<Cmp>;    expect(cmpDef.hostBindings).toBeDefined();
...
@ContentChild('foo') foo: ElementRef|undefined;
    ...
    class SomeDir {}    @Directive({selector: '[test]'})
    class TestDirective {
...
le({providedIn: 'root'})
it {eDir {
constructor(first: Legit) {}e({selector: 'test'})
tDir extends BaseDir {tDirAny = TestDir as any;stDirAny.ɵfac).toBeDefined();
 => TestDirAny.ɵfac())
  .toThrowError(
