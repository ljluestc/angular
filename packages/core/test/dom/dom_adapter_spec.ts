...
describe('getBaseHref', () => {
  beforeEach(() => getDOM().resetBaseElement());  it('should return null if base element is absent', () => {
...
headEl.appendChild(baseEl);const baseHref = getDOM().getBaseHref(defaultDoc);
headEl.removeChild(baseEl);
getDOM().resetBaseElement();expect(baseHref).toEqual('/drop/bass/connon/');
...
headEl.appendChild(baseEl);const baseHref = getDOM().getBaseHref(defaultDoc)!;
headEl.removeChild(baseEl);
getDOM().resetBaseElement();expect(baseHref.endsWith('/base')).toBe(true);
  });
});
    }
  });
