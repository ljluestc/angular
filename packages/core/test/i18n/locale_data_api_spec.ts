...
import {global} from '../../src/util/global';{
  describe('locale data api', () => {
    ...
    const fakeGlobalFr: any[] = ['fr'];    beforeAll(() => {
...
registerLocaleData(localeEnAU);// Simulate some locale data existing on the global already
global.ng = global.ng || {};
...
it('should throw if the LOCALE_DATA for the chosen locale or its parent locale is not available',expect(() => findLocaleData('pt-AO'))
.toThrowError(/Missing locale data for the locale "pt-AO"/);it('should return english data if the locale is en-US', () => {
...
});it('should return the parent LOCALE_DATA if it exists and exact locale is not available',expect(findLocaleData('fr-BE')).toEqual(localeFr);it(`should find the LOCALE_DATA even if the locale id is badly formatted`, () => {
...
});it('should find the parent LOCALE_DATA if the exact locale is not available and the parent locale is on the global object',expect(findLocaleData('de-BE')).toEqual(localeDe);it('should find the registered LOCALE_DATA even if the same locale is on the global object',expect(findLocaleData('fr')).not.toBe(fakeGlobalFr);    });    describe('getLocaleCurrencyCode()', () => {
it('should return `null` if the given locale does not provide a currency code', () => {
  expect(getLocaleCurrencyCode('de')).toBe(null);
});it('should return the code at the `LocaleDataIndex.CurrencyCode` of the given locale`s data',expect(getLocaleCurrencyCode('fr')).toEqual('EUR');    });
  });
}
