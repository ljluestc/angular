...
import {expect} from '@angular/platform-browser/testing/src/matchers';import {global} from '../../src/util/global';describe('component', () => {
...
`
 ...
 class App {
d('insertionPoint', {read: ViewContainerRef}) viewContainerRef!: ViewContainerRef;
 } TestBed.configureTestingModule({declarations: [App, Wrapper, HelloComponent]});
 const fixture = TestBed.createComponent(App);
 fixture.detectChanges(); const instance = fixture.componentInstance;
 instance.viewContainerRef.createComponent(HelloComponent); expect(fixture.nativeElement.textContent.trim()).toBe('hello');
...
d('insertionPoint', {read: ViewContainerRef}) viewContainerRef!: ViewContainerRef;
 ...
 const fixture = TestBed.createComponent(App);
 fixture.detectChanges(); const componentRef =
...
let wasOnDestroyCalled = false;@Component({selector: 'comp-with-destroy', template: ``})
...
oy() {
wasOnDestroyCalled = true; } @Component({
 'button',
 `
...
<div class="wrapper"></div> })
 class App {
Ref!: ComponentRef<DynamicComponent>;or(
...
this.componentRef.destroy(); } TestBed.configureTestingModule({declarations: [App, DynamicComponent]});
 const fixture = TestBed.createComponent(App);
 fixture.detectChanges(); let appRootEl = fixture.nativeElement.querySelector('#app-root');
 expect(appRootEl).toBeDefined();
 expect(appRootEl.innerHTML).toBe('');  // app container content is empty fixture.componentInstance.create(); appRootEl = fixture.nativeElement.querySelector('#app-root');
 expect(appRootEl).toBeDefined();
 expect(appRootEl.innerHTML).toBe('comp content'); fixture.componentInstance.destroy();
 fixture.detectChanges(); appRootEl = fixture.nativeElement.querySelector('#app-root');
...
it('should invoke `onDestroy` callbacks of dynamically created component with ' + mode,if (mode === 'ngDevMode off') {
...
class App {
  @ViewChild('anchor', {read: ViewContainerRef}) anchor!: ViewContainerRef;  constructor(private vcr: ViewContainerRef, private injector: Injector) {}  create() {
const componentRef =
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();// Add ComponentRef to ViewContainerRef instance.
fixture.componentInstance.create();
// Clear ViewContainerRef to invoke `onDestroy` callbacks on ComponentRef.
fixture.componentInstance.clear();expect(wasOnDestroyCalled).toBeTrue();    });
...
class App {
}let textContent = '';expect(() => {
  const fixture = TestBed.createComponent(App);
  fixture.detectChanges();
  textContent = fixture.nativeElement.textContent.trim();
}).not.toThrow();// Ensure that the component actually rendered.
...
constructor(elementRef: ElementRef, renderer: Renderer2) {
  ...
  renderer.appendChild(elementRef.nativeElement, elementFromRenderer);
    ...
    fixture.detectChanges();    const secondParentEl: HTMLElement = fixture.nativeElement.querySelector('parent-comp');
...
return Array.from(element.attributes).map(a => a.name).find(a => /ngcontent/.test(a));
    ...
    const viewNgContentAttr = getNgContentAttr(elementFromRenderer);    expect(hostNgContentAttr)
  .not.toBe(
 viewNgContentAttr,
...
constructor(public renderer: Renderer2) {}
    ...
    fixture.detectChanges();    // Assert like this, rather than `.not.toBe` so we get a better failure message.
...
ngDoCheck() {
  log.push('CompB:ngDoCheck');
    ...
    class MyCompA {
constructor(private _injector: EnvironmentInjector) {}createComponent() {
 ...
 {environmentInjector: this._injector, hostElement: document.createElement('div')});
    ...
    class MyCompB {
constructor(private envInjector: EnvironmentInjector) {}createComponent() {
  return createComponent(
 CompB,
...
imports: [MyModuleA, MyModuleB],
    ...
    compA.instance.a = 'a';
    compA.changeDetectorRef.detectChanges();    expect(log).toEqual(['CompA:ngDoCheck']);    log.length = 0;  // reset the log    const fixtureB = TestBed.createComponent(MyCompB);
    fixtureB.detectChanges();
    const compB = fixtureB.componentInstance.createComponent();
    compB.instance.b = 'b';
    compB.changeDetectorRef.detectChanges();    expect(log).toEqual(['CompB:ngDoCheck']);
  });  it('should preserve simple component selector in a component factory', () => {
    @Component({selector: '[foo]', template: ''})
    class AttSelectorCmp {
    }    const selector = reflectComponentType(AttSelectorCmp)?.selector;    expect(selector).toBe('[foo]');
    ...
    }    const selector = reflectComponentType(ComplexSelectorCmp)?.selector;    expect(selector).toBe('[foo],div:not(.bar)');
...
`,
    ...
    class App {
constructor(public injector: EnvironmentInjector) {}createDynamicComponent(target: any) {
  createComponent(
...
providers: [
  {provide: DOCUMENT, useFactory: _document, deps: []},
    ...
    fixture.detectChanges();    // Create an instance of DynamicComponent and provide host element *reference*
    ...
    expect(targetEl.innerHTML).toContain('DynamicComponent Content');    // Create an instance of DynamicComponent and provide host element *selector*
...
const componentRef = createComponent(StandaloneComponent, {hostElement, environmentInjector});componentRef.changeDetectorRef.detectChanges();
expect(hostElement.textContent).toBe('Hello Angular!');// Verify basic change detection works.
...
const environmentInjector = TestBed.inject(EnvironmentInjector);
const componentRef =
createComponent(NgModuleBasedComponent, {hostElement, environmentInjector});componentRef.changeDetectorRef.detectChanges();
expect(hostElement.textContent).toBe('Hello Angular!');// Verify basic change detection works.
componentRef.instance.name = 'ZoneJS';
componentRef.changeDetectorRef.detectChanges();
expect(hostElement.textContent).toBe('Hello ZoneJS!');
...
StandaloneComponent, {hostElement, environmentInjector, projectableNodes});componentRef.changeDetectorRef.detectChanges();
...
componentRef.changeDetectorRef.detectChanges();expect(hostElement.textContent).toBe('Token: EnvironmentInjector(A)');
...
class RootStandaloneComponent {
  ...
  constructor(private injector: Injector) {}  createChildComponent() {
...
fixture.detectChanges();fixture.componentInstance.createChildComponent();const rootEl = fixture.nativeElement;// Token A is coming from the Element Injector, token B - from the Environment Injector.
...
componentRef.changeDetectorRef.detectChanges();const hostElement =
(componentRef.hostView as EmbeddedViewRef<StandaloneComponent>).rootNodes[0];// A host element that matches component's selector.
expect(hostElement.tagName.toLowerCase()).toBe(selector);expect(hostElement.textContent).toBe('Hello Angular!');
...
selector: '.some-class',
standalone: true,
template: 'Hello {{ name }}!',ndaloneComponent {
name = 'Angular';ironmentInjector = TestBed.inject(EnvironmentInjector);
ponentRef = createComponent(StandaloneComponent, {environmentInjector});
Ref.changeDetectorRef.detectChanges();tElement =
  (componentRef.hostView as EmbeddedViewRef<StandaloneComponent>).rootNodes[0]; element has the `div` tag name, since component's selector doesn't contain
me information (only a class name).
stElement.tagName.toLowerCase()).toBe('div');stElement.textContent).toBe('Hello Angular!');
 });    describe('error checking', () => {
it('should throw when provided class is not a component', () => {
  class NotAComponent {}  @Directive()
  class ADirective {
  }  @Injectable()
  class AnInjectiable {
 ...
 `make sure it has the \`@Component\` decorator.`;
  const hostElement = document.createElement('div');
  const environmentInjector = TestBed.inject(EnvironmentInjector);  expect(() => createComponent(NotAComponent, {hostElement, environmentInjector}))
 .toThrowError(errorFor(NotAComponent));  expect(() => createComponent(ADirective, {hostElement, environmentInjector}))
 .toThrowError(errorFor(ADirective));  expect(() => createComponent(AnInjectiable, {hostElement, environmentInjector}))
...
function transformFn() {}@Component({
...
}const mirror = reflectComponentType(StandaloneComponent)!;expect(mirror.selector).toBe('standalone-component');
...
class NonStandaloneComponent {
  @Input({alias: 'input-alias-c', transform: transformFn}) inputC: unknown;
}const mirror = reflectComponentType(NonStandaloneComponent)!;expect(mirror.selector).toBe('non-standalone-component');
