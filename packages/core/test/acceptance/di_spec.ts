...
import {BehaviorSubject} from 'rxjs';const getProvidersByToken =
    (providers: Provider[], token: Type<unknown>|InjectionToken<unknown>): Provider[] =>
  providers.filter(provider => (provider as any).provide === token);const hasProviderWithToken = (providers: Provider[], token: InjectionToken<unknown>): boolean =>
    getProvidersByToken(providers, token).length > 0;const collectEnvironmentInitializerProviders = (providers: Provider[]) =>
    getProvidersByToken(providers, ENVIRONMENT_INITIALIZER);function unwrappedImportProvidersFrom(...sources: ImportProvidersSource[]): Provider[] {
  const providers =
(importProvidersFrom(...sources) as unknown as InternalEnvironmentProviders).ɵproviders;
  ...
  const D = new InjectionToken('D');  it('should collect providers from NgModules', () => {
    @NgModule({
providers: [
...
providers: [
  ...
  {provide: B, useValue: 'B'},
    ...
    const providers = unwrappedImportProvidersFrom(MyModule);    // 4 tokens (A, B, C, D) + 2 providers for each NgModule:
    // - the definition type itself
    // - `INJECTOR_DEF_TYPES`
    // - `ENVIRONMENT_INITIALIZER`
    expect(providers.length).toBe(10);    expect(hasProviderWithToken(providers, A)).toBe(true);
    expect(hasProviderWithToken(providers, B)).toBe(true);
    expect(hasProviderWithToken(providers, C)).toBe(true);
    expect(hasProviderWithToken(providers, D)).toBe(true);    // Expect 2 `ENVIRONMENT_INITIALIZER` providers: one for `MyModule`, another was `MyModule2`
...
{provide: A, useValue: 'A'}  // })
 ...
 class ModuleB {
rRoot(): ModuleWithProviders<ModuleB> {
return {ngModule: ModuleB, providers: [{provide: B, useValue: 'B'}]};rChild(): ModuleWithProviders<ModuleB> {
return {ngModule: ModuleB, providers: [{provide: C, useValue: 'C'}]}; } const providers = unwrappedImportProvidersFrom(ModuleB.forRoot(), ModuleB.forChild()); // Expect 2 `ENVIRONMENT_INITIALIZER` providers: one for `ModuleA`, another one for `ModuleB`
 expect(collectEnvironmentInitializerProviders(providers).length).toBe(2); // Expect exactly 1 provider for each module: `ModuleA` and `ModuleB`
 expect(getProvidersByToken(providers, ModuleA).length).toBe(1);
 expect(getProvidersByToken(providers, ModuleB).length).toBe(1); // Expect all tokens to be collected.
 expect(hasProviderWithToken(providers, A)).toBe(true);
 expect(hasProviderWithToken(providers, B)).toBe(true);
 expect(hasProviderWithToken(providers, C)).toBe(true);
...
providers: [
 ...
 [{provide: B, useValue: 'B'}, [{provide: C, useValue: 'C'}]]
    ...
    }    const providers = unwrappedImportProvidersFrom(ModuleA.forRoot());    // Expect 1 `ENVIRONMENT_INITIALIZER` provider (for `ModuleA`)
    expect(collectEnvironmentInitializerProviders(providers).length).toBe(1);    // Expect exactly 1 provider for `ModuleA`
    expect(getProvidersByToken(providers, ModuleA).length).toBe(1);    // Expect all tokens to be collected.
    expect(hasProviderWithToken(providers, A)).toBe(true);
    expect(hasProviderWithToken(providers, B)).toBe(true);
    expect(hasProviderWithToken(providers, C)).toBe(true);
 ...
 @NgModule()
 class ModuleA {
rRoot(): ModuleWithProviders<ModuleA> {
return {
  ngModule: ModuleA,
  providers: [
{provide: A, useValue: 'A'},
...
[{provide: B, useValue: 'B'}, [{provide: C, useValue: 'C'}]]
 ...
 class ModuleB {
 } const providers = unwrappedImportProvidersFrom(ModuleB); // Expect 2 `ENVIRONMENT_INITIALIZER` providers: one for `ModuleA`, another one for `ModuleB`
 expect(collectEnvironmentInitializerProviders(providers).length).toBe(2); // Expect exactly 1 provider for each module: `ModuleA` and `ModuleB`
 expect(getProvidersByToken(providers, ModuleA).length).toBe(1);
 expect(getProvidersByToken(providers, ModuleB).length).toBe(1); // Expect all tokens to be collected.
 expect(hasProviderWithToken(providers, A)).toBe(true);
...
}; } const providers = unwrappedImportProvidersFrom(ModuleA.forRoot()); // Expect all tokens to be collected.
 expect(hasProviderWithToken(providers, A)).toBe(true);
 expect(hasProviderWithToken(providers, B)).toBe(true);
 expect(hasProviderWithToken(providers, C)).toBe(true);
 expect(hasProviderWithToken(providers, D)).toBe(true); const parentEnvInjector = TestBed.inject(EnvironmentInjector);
 const injector = createEnvironmentInjector(providers, parentEnvInjector); // Verify that overridden token A has the right value.
 expect(injector.get(A)).toBe('Overridden A'); // Verify that a multi-provider has both values.
...
class Cmp {
    ...
    class ModuleB {
    }    const providers = unwrappedImportProvidersFrom([ModuleA, [ModuleB]]);    expect(hasProviderWithToken(providers, A)).toBeTrue();
...
imports: [mwp],
  ...
  createEnvironmentInjector([environmentProviders], TestBed.inject(EnvironmentInjector));
    ...
    TestBed.configureTestingModule({});    const inj = createEnvironmentInjector([wrappedProviders], TestBed.inject(EnvironmentInjector));
    expect(inj.get(TOKEN)).toEqual('token!');
...
fixture.detectChanges();const divElement = fixture.nativeElement.querySelector('div');
expect(divElement.textContent).toContain('Created');
    });
  });  describe('multi providers', () => {
    it('should process ModuleWithProvider providers after module imports', () => {
const testToken = new InjectionToken<string[]>('test-multi');@NgModule({providers: [{provide: testToken, useValue: 'A', multi: true}]})
class TestModuleA {
...
TestModuleB,
    ...
    let log: string[] = [];    @Directive({selector: '[dirB]', exportAs: 'dirB'})
    class DirectiveB {
@Input() value = 'DirB';constructor() {
  log.push(this.value);
}
    }    beforeEach(() => log = []);    it('should create directive with intra view dependencies', () => {
@Directive({selector: '[dirA]', exportAs: 'dirA'})
class DirectiveA {
  value = 'DirA';
}@Directive({selector: '[dirC]', exportAs: 'dirC'})
class DirectiveC {
  value: string;  constructor(dirA: DirectiveA, dirB: DirectiveB) {
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();const divElement = fixture.nativeElement.querySelector('span');
expect(divElement.textContent).toContain('DirADirB');
    });    it('should instantiate injected directives in dependency order', () => {
@Directive({selector: '[dirA]'})
class DirectiveA {
  value = 'dirA';  constructor(dirB: DirectiveB) {
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();expect(log).toEqual(['DirB', 'DirA (dep: DirB)']);
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();expect(log).toEqual(['DirB', 'DirA (dep: module)']);
...
fixture.detectChanges();expect(log).toEqual(['DirB', 'Comp (dep: DirB)']);
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();expect(log).toEqual(
...
@Directive({selector: '[dirA]'})
class DirectiveA {
  value = 'DirA';  constructor() {
log.push(this.value);
  }
}@Directive({selector: '[dirC]'})
class DirectiveC {
  value = 'DirC';  constructor() {
log.push(this.value);
  }
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();expect(log).toEqual(['DirA', 'DirC', 'DirB (deps: DirA and DirC)']);
    });    it('should instantiate in the correct order for complex case', () => {
@Directive({selector: '[dirC]'})
class DirectiveC {
  value = 'DirC';  constructor(dirB: DirectiveB) {
log.push(`DirC (dep: ${dirB.value})`);
  }
}@Directive({selector: '[dirA]'})
class DirectiveA {
  value = 'DirA';  constructor(dirC: DirectiveC) {
log.push(`DirA (dep: ${dirC.value})`);
  }
}@Directive({selector: '[dirD]'})
class DirectiveD {
  value = 'DirD';  constructor(dirA: DirectiveA) {
...
const fixture = TestBed.createComponent(MyApp);
fixture.detectChanges();expect(log).toEqual(['DirB', 'DirA (deps: DirB and App)']);
    });    it('should not use a parent when peer dep is available', () => {
let count = 1;@Directive({selector: '[dirB]'})
class DirectiveB {
  count: number;  constructor() {
log.push(`DirB`);
this.count = count++;
  }
}@Directive({selector: '[dirA]'})
...
const fixture = TestBed.createComponent(MyApp);
fixture.detectChanges();expect(log).toEqual(['DirB', 'DirB', 'DirA (dep: DirB - 2)']);
    });    describe('dependencies in parent views', () => {
@Directive({selector: '[dirA]', exportAs: 'dirA'})
class DirectiveA {
  injector: Injector;  constructor(public dirB: DirectiveB, public vcr: ViewContainerRef) {
...
@Input() tmp!: TemplateRef<any>;constructor(public vcr: ViewContainerRef) {}create() {
...
@ViewChild(StructuralDirective) structuralDir!: StructuralDirective;
 ...
 {declarations: [StructuralDirective, DirectiveA, DirectiveB, MyComp]});
  ...
  fixture.detectChanges();  const divElement = fixture.nativeElement.querySelector('div[value=insertion]');
...
@ViewChild(DirectiveA) dirA!: DirectiveA;
 ...
 {declarations: [DirectiveA, DirectiveB, HostBindingDirective, MyApp]});
  ...
  fixture.detectChanges();  const divElement = fixture.nativeElement.querySelector('div');
  expect(divElement.textContent).toEqual('DirB');
  expect(divElement.id).toEqual('foo');  const dirA = fixture.componentInstance.dirA;
  expect(dirA.vcr.injector).toEqual(dirA.injector);  const hostBindingDir = fixture.componentInstance.hostBindingDir;
...
childOriginWithDirB!: ViewContainerRef;addChild() {
...
expect(() => TestBed.createComponent(MyComp)).toThrowError(/No provider for DirectiveB/);
 ...
 () => {
//github.com/angular/angular/issues/42651
tA {
constructor(public injector: string) {}tB {
constructor(public a: TestA) {}createTestB() {
...
return child.get(TestB);t({
template: '',
providers: [
  {provide: TestA, useFactory: () => new TestA('component'), deps: []},
  {provide: TestB, useFactory: createTestB},
],omp {
constructor(public readonly testB: TestB) {}onfigureTestingModule({declarations: [MyComp]}); = TestBed.createComponent(MyComp);
p.componentInstance.testB).toBeInstanceOf(TestB);
p.componentInstance.testB.a.injector).toBe('standalone');
 });    it('should not have access to the directive injector in a standalone injector from within a directive-level provider factory',
 () => {
tA {
constructor(public injector: string) {}tB {
constructor(public a: TestA|null) {}createTestB() {
...
return injector.get(TestB);t({
template: '',
providers: [
  {provide: TestA, useFactory: () => new TestA('component'), deps: []},
  {provide: TestB, useFactory: createTestB},
],omp {
constructor(public readonly testB: TestB) {}onfigureTestingModule({
declarations: [MyComp],
providers: [{provide: TestA, useFactory: () => new TestA('module'), deps: []}] = TestBed.createComponent(MyComp);
p.componentInstance.testB).toBeInstanceOf(TestB);
p.componentInstance.testB.a).toBeNull();
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();const dirA = fixture.componentInstance.dirA;
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();const dirC = fixture.componentInstance.dirC;
...
const NON_EXISTING_PROVIDER = new InjectionToken<string>('non-existing');@Component({template: ''})
class MyComp {
...
expect(injector.get(NON_EXISTING_PROVIDER, 'default', InjectFlags.Host)).toBe('default');const fixture = TestBed.createComponent(MyComp);
...
it('should support @SkipSelf when injecting Injectors', () => {
 ...
 fixture.detectChanges(); const childComponent =
...
fixture.detectChanges();expect(componentInjector!.get('token')).toBe('NG_MODULE');
...
const fixture = TestBed.createComponent(MyComponent);
fixture.detectChanges();expect((moduleElement! as any).from).toBe('NG_MODULE');
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();// Assert against the `nativeElement` since Ivy always returns a new ElementRef.
...
it('should work for `ElementRef` token', () => {
 ...
 const fixture = TestBed.createComponent(ParentComponent);
 fixture.detectChanges(); expect(requestedElementRef!.nativeElement).toEqual(fixture.nativeElement.firstChild);
 ...
 const fixture = TestBed.createComponent(ParentComponent);
 fixture.detectChanges(); expect(requestedRef!.element.nativeElement).toBe(fixture.nativeElement.firstChild);
 ...
 const fixture = TestBed.createComponent(ParentComponent);
 fixture.detectChanges(); const {context} = requestedChangeDetectorRef! as ViewRefInternal<ParentComponent>;
...
it('should work for Injectors', () => {
 ...
 const fixture = TestBed.createComponent(ParentComponent);
 fixture.detectChanges(); expect(childComponentInjector!.get('token'))
...
it('should throw when using @SkipSelf for TemplateRef', () => {
 ...
 fixture.detectChanges(); expect(directiveTemplateRef).toBeNull();
...
it('should support @SkipSelf when injecting ViewContainerRef', () => {
 ...
 let childViewContainer: ViewContainerRef; @Directive({selector: '[parent]'})
 class ParentDirective {
 ...
 {declarations: [ParentDirective, ChildDirective, MyComp]});
 const fixture = TestBed.createComponent(MyComp);
 fixture.detectChanges(); // Assert against the `element` since Ivy always returns a new ViewContainerRef.
 ...
 {declarations: [ParentDirective, ChildDirective, MyComp]}); const fixture = TestBed.createComponent(MyComp);
 fixture.detectChanges(); expect(childViewContainer!.element.nativeElement)
 .toBe(parentViewContainer!.element.nativeElement);
 expect(parentViewContainer!.element.nativeElement.tagName).toBe('DIV');
});it('should get ViewContainerRef using @SkipSelf and @Host on parent', () => {
 let parentViewContainer: ViewContainerRef; @Directive({selector: '[parent]'})
 class ParentDirective {
   constructor(@SkipSelf() vc: ViewContainerRef) {
 parentViewContainer = vc;
   }
 ...
 class MyComp {
 } TestBed.configureTestingModule({declarations: [ParentDirective, MyComp]}); const fixture = TestBed.createComponent(MyComp);
 fixture.detectChanges(); expect(parentViewContainer!.element.nativeElement.tagName).toBe('DIV');
...
class MyComp {
  constructor(@SkipSelf() vc: ViewContainerRef) {}
}TestBed.configureTestingModule({declarations: [MyComp]});expect(() => TestBed.createComponent(MyComp))
...
it('should support @SkipSelf when injecting ChangeDetectorRef', () => {
 ...
 let childRef: ChangeDetectorRef|undefined; @Directive({selector: '[parent]'})
 class ParentDirective {
 ...
 {declarations: [ParentDirective, ChildDirective, MyComp]});
 const fixture = TestBed.createComponent(MyComp);
 fixture.detectChanges(); // Assert against the `rootNodes` since Ivy always returns a new ChangeDetectorRef.
 ...
 let childRef: ChangeDetectorRef|undefined; @Component({selector: 'child', template: '...'})
 class ChildComp {
   constructor(@SkipSelf() cdr: ChangeDetectorRef) {
 childRef = cdr;
   }
...
const fixture = TestBed.createComponent(MyComponent);
fixture.detectChanges();expect((moduleCDR! as any).from).toBe('NG_MODULE');
...
it('should support @SkipSelf when using viewProviders', () => {
 ...
 const fixture = TestBed.createComponent(MyApp);
 fixture.detectChanges(); const child = fixture.componentInstance.child;
 ...
 class MyApp {
 } TestBed.configureTestingModule({declarations: [Child, Parent, MyApp]}); expect(() => TestBed.createComponent(MyApp)).toThrowError(/No provider for Bar/);
...
class MyApp {
}TestBed.configureTestingModule({declarations: [Child, Parent, MyApp]});expect(() => TestBed.createComponent(MyApp)).not.toThrowError(/No provider for Bar/);
  });
  });
});describe('@Host', () => {
  @Directive({selector: '[dirA]'})
...
const fixture = TestBed.createComponent(MyApp);
fixture.detectChanges();const dirString = fixture.componentInstance.myComp.dirString;
...
let injectedControlContainer: ControlContainer|null = null;@Directive({
  selector: '[group]',
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();const dirString = fixture.componentInstance.dirString;
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();const dirB = fixture.componentInstance.dirB;
...
const childInjector = Injector.create({providers: [], parent: testBedInjector});const anyService = childInjector.get(AnyService);
expect(anyService.injector).toBe(childInjector);const rootService = childInjector.get(RootService);
expect(rootService.injector.get(ɵINJECTOR_SCOPE)).toBe('root');const platformService = childInjector.get(PlatformService);
...
class SubSubClass extends SubClass {}@Component({template: ''})
class MyComp {
...
const fixture = TestBed.createComponent(MyComp);
expect(fixture.componentInstance.myService.dep instanceof Dependency).toBe(true);expect(warnSpy).toHaveBeenCalledWith(
...
fixture.detectChanges();const provider = TestBed.inject(MyRootService);expect(provider instanceof MyRootService).toBe(true);
expect(provider.id).toBe(2);expect(warnSpy).toHaveBeenCalledWith(
...
TestBed.configureTestingModule({declarations: [MyComp]});
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();expect(fixture.componentInstance.myService instanceof MyService).toBe(true);
...
let provider: FooService|BarService;@Component({template: ''})
class App {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(provider!.getMessage()).toBe('bar');
expect(provider!.dep.name).toBe('BarServiceDep');
 ...
 () => {
der: FooService|BarService;t({template: ''})
 {
constructor(service: FooService) {
  provider = service;
}onfigureTestingModule(
  {declarations: [App], providers: [{provide: FooService, useClass: FooService}]});
ture = TestBed.createComponent(App);
etectChanges();ovider!.getMessage()).toBe('foo');
...
let overriddenProvider: FooService|BarService;@Component({template: ''})
class App {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(directProvider!.getMessage()).toBe('bar');
...
constructor(service: FooService) {
  provider = service;
}onfigureTestingModule({declarations: [App], providers: [FooService]});
ture = TestBed.createComponent(App);
etectChanges();ovider!.getMessage()).toBe('foo');
ovider!.dep.name).toBe('FooServiceDep');
 });
  });  describe('inject', () => {
...
@ViewChild(OtherInjectorDir) otherInjectorDir!: OtherInjectorDir;
  ...
  const otherInjectorDir = fixture.componentInstance.otherInjectorDir;  expect(injectorDir.injector.get(ElementRef).nativeElement).toBe(divElement);
...
@ViewChild(InjectorDir) injectorDir!: InjectorDir;
  ...
  const fixture = TestBed.createComponent(MyComp);
  fixture.detectChanges();  const divElement = fixture.nativeElement.querySelector('div');
  const injectorDir = fixture.componentInstance.injectorDir;  expect(injectorDir.injector.get(ElementRef).nativeElement).toBe(divElement);
...
@ViewChild(MyOtherDir) otherDirective!: MyOtherDir;
  ...
  const otherDirective = fixture.componentInstance.otherDirective;  expect(directive.value).toContain('ElementRef');
  expect(directive.elementRef.nativeElement).toEqual(divElement);
  expect(otherDirective.elementRef.nativeElement).toEqual(divElement);  // Each ElementRef instance should be unique
  expect(otherDirective.isSameInstance).toBe(false);
});it('should create ElementRef with comment if requesting directive is on <ng-template> node',@Directive({selector: '[dir]'})
class MyDir {
  value: string;
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();const directive = fixture.componentInstance.directive;expect(directive.value).toContain('ElementRef');
// the nativeElement should be a comment
expect(directive.elementRef.nativeElement.nodeType).toEqual(Node.COMMENT_NODE);it('should be available if used in conjunction with other tokens', () => {
  @Injectable()
  class ServiceA {
subject: any;
...
template: '...',
  ...
  class RootComp {
public childCompRef!: ComponentRef<ChildComp>;constructor(public vcr: ViewContainerRef) {}create() {
...
providers: [ServiceA],
  ...
  fixture.autoDetectChanges();  fixture.componentInstance.create();  const {elementRef} = fixture.componentInstance.childCompRef.instance.directive;
...
@ViewChild(MyOtherDir) otherDirective!: MyOtherDir;
  ...
  const fixture = TestBed.createComponent(MyComp);
  fixture.detectChanges();  const directive = fixture.componentInstance.directive;
  const otherDirective = fixture.componentInstance.otherDirective;  expect(directive.value).toContain('TemplateRef');
  expect(directive.templateRef).not.toBeNull();
  expect(otherDirective.templateRef).not.toBeNull();  // Each TemplateRef instance should be unique
  expect(otherDirective.isSameInstance).toBe(false);
...
@ViewChild(MyOtherDir) otherDirective!: MyOtherDir;
  ...
  const fixture = TestBed.createComponent(MyComp);
  fixture.detectChanges();  const directive = fixture.componentInstance.directive;
  const otherDirective = fixture.componentInstance.otherDirective;  expect(directive.value).toContain('ViewContainerRef');
  expect(directive.viewContainerRef).not.toBeNull();
  expect(otherDirective.viewContainerRef).not.toBeNull();  // Each ViewContainerRef instance should be unique
  expect(otherDirective.isSameInstance).toBe(false);
...
template: `<ng-template #tmpl>Test</ng-template>`,
  ...
  class Root {
@ViewChild(TemplateRef, {static: true}) tmpl!: TemplateRef<any>;constructor(public vcr: ViewContainerRef, public vcr2: ViewContainerRef) {}ngOnInit(): void {
...
declarations: [Root],
  ...
  fixture.detectChanges();
  const cmp = fixture.componentInstance;  expect(cmp.vcr.length).toBe(1);
  expect(cmp.vcr2.length).toBe(1);
  expect(cmp.vcr2.get(0)).toEqual(cmp.vcr.get(0));  cmp.vcr2.remove(0);
...
class MyComp {
  ...
  let pipeInstance: MyPipe;  @Pipe({name: 'pipe'})
...
template: `<div *ngIf="showing | pipe">Visible</div>`,
  ...
  class MyApp {
showing = true;constructor(public cdr: ChangeDetectorRef) {}
...
});it('should inject current component ChangeDetectorRef into directives on the same node as components',@Component({selector: 'my-app', template: '<my-comp dir otherDir #dir="dir"></my-comp>'})
class MyApp {
...
expect(app.directive.value).toContain('ViewRef');// Each ChangeDetectorRef instance should be unique
expect(app.directive!.cdr).not.toBe(comp!.cdr);
expect(app.directive!.cdr).not.toBe(app.otherDirective!.cdr);it('should inject host component ChangeDetectorRef into directives on normal elements',@Component({selector: 'my-comp', template: '<div dir otherDir #dir="dir"></div>'})
class MyComp {
  constructor(public cdr: ChangeDetectorRef) {}
  @ViewChild(MyDir) directive!: MyDir;
  @ViewChild(MyOtherDir) otherDirective!: MyOtherDir;
...
expect((comp!.cdr as ViewRefInternal<MyComp>).context).toBe(comp);
// ChangeDetectorRef is the token, ViewRef has historically been the constructor
expect(comp.directive.value).toContain('ViewRef');// Each ChangeDetectorRef instance should be unique
expect(comp.directive!.cdr).not.toBe(comp.cdr);
expect(comp.directive!.cdr).not.toBe(comp.otherDirective!.cdr);it('should inject host component ChangeDetectorRef into directives in a component\'s ContentChildren',@Component({
  selector: 'my-app',
  template: `<my-comp>
...
expect(app.directive.value).toContain('ViewRef');// Each ChangeDetectorRef instance should be unique
expect(app.directive!.cdr).not.toBe(comp.cdr);
expect(app.directive!.cdr).not.toBe(app.otherDirective!.cdr);it('should inject host component ChangeDetectorRef into directives in embedded views', () => {
  @Component({
selector: 'my-comp',
...
@ViewChild(MyOtherDir) otherDirective!: MyOtherDir;
  ...
  let dirInstance: MyDirective;  @Directive({selector: '[getCDR]'})
  class MyDirective {
constructor(public cdr: ChangeDetectorRef) {
 dirInstance = this;
}
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();const injectorDir = fixture.componentInstance.injectorDirInstance;expect(injectorDir.value).toBe('provided');
    });
...
class WrapperComp {
  @ViewChild(MyComp) myComp!: MyComp;
}TestBed.configureTestingModule({declarations: [MyComp, WrapperComp]});const fixture = TestBed.createComponent(WrapperComp);
fixture.detectChanges();
expect(fixture.componentInstance.myComp.token).toBe('token with factory');
    });
  });  describe('inject()', () => {
    it('should work in a directive constructor', () => {
const TOKEN = new InjectionToken<string>('TOKEN');@Component({
  standalone: true,
  selector: 'test-cmp',
  template: '{{value}}',
...
const TOKEN = new InjectionToken<string>('TOKEN');@Injectable()
class Service {
  value: string;
  constructor() {
this.value = inject(TOKEN);
...
const TOKEN = new InjectionToken<string>('TOKEN');@Injectable({providedIn: 'root'})
class Service {
  value: string;
  constructor() {
...
envInjector = inject(EnvironmentInjector);
  }  const {nodeInjector, envInjector} = TestBed.createComponent(TestCmp).componentInstance;  expect(nodeInjector.get(TOKEN, undefined, {optional: true})).toBeNull();
  expect(nodeInjector.get(TOKEN, undefined, InjectFlags.Optional)).toBeNull();  expect(envInjector.get(TOKEN, undefined, {optional: true})).toBeNull();
...
nodeInjector = inject(Injector);
envInjector = inject(EnvironmentInjector);
  }  const {nodeInjector, envInjector} = TestBed.createComponent(TestCmp).componentInstance;  const flags: InjectOptions = {optional: true};  let nodeInjectorResult = nodeInjector.get(TOKEN, undefined, flags);
  expect(nodeInjectorResult).toBe(null);  // Verify that `null` can be a valid value (from typing standpoint),
  // the line below would fail a type check in case the result doesn't
  // have `null` in the type.
  nodeInjectorResult = null;  let envInjectorResult = envInjector.get(TOKEN, undefined, flags);
  expect(envInjectorResult).toBe(null);  // Verify that `null` can be a valid value (from typing standpoint),
  // the line below would fail a type check in case the result doesn't
  // have `null` in the type.
...
nodeInjector = inject(Injector);
  ...
  const child = createEnvironmentInjector([{provide: TOKEN, useValue: 'from child'}], root);  expect(child.get(TOKEN)).toEqual('from child');
...
nodeInjector = inject(Injector);
  ...
  const parent = TestBed.inject(EnvironmentInjector);
  const root = createEnvironmentInjector([{provide: TOKEN, useValue: 'from root'}], parent);
  const child = createEnvironmentInjector([], root);  expect(child.get(TOKEN, undefined, {self: true, optional: true})).toBeNull();
...
[{provide: TOKEN, useValue: 'from injector'}], TestBed.inject(EnvironmentInjector));const result = runInInjectionContext(injector, () => inject(TOKEN));
...
class TestCmp {
  envInjector = inject(EnvironmentInjector);  tokenFromComponent = inject(TOKEN);
  tokenFromEnvContext = runInInjectionContext(this.envInjector, () => inject(TOKEN));  // Attempt to inject ViewContainerRef within the environment injector's context. This should
...
class TestCmp {
  injector = inject(Injector);  vcrFromEnvContext =
...
const childInjector =
createEnvironmentInjector([{provide: TOKEN, useValue: 'from child'}], parentInjector);const results = runInInjectionContext(parentInjector, () => {
...
expect(() => runInInjectionContext(injector, () => {})).toThrow();
    ...
    function placeholder() {}    it('should throw if not in an injection context', () => {
...
template: `<root></root>`,
    ...
    class App {
    }    TestBed.configureTestingModule({declarations: [Root, Comp, App]});    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(fixture.nativeElement.textContent).toBe('A from Root (processed by useFactory)');
...
template: `<root></root>`,
    ...
    class App {
    }    TestBed.configureTestingModule({declarations: [Root, Comp, App, Intermediate]});    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    // Making sure that the `@Host` takes effect and token `A` becomes unavailable in DI since it's
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();const directive = fixture.componentInstance.directiveInstance;expect(directive.exist).toBe('existValue');
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();const directive = fixture.componentInstance.directiveInstance;expect(directive.otherAttr).toBe('value');
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();expect(fixture.componentInstance.directiveInstance.attrValue).toBeNull();
...
providers: [MyService],
    ...
    const fixture = TestBed.createComponent(MyComp);
    fixture.detectChanges();    expect(fixture.nativeElement.innerHTML).toContain('Other value is: 2 (transformed)');
...
`
    ...
    class MyComp {
count = '2';@ViewChild('target', {read: ViewContainerRef}) target!: ViewContainerRef;
@ViewChild('source', {read: TemplateRef}) source!: TemplateRef<any>;create() {
...
providers: [MyService],
    ...
    const fixture = TestBed.createComponent(MyComp);
    fixture.detectChanges();    fixture.componentInstance.create();
    fixture.detectChanges();    expect(fixture.nativeElement.textContent.trim()).toBe('2 (transformed) items');
  ...
  it('should prioritize useFactory over useExisting', () => {
    ...
    }    const PROVIDER = {provide: Base, useExisting: DirA, useFactory: () => new DirB()};    @Component({selector: 'child', template: '', providers: [PROVIDER]})
    class Child {
...
mponent(App);
 ...
 'should not get dirA from parent, but create new dirB from the useFactory provider');
    ...
    const token = new InjectionToken<number>('token');    it('pipes should access providers from the component they are on', () => {
@Pipe({name: 'token'})
class TokenPipe {
  constructor(@Inject(token) private _token: string) {}  transform(value: string): string {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(fixture.nativeElement.textContent.trim()).toBe('child');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(fixture.nativeElement.textContent.trim()).toBe('parent');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(fixture.componentInstance.dir.token).toBe('child');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(fixture.componentInstance.dir.token).toBe('parent');
    });
...
constructor(_viewRef: ViewRef) {}
    ...
    const token = new InjectionToken<string>('greeting');    @Directive({selector: 'menu-trigger'})
    class MenuTrigger {
@Input('triggerFor') menu!: TemplateRef<unknown>;constructor(private viewContainerRef: ViewContainerRef) {}open(injector: Injector|undefined) {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.componentInstance.trigger.open(injector);
fixture.detectChanges();expect(fixture.componentInstance.menu.tokenValue).toBe('hello');
    });    it('should check only the current node with @Self when providing an injection token through an embedded view injector',
 () => {
e({selector: 'menu'})
u {
constructor(@Inject(token) @Self() @Optional() public tokenValue: string) {}t({
template: `
<menu-trigger [triggerFor]="menuTemplate"></menu-trigger>
<ng-template #menuTemplate>
 <menu></menu>
...
@ViewChild(MenuTrigger) trigger!: MenuTrigger;
@ViewChild(Menu) menu!: Menu;onfigureTestingModule({declarations: [App, MenuTrigger, Menu]});
ector = Injector.create({providers: [{provide: token, useValue: 'hello'}]});
ture = TestBed.createComponent(App);
etectChanges();omponentInstance.trigger.open(injector);
etectChanges();xture.componentInstance.menu.tokenValue).toBeNull();
 });    it('should be able to provide an injection token to a nested template through a custom injector',
 () => {
e({selector: 'menu'})
u {
constructor(@Inject(token) public tokenValue: string) {}t({
template: `
 <menu-trigger #outerTrigger [triggerFor]="outerTemplate"></menu-trigger>
 <ng-template #outerTemplate>
   <menu></menu>   <menu-trigger #innerTrigger [triggerFor]="innerTemplate"></menu-trigger>
...
@ViewChild('innerMenu', {read: Menu}) innerMenu!: Menu;onfigureTestingModule({declarations: [App, MenuTrigger, Menu]});
ture = TestBed.createComponent(App);
etectChanges();omponentInstance.outerTrigger.open(
  Injector.create({providers: [{provide: token, useValue: 'hello'}]}));
etectChanges();omponentInstance.innerTrigger.open(undefined);
etectChanges();xture.componentInstance.innerMenu.tokenValue).toBe('hello');
...
template: `
 ...
 <ng-template #grandparentTemplate>
   <menu></menu>   <menu-trigger #parentTrigger [triggerFor]="parentTemplate"></menu-trigger>
   <ng-template #parentTemplate>
 <menu></menu> <menu-trigger #childTrigger [triggerFor]="childTemplate"></menu-trigger>
...
@ViewChild('childMenu', {read: Menu}) childMenu!: Menu;onfigureTestingModule({declarations: [App, MenuTrigger, Menu]});
ture = TestBed.createComponent(App);
etectChanges();omponentInstance.grandparentTrigger.open(
  Injector.create({providers: [{provide: token, useValue: 'hello'}]}));
etectChanges();omponentInstance.parentTrigger.open(Injector.create({providers: []}));
etectChanges();omponentInstance.childTrigger.open(undefined);
etectChanges();xture.componentInstance.childMenu.tokenValue).toBe('hello');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.componentInstance.trigger.open(
Injector.create({providers: [{provide: token, useValue: 'hello from injector'}]}));
fixture.detectChanges();fixture.componentInstance.wrapper.trigger.open(undefined);
fixture.detectChanges();expect(fixture.componentInstance.wrapper.menu.tokenValue).toBe('hello from wrapper');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.componentInstance.trigger.open(injector);
fixture.detectChanges();expect(fixture.componentInstance.menu.tokenValue).toBe('hello from injector');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.componentInstance.trigger.open(injector);
fixture.detectChanges();expect(fixture.componentInstance.menu.elementRef.nativeElement)
...
declarations: [App, MenuTrigger, Menu],
exports: [App, MenuTrigger, Menu],
providers: [{provide: token, useValue: 'hello from module'}]ule {onfigureTestingModule({imports: [Module]});
ector = Injector.create({providers: []});
ture = TestBed.createComponent(App);
etectChanges();omponentInstance.trigger.open(injector);
etectChanges();xture.componentInstance.menu.tokenValue).toBe('hello from parent');
 });    it('should be able to inject an injectable with dependencies', () => {
@Injectable()
class Greeter {
  constructor(@Inject(token) private tokenValue: string) {}  greet() {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.componentInstance.trigger.open(injector);
fixture.detectChanges();expect(fixture.componentInstance.menu.greeter.greet()).toBe('hello from injector');
...
`ent {
@ViewChild(MenuTrigger) trigger!: MenuTrigger;
@ViewChild(Menu) menu!: Menu;t({
template: '<parent></parent>',
providers: [{provide: token, useValue: 'hello from grandparent'}]ndParent {
@ViewChild(Parent) parent!: Parent;onfigureTestingModule({declarations: [GrandParent, Parent, MenuTrigger, Menu]});
ector = Injector.create({providers: []});
ture = TestBed.createComponent(GrandParent);
etectChanges();omponentInstance.parent.trigger.open(injector);
etectChanges();xture.componentInstance.parent.menu.tokenValue).toBe('hello from grandparent');
 });    it('should be able to use a custom injector when created through TemplateRef', () => {
let injectedValue: string|undefined;@Directive({selector: 'menu'})
class Menu {
...
Injector.create({providers: [{provide: token, useValue: 'hello from injector'}]});
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.componentInstance.template.createEmbeddedView({}, injector);
fixture.detectChanges();expect(injectedValue).toBe('hello from injector');
    });    it('should use a custom injector when the view is created outside of the declaration view',
 () => {
larerToken = new InjectionToken<string>('declarerToken');
atorToken = new InjectionToken<string>('creatorToken');e({selector: 'menu'})
u {
constructor(
@Inject(token) public tokenValue: string,
@Optional() @Inject(declarerToken) public declarerTokenValue: string,
@Optional() @Inject(creatorToken) public creatorTokenValue: string) {}t({
selector: 'declarer',
template: '<ng-template><menu></menu></ng-template>',
providers: [{provide: declarerToken, useValue: 'hello from declarer'}]larer {
@ViewChild(Menu) menu!: Menu;
@ViewChild(TemplateRef) template!: TemplateRef<unknown>;t({
selector: 'creator',
template: '<menu-trigger></menu-trigger>',
providers: [{provide: creatorToken, useValue: 'hello from creator'}]ator {
@ViewChild(MenuTrigger) trigger!: MenuTrigger;t({
template: `
   <declarer></declarer>
   <creator></creator>
 ` {
@ViewChild(Declarer) declarer!: Declarer;
@ViewChild(Creator) creator!: Creator;onfigureTestingModule(
  {declarations: [App, MenuTrigger, Menu, Declarer, Creator]});
ector = Injector.create({providers: [{provide: token, useValue: 'hello'}]});
ture = TestBed.createComponent(App);
etectChanges();
clarer, creator} = fixture.componentInstance;rigger.menu = declarer.template;
rigger.open(injector);
etectChanges();clarer.menu.tokenValue).toBe('hello');
clarer.menu.declarerTokenValue).toBe('hello from declarer');
clarer.menu.creatorTokenValue).toBeNull();
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.componentInstance.trigger.open(injector);
fixture.detectChanges();expect(fixture.componentInstance.menu.tokenValue).toBe('hello from directive');
    });    it('should give precedence to value provided in custom injector over one provided higher',
 () => {
e({selector: 'menu'})
u {
constructor(@Inject(token) public tokenValue: string) {}e({
selector: '[provide-token]',
providers: [{provide: token, useValue: 'hello from directive'}]videToken {t({
template: `
   <menu-trigger [triggerFor]="menuTemplate"></menu-trigger>
   <div provide-token>
 <ng-template #menuTemplate>
...
@ViewChild(Menu) menu!: Menu;({
declarations: [App, MenuTrigger, Menu, ProvideToken],
exports: [App, MenuTrigger, Menu, ProvideToken],ule {onfigureTestingModule({imports: [Module]});
ector =
  Injector.create({providers: [{provide: token, useValue: 'hello from injector'}]});
ture = TestBed.createComponent(App);
etectChanges();omponentInstance.trigger.open(injector);
etectChanges();xture.componentInstance.menu.tokenValue).toBe('hello from injector');
...
constructor(@Inject(token) public tokenValue: string) {}e({
selector: '[provide-token]',
providers: [{provide: token, useValue: 'hello from directive'}]videToken {t({selector: 'wrapper', template: `<div><menu></menu></div>`})
pper {
@ViewChild(Menu) menu!: Menu;t({
template: `
   <menu-trigger [triggerFor]="menuTemplate"></menu-trigger>
...
exports: [App, MenuTrigger, Menu, ProvideToken, Wrapper],ule {onfigureTestingModule({imports: [Module]});
ector =
  Injector.create({providers: [{provide: token, useValue: 'hello from injector'}]});
ture = TestBed.createComponent(App);
etectChanges();omponentInstance.trigger.open(injector);
etectChanges();xture.componentInstance.wrapper.menu.tokenValue).toBe('hello from directive');
...
ture = TestBed.createComponent(App);
etectChanges();omponentInstance.trigger.open(injector);
etectChanges();xture.componentInstance.wrapper.menu.tokenValue).toBe('hello from injector');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.componentInstance.trigger.open(injector);
fixture.detectChanges();expect(fixture.componentInstance.menu.tokenValue).toBe('hello from parent');
    });
  });  it('should prioritize module providers over additional providers', () => {
    const token = new InjectionToken('token');    @NgModule({providers: [{provide: token, useValue: 'module'}]})
    class ModuleWithProvider {
    }    const injector =
  createInjector(ModuleWithProvider, null, [{provide: token, useValue: 'additional'}]);    expect(injector.get(token)).toBe('module');
  });
});
