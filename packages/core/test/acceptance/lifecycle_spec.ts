...
import {By} from '@angular/platform-browser';describe('onChanges', () => {
  it('should correctly support updating one Input among many', () => {
    let log: string[] = [];    @Component({selector: 'child-comp', template: 'child'})
    class ChildComp implements OnChanges {
@Input() a: number = 0;
@Input() b: number = 0;
@Input() c: number = 0;ngOnChanges(changes: SimpleChanges) {
...
c = 0;
    ...
    log.length = 0;    appComp.a = 1;
    fixture.detectChanges();
    expect(log).toEqual(['a: 0 -> 1']);
    log.length = 0;    appComp.b = 2;
    fixture.detectChanges();
    expect(log).toEqual(['b: 0 -> 2']);
    log.length = 0;    appComp.c = 3;
    fixture.detectChanges();
    expect(log).toEqual(['c: 0 -> 3']);
  });  it('should call onChanges method after inputs are set in creation and update mode', () => {
    const events: any[] = [];    @Component({
selector: 'comp',
template: `<p>test</p>`,
    })
    class Comp {
@Input() val1 = 'a';@Input('publicVal2') val2 = 'b';ngOnChanges(changes: SimpleChanges) {
...
declarations: [App, Comp],
    ...
    fixture.detectChanges();    expect(events).toEqual([{
name: 'comp',
changes: {
...
template: `<child [val]="val"></child>`,
    ...
    class Parent {
@Input() val = '';ngOnChanges(changes: SimpleChanges) {
...
template: `<child [name]="name" [val]="val"></child>`,
    ...
    class Parent {
@Input() val = '';@Input() name = '';ngOnChanges(changes: SimpleChanges) {
...
changes: {
  val: new SimpleChange(undefined, 'a', true),
    ...
    fixture.detectChanges();    expect(events).toEqual([]);    fixture.componentInstance.val = 'b';
...
val: new SimpleChange('a', 'b', false),
    ...
    const events: any[] = [];    @Directive({
selector: '[dir]',
    })
    class Dir {
@Input() dir = '';ngOnChanges(changes: SimpleChanges) {
...
template: `<p>{{val}}</p>`,
    ...
    class Comp {
@Input() val = '';constructor(public dir: Dir) {}ngOnChanges(changes: SimpleChanges) {
  events.push({name: 'comp', changes});
}
    }    @Component({
template: `<comp [dir]="val" [val]="val"></comp>`,
...
selector: '[injectionDir]',
    ...
    class InjectionDir {
@Input() injectionDir = '';constructor(public dir: Dir) {}ngOnChanges(changes: SimpleChanges) {
...
selector: '[dir]',
    ...
    class Dir {
@Input() dir = '';@Input('dir-val') val = '';ngOnChanges(changes: SimpleChanges) {
  events.push({name: 'dir', changes});
}
...
val: new SimpleChange('a', 'b', false),
    ...
    const events: any[] = [];    @Component({template: `<p>{{value}}</p>`})
    class App {
value = 'a';
...
declarations: [App],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(events).toEqual([]);    fixture.componentInstance.value = 'b';
    fixture.detectChanges();    expect(events).toEqual([]);
  });
});describe('meta-programming', () => {
  it('should allow adding lifecycle hook methods any time before first instance creation', () => {
    const events: any[] = [];    @Component({template: `<child name="value"></child>`})
    class App {
...
events.push('ngOnChanges');
    ...
    ChildPrototype.ngOnDestroy = () => events.push('ngOnDestroy');    TestBed.configureTestingModule({
...
'ngAfterViewInit', 'ngAfterViewChecked', 'ngOnDestroy'
 ...
 const events: any[] = []; @Component({template: `<child name="value"></child>`})
 class App {
 } class BaseChild {} @Component({selector: 'child', template: `empty`})
...
e = e['name'];
me.previousValue).toEqual(undefined);
me.currentValue).toEqual('value');
me.firstChange).toEqual(true);
sh('ngOnChanges');
 }; // These will be overwritten later
 BasePrototype.ngDoCheck = () => events.push('Expected to be overbidden');
 BasePrototype.ngAfterContentInit = () => events.push('Expected to be overbidden'); // These are define on the concrete class
 const ChildPrototype = Child.prototype as any;
 ChildPrototype.ngDoCheck = () => events.push('ngDoCheck');
 ChildPrototype.ngAfterContentInit = () => events.push('ngAfterContentInit');
 ChildPrototype.ngAfterContentChecked = () => events.push('ngAfterContentChecked');
...
ons: [App, Child],
 ...
 fixture.destroy();
 expect(events).toEqual([
ges', 'onInit', 'ngDoCheck', 'ngAfterContentInit', 'ngAfterContentChecked',
iewInit', 'ngAfterViewChecked', 'ngOnDestroy'
 ]);
     });
});it('should call all hooks in correct order when several directives on same node', () => {
  let log: string[] = [];  class AllHooks {
    id: number = -1;    /** @internal */
...
this._log('afterViewChecked', this.id);
  ...
  let log: string[] = [];  @Directive({selector: 'div'})
  class DirA {
    @Input() a: number = 0;
    ngOnInit() {
...
log.push('doCheckC' + this.c);
  ...
  it('should call onInit after inputs are the first time', () => {
    ...
    const input2Values: string[] = [];    @Component({
selector: 'my-comp',
template: `<p>test</p>`,
    })
    class MyComponent {
@Input() input1 = '';@Input() input2 = '';ngOnInit() {
...
declarations: [App, MyComponent],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(input1Values).toEqual(['a']);
    expect(input2Values).toEqual(['b']);    fixture.componentInstance.value1 = 'c';
    fixture.componentInstance.value2 = 'd';
    fixture.detectChanges();    // Shouldn't be called again just because change detection ran.
    expect(input1Values).toEqual(['a']);
    expect(input2Values).toEqual(['b']);
  });  it('should be called on root component', () => {
    let onInitCalled = 0;    @Component({template: ``})
    class App {
ngOnInit() {
  onInitCalled++;
}
...
declarations: [App],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(onInitCalled).toBe(1);
  });  it('should call parent onInit before it calls child onInit', () => {
    const initCalls: string[] = [];    @Component({
selector: `child-comp`,
template: `<p>child</p>`,
    })
    class ChildComp {
...
declarations: [ParentComp, ChildComp],
    ...
    fixture.detectChanges();    expect(initCalls).toEqual(['parent', 'child']);
...
declarations: [App, ParentComp, ChildComp],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(initCalls).toEqual(['parent 1', 'parent 2', 'child of parent 1', 'child of parent 2']);
  });  it('should call onInit every time a new view is created (if block)', () => {
    let onInitCalls = 0;    @Component({selector: 'my-comp', template: '<p>test</p>'})
    class MyComp {
ngOnInit() {
  onInitCalls++;
}
...
declarations: [App, MyComp],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(onInitCalls).toBe(1);    fixture.componentInstance.show = false;
    fixture.detectChanges();    expect(onInitCalls).toBe(1);    fixture.componentInstance.show = true;
    fixture.detectChanges();    expect(onInitCalls).toBe(2);
  });  it('should call onInit for children of dynamically created components', () => {
    @Component({selector: 'my-comp', template: '<p>test</p>'})
    class MyComp {
onInitCalled = false;ngOnInit() {
...
`,
    ...
    class App {
@ViewChild('container', {read: ViewContainerRef}) viewContainerRef!: ViewContainerRef;createDynamicView() {
  this.viewContainerRef.createComponent(DynamicComp);
}
    }    TestBed.configureTestingModule({declarations: [App, MyComp, DynamicComp]});    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    fixture.componentInstance.createDynamicView();
    fixture.detectChanges();    const myComp = fixture.debugElement.query(By.directive(MyComp)).componentInstance;
    expect(myComp.onInitCalled).toBe(true);
  });  it('should call onInit in hosts before their content children', () => {
    const initialized: string[] = [];    @Component({
selector: 'projected',
template: '',
    })
    class Projected {
...
declarations: [App, Comp, Projected],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(initialized).toEqual(['app', 'comp', 'projected']);
...
declarations: [App, Comp, Projected],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(initialized).toEqual(['app', 'comp 1', 'projected 1', 'comp 2', 'projected 2']);
  });  it('should be called on directives after component by default', () => {
    const initialized: string[] = [];    @Directive({
selector: '[dir]',
    })
    class Dir {
@Input('dir-name') name = '';ngOnInit() {
...
declarations: [App, Comp, Dir],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(initialized).toEqual(['app', 'comp 1', 'dir 1', 'comp 2', 'dir 2']);
  });  it('should be called on multiple directives in injection order', () => {
    const events: any[] = [];    @Directive({
selector: '[dir]',
    })
...
selector: '[injectionDir]',
    ...
    class InjectionDir {
@Input() injectionDir = '';constructor(public dir: Dir) {}ngOnInit() {
...
declarations: [App, InjectionDir, Dir],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(events).toEqual(['app', 'dir', 'injectionDir']);
  });  it('should be called on directives before component if component injects directives', () => {
    const initialized: string[] = [];    @Directive({
selector: '[dir]',
    })
...
template: `<p></p>`,
    ...
    class Comp {
@Input() name = '';constructor(public dir: Dir) {}ngOnInit() {
  initialized.push('comp ' + this.name);
}
    }    @Component({
template: `
...
declarations: [App, Comp, Dir],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(initialized).toEqual(['app', 'dir 1', 'comp 1', 'dir 2', 'comp 2']);
  });  it('should be called on directives on an element', () => {
    const initialized: string[] = [];    @Directive({
selector: '[dir]',
    })
...
declarations: [App, Dir],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(initialized).toEqual(['app', 'dir 1', 'dir 2']);
...
imports: [CommonModule],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(initialized).toEqual([
...
'parent 0',
'parent 1',// Then our 5 embedded views
'parent 2',
'child of parent 2',
'parent 3',
'child of parent 3',
...
'child of parent 5',
'parent 6',
'child of parent 6',// Then the children of the root level components
'child of parent 0',
'child of parent 1',
    ]);
  });
  ...
  it('should call doCheck on every refresh', () => {
    let doCheckCalled = 0;    @Component({template: ``})
    class App {
ngDoCheck() {
  doCheckCalled++;
}
...
declarations: [App],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(doCheckCalled).toBe(1);    fixture.detectChanges();    expect(doCheckCalled).toBe(2);
  });  it('should call parent doCheck before child doCheck', () => {
    const doChecks: string[] = [];    @Component({
selector: 'parent',
...
declarations: [App, Parent, Child],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(doChecks).toEqual(['app', 'parent', 'child']);
...
declarations: [App],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(events).toEqual(['onInit', 'doCheck']);
  });  it('should be called on directives after component by default', () => {
...
declarations: [App, Comp, Dir],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(doChecks).toEqual(['app', 'comp 1', 'dir 1', 'comp 2', 'dir 2']);
  });  it('should be called on directives before component if component injects directives', () => {
    const doChecks: string[] = [];
    @Directive({
selector: '[dir]',
...
template: `<p>test</p>`,
    ...
    class Comp {
@Input() name = '';constructor(public dir: Dir) {}ngDoCheck() {
  doChecks.push('comp ' + this.name);
}
    }    @Component({
template: `
...
declarations: [App, Comp, Dir],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(doChecks).toEqual(['app', 'dir 1', 'comp 1', 'dir 2', 'comp 2']);
  });  it('should be called on multiple directives in injection order', () => {
    const events: any[] = [];    @Directive({
selector: '[dir]',
    })
...
selector: '[injectionDir]',
    ...
    class InjectionDir {
@Input() injectionDir = '';constructor(public dir: Dir) {}ngDoCheck() {
  events.push('injectionDir');
}
    }    @Component({
template: `<div [injectionDir]="val" [dir]="val"></div>`,
...
declarations: [App, InjectionDir, Dir],
    ...
    const doChecks: string[] = [];    @Directive({
selector: '[dir]',
    })
    class Dir {
@Input('dir') name = '';ngDoCheck() {
...
declarations: [App, Dir],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(doChecks).toEqual(['app', 'dir 1', 'dir 2']);
  });
});describe('afterContentinit', () => {
  it('should be called only in creation mode', () => {
    let afterContentInitCalls = 0;    @Component({
selector: 'comp',
template: `<p>test</p>`,
    })
    class Comp {
...
declarations: [App, Comp],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    // two updates
    fixture.detectChanges();
    fixture.detectChanges();    expect(afterContentInitCalls).toBe(1);
  });  it('should be called on root component in creation mode', () => {
    let afterContentInitCalls = 0;    @Component({template: `<p>test</p>`})
    class App {
ngAfterContentInit() {
  afterContentInitCalls++;
}
...
imports: [CommonModule],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(events).toEqual(['app afterContentInit', 'comp afterContentInit']);    fixture.componentInstance.show = false;
    fixture.detectChanges();    expect(events).toEqual(['app afterContentInit', 'comp afterContentInit']);    fixture.componentInstance.show = true;
    fixture.detectChanges();    expect(events).toEqual(
...
`
    ...
    class App {
numbers = [0, 1, 2, 3];ngAfterContentInit() {
  events.push('app');
}
    }    TestBed.configureTestingModule({
declarations: [App, Comp],
imports: [CommonModule],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(events).toEqual(['app', 'comp 0', 'comp 1', 'comp 2', 'comp 3', 'comp 4', 'comp 5']);
...
selector: '[dir]',
    ...
    class Dir {
@Input('dir') name = '';ngAfterContentInit() {
...
'comp afterContentChecked',
  ...
  it('should be called on creation and not in update mode', () => {
    let afterViewInitCalls = 0;    @Component({
selector: 'comp',
template: `<p>test</p>`,
    })
    class Comp {
...
declarations: [App, Comp],
    ...
    fixture.detectChanges();    // two updates
    fixture.detectChanges();
    fixture.detectChanges();    expect(afterViewInitCalls).toBe(1);
  });  it('should be called on root component in creation mode', () => {
    let afterViewInitCalls = 0;    @Component({template: `<p>test</p>`})
    class App {
ngAfterViewInit() {
  afterViewInitCalls++;
}
...
declarations: [App, Comp],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(events).toEqual(['comp', 'app']);    fixture.componentInstance.show = false;
    fixture.detectChanges();    expect(events).toEqual(['comp', 'app']);    fixture.componentInstance.show = true;
    fixture.detectChanges();    expect(events).toEqual(['comp', 'app', 'comp']);
...
`
    ...
    class App {
numbers = [0, 1, 2, 3];ngAfterViewInit() {
  events.push('app');
}
    }    TestBed.configureTestingModule({
declarations: [App, Comp],
...
'app',
    ...
    const events: string[] = [];    @Directive({
selector: '[dir]',
    })
    class Dir {
@Input('dir') name = '';ngAfterViewInit() {
  events.push('dir ' + this.name);
}
    }    @Component({
selector: 'comp',
...
'app',
  ...
  it('should call ngAfterViewChecked every update', () => {
    let afterViewCheckedCalls = 0;    @Component({
selector: 'comp',
template: `<p>test</p>`,
    })
    class Comp {
...
declarations: [App, Comp],
    ...
    expect(afterViewCheckedCalls).toBe(1);    fixture.detectChanges();
    expect(afterViewCheckedCalls).toBe(2);    fixture.detectChanges();
    expect(afterViewCheckedCalls).toBe(3);
  });  it('should be called on root component', () => {
    let afterViewCheckedCalls = 0;    @Component({template: `<p>test</p>`})
    class App {
ngAfterViewChecked() {
  afterViewCheckedCalls++;
}
...
declarations: [App, Comp],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();
    expect(afterViewCheckedCalls).toBe(1);    fixture.componentInstance.value = 1337;
    fixture.detectChanges();
...
`
    ...
    class App {
numbers = [0, 1, 2, 3];ngAfterViewChecked() {
  events.push('app');
}
    }    TestBed.configureTestingModule({
declarations: [App, Parent, Child],
...
selector: '[dir]',
    ...
    class Dir {
@Input('dir') name = '';ngAfterViewChecked() {
  events.push('dir ' + this.name);
}
    }    @Component({
selector: 'comp',
...
imports: [CommonModule],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(destroyCalled).toBe(0);    fixture.componentInstance.show = false;
    fixture.detectChanges();    expect(destroyCalled).toBe(1);    fixture.componentInstance.show = true;
    fixture.detectChanges();    expect(destroyCalled).toBe(1);    fixture.componentInstance.show = false;
    fixture.detectChanges();    expect(destroyCalled).toBe(2);
...
imports: [CommonModule],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(events).toEqual([]);    fixture.componentInstance.show = false;
    fixture.detectChanges();    expect(events).toEqual(['comp 1', 'comp 2']);
...
imports: [CommonModule],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(events).toEqual([]);    fixture.componentInstance.showMiddle = false;
    fixture.detectChanges();
    expect(events).toEqual(['comp 2']);    fixture.componentInstance.showAll = false;
    fixture.detectChanges();
    expect(events).toEqual([
'comp 2',
'comp 1',
...
`
    ...
    class App {
show = true;clicksToButton1 = 0;clicksToButton2 = 0;handleClick1() {
...
declarations: [App, Comp],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    const buttons = fixture.debugElement.queryAll(By.css('button'));
    buttons.forEach(button => button.nativeElement.click());    expect(fixture.componentInstance.clicksToButton1).toBe(1);
    expect(fixture.componentInstance.clicksToButton2).toBe(1);
    expect(events).toEqual([]);    fixture.componentInstance.show = false;
    fixture.detectChanges();    buttons.forEach(button => button.nativeElement.click());
    expect(fixture.componentInstance.clicksToButton1).toBe(1);
    expect(fixture.componentInstance.clicksToButton2).toBe(1);    expect(events).toEqual(['comp']);
...
`
    ...
    class App {
@ViewChild('container', {read: ViewContainerRef, static: true}) container!: ViewContainerRef;@ViewChild('tpl', {read: TemplateRef, static: true}) tpl!: TemplateRef<any>;ngOnInit() {
  this.container.createEmbeddedView(this.tpl);
}
    }    @Directive({selector: '[dir]'})
    class Dir {
constructor(public cdr: ChangeDetectorRef) {}ngOnDestroy() {
...
declarations: [App, Parent, Child, Dir],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.autoDetectChanges();    expect(() => fixture.destroy()).not.toThrow();
  });  it('should be called on directives after component', () => {
    const events: string[] = [];    @Directive({
selector: '[dir]',
    })
    class Dir {
@Input('dir') name = '';ngOnDestroy() {
  events.push('dir ' + this.name);
}
    }    @Component({
selector: 'comp',
...
imports: [CommonModule],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(events).toEqual([]);    fixture.componentInstance.show = false;
    fixture.detectChanges();    expect(events).toEqual(['dir']);
  });
});describe('hook order', () => {
  let events: string[] = [];  beforeEach(() => events = []);  @Component({
    selector: 'comp',
    template: `{{value}}<div><ng-content></ng-content></div>`,
  })
  class Comp {
    @Input() value = '';    @Input() name = '';    ngOnInit() {
...
declarations: [App, OnDestroyDir],
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(destroyed).toBeFalsy();    fixture.componentInstance.show = false;
    fixture.detectChanges();    expect(destroyed).toBeTruthy();
     ...
     () => {
 const hooks: string[] = []; @Directive({selector: '[testDir]'})
 class TestDirective implements OnChanges, AfterViewInit {
or(private _changeDetectorRef: ChangeDetectorRef) {}estDir')
(_value: any) {
this._changeDetectorRef.detectChanges();es() {
hooks.push('ngOnChanges');ewInit() {
hooks.push('ngAfterViewInit'); } @Component({template: `<div [testDir]="value">{{value}}</div>`})
 class App {
;
...
constructor(private _changeDetectorRef: ChangeDetectorRef) {}@Input('testDir')
