...
import {expect} from '@angular/platform-browser/testing/src/matchers';describe('renderer factory lifecycle', () => {
  let logs: string[] = [];
  let lastCapturedType: RendererType2|null = null;  @Component({selector: 'some-component', template: `foo`})
...
logs.push('test_component update');
  ...
  function createPatchedRendererFactory(document: any) {
    ...
    const createRender = rendererFactory.createRenderer;    rendererFactory.createRenderer = (hostElement: any, type: RendererType2|null) => {
logs.push('create');
lastCapturedType = type;
return createRender.apply(rendererFactory, [hostElement, type]);
    };    rendererFactory.begin = () => logs.push('begin');
    rendererFactory.end = () => logs.push('end');    return rendererFactory;
  }  beforeEach(() => {
    logs = [];    TestBed.configureTestingModule({
...
encapsulation: ViewEncapsulation.ShadowDom,
    ...
    class StyledComp {
    }    TestBed.createComponent(StyledComp);    expect(lastCapturedType!.styles).toEqual(['.some-css-class { color: red; }']);
    expect(lastCapturedType!.encapsulation).toEqual(ViewEncapsulation.ShadowDom);
  });  describe('component animations', () => {
    it('should pass in the component styles directly into the underlying renderer', () => {
const animA = {name: 'a'};
const animB = {name: 'b'};@Component({
  standalone: true,
  template: '',
  animations: [
animA,
...
class AnimComp {
}TestBed.createComponent(AnimComp);const capturedAnimations = lastCapturedType!.data!['animation'];
...
class AnimComp {
}TestBed.createComponent(AnimComp);const data = lastCapturedType!.data;
...
class AnimComp {
}const rendererFactory = new MockRendererFactory(['setProperty']);TestBed.configureTestingModule({
  providers: [{
provide: RendererFactory2,
...
fixture.detectChanges();const renderer = rendererFactory.lastRenderer!;
const spy = renderer.spies['setProperty'];
const [_, prop, __] = spy.calls.mostRecent().args;expect(prop).toEqual('@fooAnimation');
    });
...
providers: [{
  ...
  deps: [DOCUMENT],
    ...
    comp!.visible = false;
    fixture.detectChanges();    comp!.visible = true;
    fixture.detectChanges();    const renderer = rendererFactory.lastRenderer!;
    const destroySpy = renderer.spies['destroy'];
    const createElementSpy = renderer.spies['createElement'];    // we should never see `destroy` method being called
    // in case child views are created/removed.
    expect(destroySpy.calls.count()).toBe(0);    // Make sure other methods on the renderer were invoked.
    expect(createElementSpy.calls.count() > 0).toBe(true);
  });
});describe('animation renderer factory', () => {
  let eventLogs: string[] = [];
  let rendererFactory: RendererFactory2|null = null;  function getAnimationLog(): MockAnimationPlayer[] {
    return MockAnimationDriver.log as MockAnimationPlayer[];
  }  beforeEach(() => {
    eventLogs = [];
    rendererFactory = null;
    MockAnimationDriver.log = [];    TestBed.configureTestingModule({
...
options: {}
  ...
  class SomeComponentWithAnimation {
    exp: string|undefined;    callback(event: AnimationEvent) {
eventLogs.push(`${event.fromState ? event.fromState : event.toState} - ${event.phaseName}`);
  ...
  class SomeComponent {
    ...
    fixture.detectChanges();    expect(rendererFactory).toBeTruthy();
    expect(fixture.nativeElement.innerHTML)
  .toMatch(/<div class="ng-tns-c\d+-0 ng-trigger ng-trigger-myAnimation">\s+foo\s+<\/div>/);    fixture.componentInstance.exp = 'on';
    fixture.detectChanges();    const [player] = getAnimationLog();
    expect(player.keyframes).toEqual([
new Map<string, string|number>([['opacity', '*'], ['offset', 0]]),
new Map<string, string|number>([['opacity', 1], ['offset', 1]]),
    ]);
    player.finish();    rendererFactory!.whenRenderingDone!().then(() => {
...
<basic-comp *ngIf="isContentVisible">C</basic-comp>
  ...
  class AppWithComponents {
    ...
    ngDevModeResetPerfCounters();    TestBed.configureTestingModule({
...
providers: [{
  ...
  deps: [DOCUMENT]
    ...
    fixture.detectChanges();    expect(fixture.nativeElement.textContent).toBe('ABC');    fixture.componentInstance.isContentVisible = false;
    fixture.detectChanges();    expect(fixture.nativeElement.textContent).toBe('');
    ...
    fixture.detectChanges();    expect(fixture.nativeElement.textContent).toBe('comp(A)comp(B)comp(C)');    fixture.componentInstance.isContentVisible = false;
    fixture.detectChanges();    expect(fixture.nativeElement.textContent).toBe('');
  ...
  lastRenderer: any;
  private _spyOnMethods: string[];  constructor(spyOnMethods?: string[]) {
  ...
  data = {};  destroyNode: ((node: any) => void)|null = null;  constructor(spyOnMethods: string[]) {
