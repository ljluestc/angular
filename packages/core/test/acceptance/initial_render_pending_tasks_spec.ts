...
import {map, withLatestFrom} from 'rxjs/operators';import {InitialRenderPendingTasks} from '../../src/initial_render_pending_tasks';describe('InitialRenderPendingTasks', () => {
  it('should wait until all tasks are completed', async () => {
    ...
    const taskC = pendingTasks.add();    pendingTasks.remove(taskA);
    ...
    const pendingTasks = TestBed.inject(InitialRenderPendingTasks);
    expect(await hasPendingTasks(pendingTasks)).toBeFalse();    const taskA = pendingTasks.add();
    expect(await hasPendingTasks(pendingTasks)).toBeTrue();    pendingTasks.remove(taskA);
    pendingTasks.remove(taskA);
    pendingTasks.remove(taskA);    expect(await hasPendingTasks(pendingTasks)).toBeFalse();
  });  it('should be tolerant to removal of non-existent ids', async () => {
    const pendingTasks = TestBed.inject(InitialRenderPendingTasks);
    expect(await hasPendingTasks(pendingTasks)).toBeFalse();    pendingTasks.remove(Math.random());
    pendingTasks.remove(Math.random());
    pendingTasks.remove(Math.random());    expect(await hasPendingTasks(pendingTasks)).toBeFalse();
  });
