...
import {TestBed} from '@angular/core/testing';describe('TemplateRef', () => {
  describe('rootNodes', () => {
...
fixture.detectChanges();const embeddedView = fixture.componentInstance.templateRef.createEmbeddedView({});
embeddedView.detectChanges();return embeddedView.rootNodes;
...
class App {
  @ViewChild(MenuContent) content!: MenuContent;  constructor(public viewContainerRef: ViewContainerRef) {}
...
viewRef.rootNodes.map(node => node && node.textContent.trim())
   .filter(text => text !== '' && text.indexOf('ng-reflect-ng-if') === -1);expect(rootNodeTextContent).toEqual(['Header', 'Item one', 'Item two', 'Item three']);
    });    it('should descend into view containers on ng-template', () => {
const rootNodes = getRootNodes(`
<ng-template #templateRef>
  <ng-template [ngIf]="true">text|</ng-template>SUFFIX
</ng-template>`);expect(rootNodes.length).toBe(3);
...
class TestCmp {
 ...
 DynamicCmp, `<ng-template #templateRef><ng-content></ng-content></ng-template>`);  const fixture = TestBed.createComponent(TestCmp);
  // Number of projectable nodes matches the number of slots - all nodes should be returned
  const projectableNodes = [[document.createTextNode('textNode')]];  const cmptRef = fixture.componentInstance.vcr.createComponent(
 DynamicCmp, {injector: Injector.NULL, projectableNodes});  const viewRef = cmptRef.instance.templateRef.createEmbeddedView({});
  expect(viewRef.rootNodes.length).toBe(1);
});it('should return an empty collection when no projectable nodes were provided', () => {
  TestBed.overrideTemplate(
 DynamicCmp, `<ng-template #templateRef><ng-content></ng-content></ng-template>`);  const fixture = TestBed.createComponent(TestCmp);  // There are slots but projectable nodes were not provided - nothing should be returned
...
});it('should return an empty collection when projectable nodes were provided but there are no slots',TestBed.overrideTemplate(DynamicCmp, `<ng-template #templateRef></ng-template>`);const fixture = TestBed.createComponent(TestCmp);// There are no slots but projectable were provided - nothing should be returned
const projectableNodes = [[document.createTextNode('textNode')]];const cmptRef = fixture.componentInstance.vcr.createComponent(
DynamicCmp, {injector: Injector.NULL, projectableNodes});
const viewRef = cmptRef.instance.templateRef.createEmbeddedView({});
expect(viewRef.rootNodes.length).toBe(0);    });
...
fixture.detectChanges();expect(fixture.nativeElement.textContent).toBe('Frodo');context.name = 'Bilbo';
fixture.detectChanges();expect(fixture.nativeElement.textContent).toBe('Bilbo');
...
fixture.componentInstance.containerRef.insert(viewRef);
fixture.detectChanges();expect(fixture.nativeElement.textContent).toBe('Frodo');viewRef.context = {name: 'Bilbo'};
fixture.detectChanges();expect(fixture.nativeElement.textContent).toBe('Bilbo');
...
class ListenerTest {
  @ViewChild('templateRef') templateRef!: TemplateRef<any>;
  @ViewChild('containerRef', {read: ViewContainerRef}) containerRef!: ViewContainerRef;  log(name: string) {
events.push(name);
  }
...
button.click();
expect(events).toEqual(['Frodo']);viewRef.context = {name: 'Bilbo'};
fixture.detectChanges();
button.click();
