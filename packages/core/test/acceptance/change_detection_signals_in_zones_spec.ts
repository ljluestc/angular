...
import {TestBed} from '@angular/core/testing';import {signal} from '../../src/signals';describe('CheckAlways components', () => {
...
value = signal('initial');
    ...
    expect(fixture.nativeElement.textContent.trim()).toEqual('initial');    instance.value.set('new');
...
show = signal(true);
    ...
    const instance = fixture.componentInstance;    fixture.detectChanges();
    expect(fixture.nativeElement.textContent.trim()).toEqual('John aged 25');    instance.show.set(false);
    fixture.detectChanges();
    expect(fixture.nativeElement.textContent.trim()).toEqual('anonymous');    instance.name.set('Bob');    fixture.detectChanges();
    expect(fixture.nativeElement.textContent.trim()).toEqual('anonymous');    instance.show.set(true);    fixture.detectChanges();
...
changeDetection: ChangeDetectionStrategy.OnPush
    ...
    fixture.detectChanges();
    expect(fixture.nativeElement.textContent.trim()).toEqual('initial');    value.set('new');
    fixture.detectChanges();
...
incrementTemplateExecutions() {
  ...
  return '';
    ...
    expect(fixture.nativeElement.textContent.trim()).toEqual('initial');    fixture.detectChanges();
    // Should not be dirty, should not execute template
    expect(instance.numTemplateExecutions).toBe(1);    instance.value.set('new');
    fixture.detectChanges();
    ...
    expect(instance.value()).toBe('new');
     ...
     () => {
 const state = signal('initial'); @Component({
 'child',
 `child`,
ection: ChangeDetectionStrategy.OnPush,
e: true,
 })
...
ection: ChangeDetectionStrategy.OnPush,
e: true,
[NgIf, ChildReadingSignalCmp],
 })
 class OnPushCmp {
teExecutions = 0;
TemplateExecutions() {
this.numTemplateExecutions++;
return ''; } const fixture = TestBed.createComponent(OnPushCmp);
 const instance = fixture.componentInstance; fixture.detectChanges();
 expect(instance.numTemplateExecutions).toBe(1);
 expect(fixture.nativeElement.textContent.trim()).toEqual('child'); // The "state" signal is not accesses in the template's update function anywhere so it
...
;nput(newValue: string) {
this.test = state() + ':' + newValue; } @Component({
 `
 {{incrementTemplateExecutions()}}
 <!-- Template constructed to execute child component constructor in the update pass of a host component -->
...
return ''; } const fixture = TestBed.createComponent(OnPushCmp);
 const instance = fixture.componentInstance; fixture.detectChanges();
 expect(instance.numTemplateExecutions).toBe(1);
 expect(fixture.nativeElement.textContent.trim()).toEqual('initial:input'); // The "state" signal is not accesses in the template's update function anywhere so it
 // shouldn't mark components as dirty / impact change detection.
 state.set('new');
 fixture.detectChanges();
 expect(instance.numTemplateExecutions).toBe(1);
...
template: `
ntTemplateExecutions()}}
late constructed to execute child component constructor in the update pass of a host component -->
ate [ngIf]="true"><with-query-setter /></ng-template>
 `,
changeDetection: ChangeDetectionStrategy.OnPush,
standalone: true,
...
standalone: true,
    ...
    class ChildCmp {
useBlue = signal(false);numTemplateExecutions = 0;
incrementTemplateExecutions() {
  this.numTemplateExecutions++;
  return '';
}
...
standalone: true,
    ...
    expect(child.nativeElement.outerHTML).not.toContain('blue');    childInstance.useBlue.set(true);
    fixture.detectChanges();
...
standalone: true,
 ...
 .componentInstance as ChildCmp;    fixture.detectChanges();
    ...
    expect(fixture.nativeElement.outerHTML).not.toContain('blue');    child.useBlue.set(true);
    fixture.detectChanges();
    ...
    fixture.detectChanges();
    expect(fixture.nativeElement.outerHTML).toContain('new');    // Set the signal in the child host binding again and ensure it is still updated
