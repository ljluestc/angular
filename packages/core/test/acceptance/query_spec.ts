...
import {By} from '@angular/platform-browser';describe('query logic', () => {
  beforeEach(() => {
...
<div><simple-comp-b #viewQuery></simple-comp-b></div>const fixture = initWithTemplate(QueryComp, template);
...
const template = `
 ...
 `;
const fixture = initWithTemplate(QueryComp, template);
const comp = fixture.componentInstance;expect(comp.viewChild).toBeInstanceOf(ElementRef);
expect(comp.viewChild.nativeElement).toBe(fixture.debugElement.children[0].nativeElement);expect(comp.viewChildren.first).toBeInstanceOf(ElementRef);
...
.toBe(fixture.debugElement.childNodes[0].nativeNode);expect(comp.viewChildren.first).toBeInstanceOf(TemplateRef);
...
const component = fixture.componentInstance;// static ViewChild query should be set in creation mode, before CD runs
expect(component.textDir).toBeInstanceOf(TextDirective);
expect(component.textDir.text).toEqual('');
expect(component.setEvents).toEqual(['textDir set']);// dynamic ViewChild query should not have been resolved yet
expect(component.foo).not.toBeDefined();const span = fixture.nativeElement.querySelector('span');
fixture.detectChanges();
...
const spans = fixture.nativeElement.querySelectorAll('span');// static ViewChild queries should be set in creation mode, before CD runs
expect(component.textDir).toBeInstanceOf(TextDirective);
expect(component.textDir.text).toEqual('');
expect(component.bar.nativeElement).toEqual(divs[1]);// dynamic ViewChild queries should not have been resolved yet
expect(component.foo).not.toBeDefined();
expect(component.baz).not.toBeDefined();fixture.detectChanges();
expect(component.textDir.text).toEqual('some text');
...
<static-view-query-comp></static-view-query-comp>TestBed.overrideComponent(AppComp, {set: new Component({template})});
const fixture = TestBed.createComponent(AppComp);const firstComponent = fixture.debugElement.children[0].injector.get(StaticViewQueryComp);
const secondComponent = fixture.debugElement.children[1].injector.get(StaticViewQueryComp);// static ViewChild query should be set in creation mode, before CD runs
...
expect(secondComponent.setEvents).toEqual(['textDir set']);// dynamic ViewChild query should not have been resolved yet
expect(firstComponent.foo).not.toBeDefined();
expect(secondComponent.foo).not.toBeDefined();const spans = fixture.nativeElement.querySelectorAll('span');
fixture.detectChanges();
...
const comp = fixture.componentInstance;
fixture.detectChanges();expect(comp.headers).toBeTruthy();
...
}TestBed.configureTestingModule({declarations: [SubComp]});const fixture = TestBed.createComponent(SubComp);
fixture.detectChanges();
...
class MySuperComp {
  @ViewChild('foo') foo: any;
}class MyComp extends MySuperComp {}@Component({selector: 'sub-comp', template: '<div #foo></div>'})
class SubComp extends MyComp {
}TestBed.configureTestingModule({declarations: [SubComp]});const fixture = TestBed.createComponent(SubComp);
fixture.detectChanges();
expect(fixture.componentInstance.foo).toBeInstanceOf(ElementRef);
...
class SubComp extends MyComp {
}TestBed.configureTestingModule({declarations: [SubComp, SomeDir]});const fixture = TestBed.createComponent(SubComp);
fixture.detectChanges();
...
class SomeDir {
  ...
  @ViewChildren(SomeDir) foo!: QueryList<SomeDir>;
}class MyComp extends MySuperComp {}@Component({
  selector: 'sub-comp',
  template: `
<div some-dir></div>
<div some-dir></div>
...
class App {
  ...
  viewChildAvailableInAfterViewInit?: boolean;  ngAfterViewInit() {
...
let queryInstance: QueryList<any>;@Component({
...
fixture.detectChanges();expect((queryInstance!.changes as EventEmitter<any>).closed).toBeFalsy();fixture.componentInstance.condition = false;
fixture.detectChanges();expect((queryInstance!.changes as EventEmitter<any>).closed).toBeTruthy();
    });
...
</local-ref-query-component>const fixture = initWithTemplate(AppComp, template);
...
const comp = firstChild.references['q'];expect(comp.contentChild).toBeInstanceOf(ElementRef);
expect(comp.contentChild.nativeElement).toBe(firstChild.children[0].nativeElement);expect(comp.contentChildren.first).toBeInstanceOf(ElementRef);
expect(comp.contentChildren.last).toBeInstanceOf(ElementRef);
expect(comp.contentChildren.length).toBe(2);
    });    it('should return TemplateRef when template is labeled and retrieved', () => {
const template = `
 <local-ref-query-component #q>
ate #contentQuery></ng-template>
 </local-ref-query-component>
     `;
const fixture = initWithTemplate(AppComp, template);
const comp = fixture.debugElement.children[0].references['q'];
...
const fixture = initWithTemplate(AppComp, template);
const firstChild = fixture.debugElement.children[0];
const comp = firstChild.references['q'];expect(comp.contentChild).toBeInstanceOf(TemplateRef);
expect(comp.contentChild.elementRef.nativeElement).toBe(firstChild.childNodes[0].nativeNode);expect(comp.contentChildren.first).toBeInstanceOf(TemplateRef);
...
plate = `
 ...
 `;
verrideComponent(AppComp, {set: new Component({template})});
ture = TestBed.createComponent(AppComp);
ponent = fixture.debugElement.children[0].injector.get(StaticContentQueryComp); ContentChild query should be set in creation mode, before CD runs
mponent.textDir).toBeInstanceOf(TextDirective);
mponent.textDir.text).toEqual('');
mponent.setEvents).toEqual(['textDir set']);c ContentChild query should not have been resolved yet
mponent.foo).not.toBeDefined();n = fixture.nativeElement.querySelector('span');
componentInstance as any).text = 'some text';
etectChanges();mponent.textDir.text).toEqual('some text');
mponent.foo.nativeElement).toBe(span);
mponent.setEvents).toEqual(['textDir set', 'foo set']);
...
const divs = fixture.nativeElement.querySelectorAll('div');
const spans = fixture.nativeElement.querySelectorAll('span');// static ContentChild queries should be set in creation mode, before CD runs
expect(component.textDir).toBeInstanceOf(TextDirective);
expect(component.textDir.text).toEqual('');
expect(component.bar.nativeElement).toEqual(divs[1]);// dynamic ContentChild queries should not have been resolved yet
expect(component.foo).not.toBeDefined();
expect(component.baz).not.toBeDefined();(fixture.componentInstance as any).text = 'some text';
fixture.detectChanges();
expect(component.textDir.text).toEqual('some text');
expect(component.foo.nativeElement).toBe(spans[0]);
expect(component.baz.nativeElement).toBe(spans[1]);
...
TestBed.overrideComponent(AppComp, {set: new Component({template})});
const fixture = TestBed.createComponent(AppComp);
const component = fixture.debugElement.children[0].injector.get(StaticContentQueryDir);// static ContentChild query should be set in creation mode, before CD runs
expect(component.textDir).toBeInstanceOf(TextDirective);
expect(component.textDir.text).toEqual('');
expect(component.setEvents).toEqual(['textDir set']);// dynamic ContentChild query should not have been resolved yet
expect(component.foo).not.toBeDefined();const span = fixture.nativeElement.querySelector('span');
(fixture.componentInstance as any).text = 'some text';
fixture.detectChanges();expect(component.textDir.text).toEqual('some text');
expect(component.foo.nativeElement).toBe(span);
expect(component.setEvents).toEqual(['textDir set', 'foo set']);
...
const secondComponent = fixture.debugElement.children[1].injector.get(StaticContentQueryComp);// static ContentChild query should be set in creation mode, before CD runs
expect(firstComponent.textDir).toBeInstanceOf(TextDirective);
expect(secondComponent.textDir).toBeInstanceOf(TextDirective);
expect(firstComponent.textDir.text).toEqual('');
expect(secondComponent.textDir.text).toEqual('');
expect(firstComponent.setEvents).toEqual(['textDir set']);
expect(secondComponent.setEvents).toEqual(['textDir set']);// dynamic ContentChild query should not have been resolved yet
expect(firstComponent.foo).not.toBeDefined();
expect(secondComponent.foo).not.toBeDefined();const spans = fixture.nativeElement.querySelectorAll('span');
(fixture.componentInstance as any).text = 'some text';
fixture.detectChanges();expect(firstComponent.textDir.text).toEqual('some text');
expect(secondComponent.textDir.text).toEqual('some text');expect(firstComponent.foo.nativeElement).toBe(spans[0]);
expect(secondComponent.foo.nativeElement).toBe(spans[1]);expect(firstComponent.setEvents).toEqual(['textDir set', 'foo set']);
expect(secondComponent.setEvents).toEqual(['textDir set', 'foo set']);
...
fixture.detectChanges();expect(fixture.componentInstance.subComp.foo).toBeInstanceOf(ElementRef);
    });    it('should support ContentChild query inherited from undecorated grand superclasses', () => {
class MySuperComp {
  @ContentChild('foo') foo: any;
}class MyComp extends MySuperComp {}@Component({selector: 'sub-comp', template: '<ng-content></ng-content>'})
class SubComp extends MyComp {
}@Component({template: '<sub-comp><div #foo></div></sub-comp>'})
class App {
  @ViewChild(SubComp) subComp!: SubComp;
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(fixture.componentInstance.subComp.foo).toBeInstanceOf(QueryList);
...
fixture.detectChanges();const shallowComp = fixture.debugElement.query(By.directive(ShallowComp)).componentInstance;
const queryList = shallowComp!.foos;
expect(queryList.length).toBe(0);fixture.componentInstance.showing = true;
fixture.detectChanges();
expect(queryList.length).toBe(1);fixture.componentInstance.showing = false;
fixture.detectChanges();
expect(queryList.length).toBe(0);
    });    it('should support content queries for directives within repeated embedded views', () => {
const withContentInstances: DirWithContentQuery[] = [];@Directive({
  selector: '[with-content]',
})
class DirWithContentQuery {
  constructor() {
withContentInstances.push(this);
  }  @ContentChildren('foo', {descendants: false}) foos!: QueryList<ElementRef>;  contentInitQuerySnapshot = 0;
  contentCheckedQuerySnapshot = 0;  ngAfterContentInit() {
...
const fixture = TestBed.createComponent(Root);
fixture.detectChanges();for (let i = 0; i < 3; i++) {
  expect(withContentInstances[i].foos.length)
 .toBe(1, `Expected content query to match <span #foo>.`);  expect(withContentInstances[i].contentInitQuerySnapshot)
 .toBe(
 1,
 `Expected content query results to be available when ngAfterContentInit was called.`);  expect(withContentInstances[i].contentCheckedQuerySnapshot)
 .toBe(
 1,
...
fixture.detectChanges();const qList = fixture.componentInstance.contentQueryDirective.foos;
...
fixture.detectChanges();const inQList = fixture.componentInstance.in.qlist;
expect(inQList.length).toBe(1);const outQList = fixture.componentInstance.out.qlist;
...
llowContentQueryDirective {
@ContentChildren('foo') qlist!: QueryList<ElementRef>;e({selector: '[deep-content-query]', standalone: true, exportAs: 'deep-query'})
pContentQueryDirective {
@ContentChildren('foo', {descendants: true}) qlist!: QueryList<ElementRef>;t({
standalone: true,
...
@ViewChild('deep', {static: true}) deep!: DeepContentQueryDirective;ture = TestBed.createComponent(TestCmp);
etectChanges();List = fixture.componentInstance.shallow.qlist;
QList.length).toBe(1);QList = fixture.componentInstance.deep.qlist;
tQList.length).toBe(2);
...
fixture.detectChanges();expect(fixture.componentInstance.queryDir.shallow).toBeUndefined();
...
fixture.detectChanges();const contentQList = fixture.componentInstance.contentQueryDir.foos;
expect(contentQList.length).toBe(1);
expect(contentQList.first.nativeElement.getAttribute('id')).toBe('contentAndView');const viewQList = fixture.componentInstance.fooBars;
...
fixture.detectChanges();expect(fixture.componentInstance.texts.map(item => item.text)).toEqual([
  'A', 'B', 'C', 'D', 'E'
]);
...
fixture.detectChanges();expect(fixture.componentInstance.contentQueryDirective.texts.map(item => item.text)).toEqual([
  'A', 'B', 'C', 'D', 'E'
]);
    });
...
return result.nativeElement != null;
 ...
 () => {
e({selector: '[myDef]'})
ef {t({selector: 'my-container', template: ``})
ontainer {
@ContentChildren(MyDef) myDefs!: QueryList<MyDef>;t(
  {selector: 'test-cmpt', template: `<my-container><tr myDef></tr></my-container>`})
tCmpt {onfigureTestingModule({declarations: [TestCmpt, MyContainer, MyDef]});
ture = TestBed.createComponent(TestCmpt);
tWithQuery = fixture.debugElement.children[0].injector.get(MyContainer);etectChanges();
ptWithQuery.myDefs.length).toBe(1);
...
const cmptWithQuery = fixture.debugElement.children[0].injector.get(NeedsTarget);fixture.detectChanges();
...
fixture.detectChanges();const qList = fixture.componentInstance.query!;
const elToQuery = fixture.nativeElement.querySelector('div');expect(qList.length).toBe(1);
...
fixture.detectChanges();const qList = fixture.componentInstance.query!;
const elToQuery = fixture.nativeElement.querySelectorAll('div');expect(qList.length).toBe(2);
...
t({
standalone: true,
template: `<ng-container #foo></ng-container>`,tCmp {
@ViewChildren('foo', {read: ElementRef}) query?: QueryList<ElementRef>;ture = TestBed.createComponent(TestCmp);
etectChanges();st = fixture.componentInstance.query!;
ist.length).toBe(1);
ist.first.nativeElement.nodeType).toBe(Node.COMMENT_NODE);
...
t({
standalone: true,
template: `<ng-template #foo></ng-template>`,tCmp {
@ViewChildren('foo', {read: ElementRef}) query?: QueryList<ElementRef>;ture = TestBed.createComponent(TestCmp);
etectChanges();st = fixture.componentInstance.query!;
ist.length).toBe(1);
ist.first.nativeElement.nodeType).toBe(Node.COMMENT_NODE);
...
expect(qList.first).toBeInstanceOf(ChildCmp);
 ...
 () => {
e({selector: '[child]', exportAs: 'child', standalone: true})
ldDirective {t({
standalone: true,
imports: [ChildDirective],
template: `<div #foo="child" child></div>`,tCmp {
@ViewChildren('foo') query?: QueryList<ChildDirective>;ture = TestBed.createComponent(TestCmp);
etectChanges();st = fixture.componentInstance.query!;
ist.length).toBe(1);
ist.first).toBeInstanceOf(ChildDirective);
...
fixture.detectChanges();const cmptInstance = fixture.componentInstance;const targetElement = fixture.nativeElement.querySelector('#target');
const fooList = cmptInstance.fooQuery;expect(fooList.length).toBe(1);
expect(fooList.first.nativeElement).toEqual(targetElement);const barList = cmptInstance.barQuery;
...
fixture.detectChanges();const tplQuery = fixture.componentInstance.tplQuery!;
const elRefQuery = fixture.componentInstance.elRefQuery!;expect(tplQuery.length).toBe(3);
expect(tplQuery.first).toBeInstanceOf(TemplateRef);expect(elRefQuery.length).toBe(3);
...
let changes = 0;
fixture.detectChanges();fixture.componentInstance.foos.changes.subscribe((value: any) => {
  changes += 1;
  expect(value).toBe(fixture.componentInstance.foos);
});// refresh without setting dirty - no emit
fixture.detectChanges();
expect(changes).toBe(0);// refresh with setting dirty - emit
...
fixture.detectChanges();componentInstance.foos.changes.subscribe((value: any) => {
...
expect(componentInstance.foos.toArray().length).toEqual(1);// now verify that removing a view does not needlessly fire subscription
fixture.componentInstance.showing = false;
fixture.detectChanges();
expect(changesStrict).toBe(1);  // We detected a change
expect(componentInstance.foos.toArray().length).toEqual(1);// now verify that adding a view does not needlessly fire subscription
fixture.componentInstance.showing = true;
fixture.detectChanges();
expect(changesStrict).toBe(1);  // We detected a change
...
class ViewContainerManipulatorDirective {
  constructor(private _vcRef: ViewContainerRef) {}  insertTpl(tpl: TemplateRef<{}>, ctx: {}, idx?: number): ViewRef {
...
@ViewChildren('foo') query!: QueryList<any>;
  }  TestBed.configureTestingModule({declarations: [TestComponent]});  const fixture = TestBed.createComponent(TestComponent);
  fixture.detectChanges();  const queryList = fixture.componentInstance.query;
  expect(queryList.length).toBe(0);  fixture.componentInstance.value = true;
  fixture.detectChanges();
  expect(queryList.length).toBe(1);  fixture.componentInstance.value = false;
  fixture.detectChanges();
  expect(queryList.length).toBe(0);
...
@ViewChildren('foo') query!: QueryList<any>;
  ...
  const fixture = TestBed.createComponent(TestComponent);
  fixture.detectChanges();  const queryList = fixture.componentInstance.query;
  expect(queryList.length).toBe(0);  fixture.componentInstance.value = ['a', 'b', 'c'];
  fixture.detectChanges();
  expect(queryList.length).toBe(3);  // Remove the "b" element from the value.
  fixture.componentInstance.value.splice(1, 1);
  fixture.detectChanges();
  expect(queryList.length).toBe(2);  // make sure that the "b" element has been removed from query results
 ...
 */
it('should NOT notify on changes when a given view is removed and re-inserted at the same index',@Component({
  selector: 'test-comp',
  template: `
...
class TestComponent implements AfterViewInit {
  queryListNotificationCounter = 0;  @ViewChild(ViewContainerManipulatorDirective) vc!: ViewContainerManipulatorDirective;
  @ViewChild('tpl') tpl!: TemplateRef<any>;
  @ViewChildren('foo') query!: QueryList<any>;  ngAfterViewInit() {
...
const fixture = TestBed.createComponent(TestComponent);
fixture.detectChanges();const queryList = fixture.componentInstance.query;
const {tpl, vc} = fixture.componentInstance;const viewRef = vc.insertTpl(tpl, {}, 0);
fixture.detectChanges();
expect(queryList.length).toBe(1);
expect(fixture.componentInstance.queryListNotificationCounter).toBe(1);vc.move(viewRef, 0);
fixture.detectChanges();
expect(queryList.length).toBe(1);
expect(fixture.componentInstance.queryListNotificationCounter).toBe(1);it('should support a mix of content queries from the declaration and embedded view', () => {
...
items = [1, 2];
 ...
 {declarations: [TestComponent, QueryForContent, QueryForLotsOfContent]});  const fixture = TestBed.createComponent(TestComponent);
  fixture.detectChanges();  const lotsOfContentEl = fixture.debugElement.query(By.directive(QueryForLotsOfContent));
  const lotsOfContentInstance = lotsOfContentEl.injector.get(QueryForLotsOfContent);  const contentEl = fixture.debugElement.query(By.directive(QueryForContent));
  const contentInstance = contentEl.injector.get(QueryForContent);  expect(lotsOfContentInstance.foos1.length).toBe(2);
  expect(lotsOfContentInstance.foos2.length).toBe(2);
  expect(contentInstance.foos.length).toBe(1);  fixture.componentInstance.items = [];
  fixture.detectChanges();
...
it('should report results when different instances of TemplateRef are inserted into one ViewContainerRefs',@Component({
  selector: 'test-comp',
  template: `
<ng-template #tpl1 let-idx="idx">
  <div #foo [id]="'foo1_' + idx"></div>
</ng-template><div #foo id="middle"></div><ng-template #tpl2 let-idx="idx">
  <div #foo [id]="'foo2_' + idx"></div>
</ng-template><ng-template vc></ng-template>
  `,
})
class TestComponent {
...
const fixture = TestBed.createComponent(TestComponent);
fixture.detectChanges();const queryList = fixture.componentInstance.query;
const {tpl1, tpl2, vc} = fixture.componentInstance;expect(queryList.length).toBe(1);
expect(queryList.first.nativeElement.getAttribute('id')).toBe('middle');vc.insertTpl(tpl1!, {idx: 0}, 0);
vc.insertTpl(tpl2!, {idx: 1}, 1);
fixture.detectChanges();expect(queryList.length).toBe(3);
...
expect(qListArr[2].nativeElement.getAttribute('id')).toBe('foo2_1');vc.insertTpl(tpl1!, {idx: 1}, 1);
fixture.detectChanges();expect(queryList.length).toBe(4);
...
expect(qListArr[3].nativeElement.getAttribute('id')).toBe('foo2_1');vc.remove(1);
fixture.detectChanges();expect(queryList.length).toBe(3);
qListArr = queryList.toArray();
expect(qListArr[0].nativeElement.getAttribute('id')).toBe('foo1_0');
expect(qListArr[1].nativeElement.getAttribute('id')).toBe('middle');
expect(qListArr[2].nativeElement.getAttribute('id')).toBe('foo2_1');vc.remove(1);
fixture.detectChanges();expect(queryList.length).toBe(2);
qListArr = queryList.toArray();
expect(qListArr[0].nativeElement.getAttribute('id')).toBe('foo1_0');
expect(qListArr[1].nativeElement.getAttribute('id')).toBe('middle');// https://stackblitz.com/edit/angular-7vvo9j?file=src%2Fapp%2Fapp.component.ts
// https://stackblitz.com/edit/angular-xzwp6n
it('should report results when the same TemplateRef is inserted into different ViewContainerRefs',@Component({
  selector: 'test-comp',
  template: `
<ng-template #tpl let-idx="idx" let-container_idx="container_idx">
  <div #foo [id]="'foo_' + container_idx + '_' + idx"></div>
</ng-template><ng-template vc #vi0="vc"></ng-template>
...
const fixture = TestBed.createComponent(TestComponent);
fixture.detectChanges();const queryList = fixture.componentInstance.query;
const {tpl, vi0, vi1} = fixture.componentInstance;expect(queryList.length).toBe(0);vi0.insertTpl(tpl!, {idx: 0, container_idx: 0}, 0);
vi1.insertTpl(tpl!, {idx: 0, container_idx: 1}, 0);
fixture.detectChanges();expect(queryList.length).toBe(2);
let qListArr = queryList.toArray();
expect(qListArr[0].nativeElement.getAttribute('id')).toBe('foo_0_0');
expect(qListArr[1].nativeElement.getAttribute('id')).toBe('foo_1_0');vi0.remove();
fixture.detectChanges();expect(queryList.length).toBe(1);
qListArr = queryList.toArray();
expect(qListArr[0].nativeElement.getAttribute('id')).toBe('foo_1_0');vi1.remove();
fixture.detectChanges();
expect(queryList.length).toBe(0);// https://stackblitz.com/edit/angular-wpd6gv?file=src%2Fapp%2Fapp.component.ts
...
template: `
 ...
 </ng-template> <ng-template [ngTemplateOutlet]="show ? tpl : null"></ng-template>
`,
  })
  class MyApp {
show = false;
@ViewChildren('foo') query!: QueryList<any>;
  ...
  fixture.detectChanges();  const queryList = fixture.componentInstance.query;
  expect(queryList.length).toBe(0);  fixture.componentInstance.show = true;
  fixture.detectChanges();
  expect(queryList.length).toBe(1);
  expect(queryList.first.nativeElement.id).toBe('from_tpl');  fixture.componentInstance.show = false;
  fixture.detectChanges();
  expect(queryList.length).toBe(0);
});
    });
...
const fixture = TestBed.createComponent(TestCmpt);
fixture.detectChanges();expect(fixture.componentInstance.instances.length).toBe(1);
...
const fixture = TestBed.createComponent(TestCmpt);
fixture.detectChanges();expect(fixture.componentInstance.queryResults.length).toBe(1);
...
const fixture = TestBed.createComponent(TestCmpt);
fixture.detectChanges();expect(fixture.componentInstance.queryResults.length).toBe(2);
expect(fixture.componentInstance.queryResults.first).toBeInstanceOf(WithMultiProvider);
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(fixture.componentInstance.group).toBeInstanceOf(GroupDir);
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();const queryList = fixture.componentInstance.groups;
expect(queryList.length).toBe(3);const groups = queryList.toArray();
...
expect(fixture.componentInstance.token).toBeInstanceOf(TextTokenDirective);
 ...
 () => {
t({template: '<div text-token #Token></div>'})
 {
@ViewChild('Token') token: any;onfigureTestingModule({declarations: [App, TextTokenDirective]});
ture = TestBed.createComponent(App);
etectChanges();
xture.componentInstance.token).toBeInstanceOf(ElementRef);
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();const tokens = fixture.componentInstance.tokens;
...
t({template: '<div text-token #Token></div>'})
 {
@ViewChildren('Token') tokens!: QueryList<any>;onfigureTestingModule({declarations: [App, TextTokenDirective]});
ture = TestBed.createComponent(App);
etectChanges();xture.componentInstance.tokens.toArray()).toEqual([
jasmine.any(ElementRef), jasmine.any(TextTokenDirective) });    it('should match string injection token in a ContentChild query', () => {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(fixture.componentInstance.queryComp.token).toBeInstanceOf(TextTokenDirective);
 ...
 () => {
t({selector: 'has-query', template: '<ng-content></ng-content>'})
Query {
@ContentChild('Token') token: any;t({template: '<has-query><div text-token #Token></div></has-query>'})
 {
@ViewChild(HasQuery) queryComp!: HasQuery;onfigureTestingModule({declarations: [App, HasQuery, TextTokenDirective]});
ture = TestBed.createComponent(App);
etectChanges();xture.componentInstance.queryComp.token).toBeInstanceOf(ElementRef);
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();const tokens = fixture.componentInstance.queryComp.tokens;
expect(tokens.length).toBe(1);
expect(tokens.first).toBeInstanceOf(TextTokenDirective);
...
t({selector: 'has-query', template: '<ng-content></ng-content>'})
Query {
@ContentChildren('Token') tokens!: QueryList<any>;t({template: '<has-query><div text-token #Token></div></has-query>'})
 {
@ViewChild(HasQuery) queryComp!: HasQuery;onfigureTestingModule({declarations: [App, HasQuery, TextTokenDirective]});
ture = TestBed.createComponent(App);
etectChanges();xture.componentInstance.queryComp.tokens.toArray()).toEqual([
jasmine.any(ElementRef), jasmine.any(TextTokenDirective) });    it('should match string token specified through the `read` option of a view query', () => {
...
class QueryComp {
  ...
  @ContentChild('contentQuery') contentChild!: any;  @ViewChildren('viewQuery') viewChildren!: QueryList<any>;
...
class StaticViewQueryComp {
  ...
  setEvents: string[] = [];  @ViewChild(TextDirective, {static: true})
...
class SubclassStaticViewQueryComp extends StaticViewQueryComp {
  @ViewChild('bar', {static: true}) bar!: ElementRef;  @ViewChild('baz') baz!: ElementRef;
}@Component({selector: 'static-content-query-comp', template: `<ng-content></ng-content>`})
class StaticContentQueryComp {
  private _textDir!: TextDirective;
  private _foo!: ElementRef;
  setEvents: string[] = [];  @ContentChild(TextDirective, {static: true})
  get textDir(): TextDirective {
    return this._textDir;
  }  set textDir(value: TextDirective) {
    this.setEvents.push('textDir set');
...
class SubclassStaticContentQueryComp extends StaticContentQueryComp {
  @ContentChild('bar', {static: true}) bar!: ElementRef;  @ContentChild('baz') baz!: ElementRef;
}@Component({
...
export class QueryCompWithChanges {
  @ViewChildren('foo') foos!: QueryList<any>;  showing = false;
}@Component({
...
export class QueryCompWithStrictChangeEmitParent {
  ...
  foos!: QueryList<any>;  constructor(public queryCompWithNoChanges: QueryCompWithNoChanges) {
...
}const MY_OPTION_TOKEN = new InjectionToken<TestComponentWithToken>('ComponentWithToken');@Component({
  selector: 'my-option',
  template: 'Option',
