...
import {TestBed} from '@angular/core/testing';describe('DestroyRef', () => {
  describe('for environnement injector', () => {
    it('should inject cleanup context in services', () => {
let destroyed = false;
const envInjector = createEnvironmentInjector([], TestBed.inject(EnvironmentInjector));envInjector.get(DestroyRef).onDestroy(() => destroyed = true);
expect(destroyed).toBe(false);envInjector.destroy();
expect(destroyed).toBe(true);
    });    it('should allow removal of destroy callbacks', () => {
let destroyed = false;
const envInjector = createEnvironmentInjector([], TestBed.inject(EnvironmentInjector));const unRegFn = envInjector.get(DestroyRef).onDestroy(() => destroyed = true);
expect(destroyed).toBe(false);// explicitly unregister before destroy
unRegFn();
envInjector.destroy();
...
const destroyRef = envInjector.get(DestroyRef);// Register the same fn 3 times:
const unregFn = destroyRef.onDestroy(onDestroyCallback);
destroyRef.onDestroy(onDestroyCallback);
destroyRef.onDestroy(onDestroyCallback);// Unregister the fn 1 time:
unregFn();envInjector.destroy();// Check that the callback was invoked only 2 times
...
const envInjector = createEnvironmentInjector([], TestBed.inject(EnvironmentInjector));
const destroyRef = envInjector.get(DestroyRef);envInjector.destroy();expect(() => {
...
expect(destroyed).toBe(false);fixture.componentRef.destroy();
expect(destroyed).toBe(true);
...
fixture.detectChanges();
expect(destroyed).toBe(false);fixture.componentInstance.show = false;
fixture.detectChanges();
expect(destroyed).toBe(true);
...
constructor(destroyCtx: DestroyRef) {
  this.unRegFn = destroyCtx.onDestroy(() => destroyed = true);
    ...
    expect(destroyed).toBe(false);    // explicitly unregister before destroy
    fixture.componentInstance.unRegFn();    fixture.componentRef.destroy();
    expect(destroyed).toBe(false);
  });  it('should removal single destroy callback if many identical ones are registered', () => {
    let onDestroyCalls = 0;
    const onDestroyCallback = () => onDestroyCalls++;    @Component({
selector: 'test',
standalone: true,
template: ``,
    })
...
constructor(destroyCtx: DestroyRef) {
  ...
  this.unRegFn = destroyCtx.onDestroy(onDestroyCallback);
    ...
    expect(onDestroyCalls).toBe(0);    // explicitly unregister 1-time before destroy
    fixture.componentInstance.unRegFn();    fixture.componentRef.destroy();
...
destroyedLog.push('second');
    ...
    expect(destroyedLog).toEqual([]);    fixture.componentRef.destroy();
