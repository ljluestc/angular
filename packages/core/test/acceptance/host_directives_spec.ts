...
import {By} from '@angular/platform-browser';import {getComponent, getDirectives} from '../../src/render3/util/discovery_utils';describe('host directives', () => {
...
constructor() {
  logs.push('Dir');
    ...
    fixture.detectChanges();    expect(logs).toEqual(['HostDir', 'Dir']);
...
constructor() {
  logs.push('OtherHostDir');
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(logs).toEqual(['HostDir', 'OtherHostDir', 'Dir']);
  });  it('should apply a chain of host directives', () => {
    const logs: string[] = [];
    const token = new InjectionToken('message');
    let diTokenValue: string;    @Directive({
host: {
  'class': 'leaf',
  'id': 'leaf-id',
},
...
constructor() {
  logs.push('SelectorMatchedDir');
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(diTokenValue!).toBe('host value');
    expect(fixture.nativeElement.innerHTML)
...
'SelectorMatchedDir',
    ...
    let secondHostDirInstance!: SecondHostDir;    @Directive({standalone: true})
    class SecondHostDir {
constructor() {
...
@ViewChild(SecondHostDir) secondHost!: SecondHostDir;
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(hostInstance instanceof Host).toBe(true);
    expect(firstHostDirInstance instanceof FirstHostDir).toBe(true);
    expect(secondHostDirInstance instanceof SecondHostDir).toBe(true);    expect(fixture.componentInstance.firstHost).toBe(firstHostDirInstance);
...
`
    ...
    const fixture = TestBed.createComponent(App);
    fixture.detectChanges();    expect(fixture.nativeElement.textContent).toContain('FirstHost | SecondHost');
  });  describe('lifecycle hooks', () => {
    it('should invoke lifecycle hooks from the host directives', () => {
const logs: string[] = [];@Directive({standalone: true})
...
const logs: string[] = [];// Utility so we don't have to repeat the logging code.
@Directive({standalone: true})
abstract class LogsLifecycles implements OnInit, AfterViewInit {
  abstract name: string;  ngOnInit() {
...
let logs: string[] = [];// Utility so we don't have to repeat the logging code.
@Directive({standalone: true})
abstract class LogsLifecycles implements OnChanges {
  @Input() someInput: any;
  abstract name: string;  ngOnChanges(changes: SimpleChanges) {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(logs).toEqual(
['HostDir - hello', 'OtherHostDir - hello', 'HostComp - hello', 'PlainDir - hello']);logs = [];
...
const clicks: string[] = [];@Directive({standalone: true, host: {'host-dir-attr': 'true', '(click)': 'handleClick()'}})
class HostDir {
  handleClick() {
clicks.push('HostDir');
  }
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();const host = fixture.nativeElement.querySelector('[dir]');expect(host.outerHTML)
.toBe(
   '<button host-dir-attr="true" other-host-dir-attr="true" host-attr="true" dir=""></button>');host.click();
fixture.detectChanges();expect(clicks).toEqual(['HostDir', 'OtherHostDir', 'Dir']);
    });    it('should have the host bindings take precedence over the ones from the host directives',
 () => {
e({standalone: true, host: {'id': 'host-dir'}})
tDir {e({standalone: true, host: {'id': 'other-host-dir'}})
erHostDir {e(
  {selector: '[dir]', host: {'id': 'host'}, hostDirectives: [HostDir, OtherHostDir]})
 {t({template: '<div dir></div>'})
 {onfigureTestingModule({declarations: [App, Dir]});
ture = TestBed.createComponent(App);
etectChanges();xture.nativeElement.querySelector('[dir]').getAttribute('id')).toBe('host');
 });
...
class Host {
  ...
  secondHostDir = inject(SecondHostDir);  constructor() {
hostInstance = this;
  }
}@Component({template: '<div dir></div>'})
class App {
...
fixture.detectChanges();expect(hostInstance instanceof Host).toBe(true);
expect(firstHostDirInstance instanceof FirstHostDir).toBe(true);
expect(secondHostDirInstance instanceof SecondHostDir).toBe(true);expect(hostInstance.firstHostDir).toBe(firstHostDirInstance);
...
let hostDirectiveInstance!: HostDir;@Component({selector: 'child', template: ''})
class Child {
...
const injectedInstance = fixture.componentInstance.host.child.hostDir;expect(injectedInstance instanceof HostDir).toBe(true);
...
fixture.detectChanges();expect(hostInstance instanceof Host).toBe(true);
expect(firstHostDirInstance instanceof FirstHostDir).toBe(true);
expect(secondHostDirInstance instanceof SecondHostDir).toBe(true);expect(firstHostDirInstance.host).toBe(hostInstance);
expect(secondHostDirInstance.host).toBe(hostInstance);
 ...
 () => {
en = new InjectionToken<string>('token');
nstance!: Host;
HostDirInstance!: FirstHostDir;
dHostDirInstance!: SecondHostDir;e({standalone: true, providers: [{provide: token, useValue: 'SecondDir'}]})
ondHostDir {
tokenValue = inject(token);constructor() {
  secondHostDirInstance = this;
}e({
standalone: true,
hostDirectives: [SecondHostDir],
providers: [{provide: token, useValue: 'FirstDir'}]stHostDir {
tokenValue = inject(token);constructor() {
  firstHostDirInstance = this;
}e({
selector: '[dir]',
hostDirectives: [FirstHostDir],
providers: [{provide: token, useValue: 'HostDir'}]t {
tokenValue = inject(token);constructor() {
  hostInstance = this;
}t({template: '<div dir></div>'})
 {onfigureTestingModule({declarations: [App, Host]});
ture = TestBed.createComponent(App);
etectChanges();stInstance instanceof Host).toBe(true);
rstHostDirInstance instanceof FirstHostDir).toBe(true);
condHostDirInstance instanceof SecondHostDir).toBe(true);stInstance.tokenValue).toBe('HostDir');
rstHostDirInstance.tokenValue).toBe('HostDir');
condHostDirInstance.tokenValue).toBe('HostDir');
 });    it('should allow the host to inject tokens from the host directives', () => {
const firstToken = new InjectionToken<string>('firstToken');
const secondToken = new InjectionToken<string>('secondToken');@Directive({standalone: true, providers: [{provide: secondToken, useValue: 'SecondDir'}]})
class SecondHostDir {
}@Directive({
  standalone: true,
  hostDirectives: [SecondHostDir],
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(fixture.componentInstance.host.firstTokenValue).toBe('FirstDir');
expect(fixture.componentInstance.host.secondTokenValue).toBe('SecondDir');
 ...
 () => {
en = new InjectionToken<string>('token');
Value: string|undefined;e({standalone: true, providers: [{provide: token, useValue: 'host-dir'}]})
tDir {t({
selector: 'host',
hostDirectives: [HostDir],
providers: [{provide: token, useValue: 'host'}],
template: '<span child></span>',t {e({selector: '[child]'})
ld {
constructor() {
  tokenValue = inject(token);
}t({template: '<host></host>'})
 {onfigureTestingModule({declarations: [App, Host, Child]});
ture = TestBed.createComponent(App);
etectChanges();kenValue).toBe('host');
 });    it('should not be able to access viewProviders from the host in the host directives', () => {
const token = new InjectionToken<string>('token');
let tokenValue: string|null = null;@Directive({standalone: true})
class HostDir {
  constructor() {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(tokenValue).toBe(null);
    });    it('should throw a circular dependency error if a host and a host directive inject each other',
 () => {
e({standalone: true})
tDir {
host = inject(Host);e({selector: '[dir]', hostDirectives: [HostDir]})
t {
hostDir = inject(HostDir);t({template: '<div dir></div>'})
 {onfigureTestingModule({declarations: [App, Host]});
...
type InternalChangeDetectorRef = ChangeDetectorRef&{_lView: unknown};@Directive({standalone: true})
class HostDir {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();const hostDirectiveCdr = fixture.componentInstance.hostDir.changeDetectorRef;
const componentCdr = fixture.componentInstance.comp.changeDetectorRef;// We can't assert that the change detectors are the same by comparing
...
let hostDirectiveInstance: HostDir|undefined;@Directive({standalone: true, host: {'(click)': 'hasBeenClicked.emit()'}})
class HostDir {
  @Output() hasBeenClicked = new EventEmitter<void>();  constructor() {
hostDirectiveInstance = this;
  }
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.nativeElement.querySelector('button').click();
fixture.detectChanges();expect(hostDirectiveInstance instanceof HostDir).toBe(true);
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.nativeElement.querySelector('button').click();
fixture.detectChanges();expect(fixture.componentInstance.spy).toHaveBeenCalledOnceWith('hello');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.nativeElement.querySelector('button').click();
fixture.detectChanges();expect(fixture.componentInstance.validSpy).toHaveBeenCalledOnceWith('hello');
...
expect(fixture.componentInstance.invalidSpy).not.toHaveBeenCalled();
 ...
 () => {
e({standalone: true, host: {'(click)': 'hasBeenClicked.emit("HostDir")'}})
tDir {
@Output() hasBeenClicked = new EventEmitter<string>();e({
selector: '[dir]',
hostDirectives: [HostDir],
host: {'(click)': 'hasBeenClicked.emit("Dir")'} {
@Output() hasBeenClicked = new EventEmitter<string>();t({template: '<button dir (hasBeenClicked)="spy($event)"></button>'})
 {
spy = jasmine.createSpy('click spy');onfigureTestingModule({declarations: [App, Dir]});
ture = TestBed.createComponent(App);
etectChanges();ativeElement.querySelector('button').click();
etectChanges();xture.componentInstance.spy).toHaveBeenCalledOnceWith('Dir');
...
ture = TestBed.createComponent(App);
etectChanges();ativeElement.querySelector('button').click();
etectChanges();xture.componentInstance.spy).toHaveBeenCalledTimes(2);
xture.componentInstance.spy).toHaveBeenCalledWith('HostDir');
xture.componentInstance.spy).toHaveBeenCalledWith('Dir');
...
@Output() wasClicked = new EventEmitter<string>();t({template: '<button dir (wasClicked)="spy($event)"></button>'})
 {
spy = jasmine.createSpy('click spy');onfigureTestingModule({declarations: [App, Dir]});
ture = TestBed.createComponent(App);
etectChanges();ativeElement.querySelector('button').click();
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.nativeElement.querySelector('button').click();
fixture.detectChanges();expect(fixture.componentInstance.spy).toHaveBeenCalledTimes(1);
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();fixture.nativeElement.querySelector('button').click();
fixture.detectChanges();expect(fixture.componentInstance.spy).toHaveBeenCalledOnceWith('ExposedHostDir');
...
e({
standalone: true,
host: {'(click)': 'firstHasBeenClicked.emit("FirstHostDir")'},stHostDir {
@Output() firstHasBeenClicked = new EventEmitter<string>();e({
standalone: true,
host: {'(click)': 'secondHasBeenClicked.emit("SecondHostDir")'},ondHostDir {
@Output() secondHasBeenClicked = new EventEmitter<string>();e({
selector: '[dir]',
hostDirectives: [
...
] {t({template: '<button dir (wasClicked)="spy($event)"></button>'})
 {
spy = jasmine.createSpy('click spy');onfigureTestingModule({declarations: [App, Dir]});
ture = TestBed.createComponent(App);
etectChanges();ativeElement.querySelector('button').click();
etectChanges();xture.componentInstance.spy).toHaveBeenCalledTimes(2);
xture.componentInstance.spy).toHaveBeenCalledWith('FirstHostDir');
xture.componentInstance.spy).toHaveBeenCalledWith('SecondHostDir');
 });
  });  describe('inputs', () => {
...
class App {
  color = 'red';
}TestBed.configureTestingModule({declarations: [App, Dir], errorOnUnknownProperties: true});expect(() => {
  const fixture = TestBed.createComponent(App);
  fixture.detectChanges();
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();
expect(fixture.componentInstance.hostDir.color).toBe('red');fixture.componentInstance.color = 'green';
fixture.detectChanges();
...
e({standalone: true})
tDir {
@Input() color?: string;e({
selector: '[dir]',
hostDirectives: [HostDir], {
@Input() color?: string;t({template: '<button dir [color]="color"></button>'})
 {
@ViewChild(Dir) dir!: Dir;
@ViewChild(HostDir) hostDir!: HostDir;
color = 'red';onfigureTestingModule({declarations: [App, Dir]});
ture = TestBed.createComponent(App);
etectChanges();
r, hostDir} = fixture.componentInstance;r.color).toBe('red');
stDir.color).toBe(undefined);omponentInstance.color = 'green';
etectChanges();r.color).toBe('green');
stDir.color).toBe(undefined);
 });    it('should set an input of a host that has the same name as an exposed input of a host directive',
 () => {
e({standalone: true})
tDir {
@Input() color?: string;e({selector: '[dir]', hostDirectives: [{directive: HostDir, inputs: ['color']}]})
 {
@Input() color?: string;t({template: '<button dir [color]="color"></button>'})
 {
@ViewChild(Dir) dir!: Dir;
...
etectChanges();
r, hostDir} = fixture.componentInstance;r.color).toBe('red');
stDir.color).toBe('red');omponentInstance.color = 'green';
etectChanges();r.color).toBe('green');
stDir.color).toBe('green');
...
selector: '[dir]',
hostDirectives: [{directive: HostDir, inputs: ['color: buttonColor']}] {
@Input() buttonColor?: string;t({template: '<button dir [buttonColor]="color"></button>'})
 {
@ViewChild(Dir) dir!: Dir;
@ViewChild(HostDir) hostDir!: HostDir;
color = 'red';onfigureTestingModule({declarations: [App, Dir]});
ture = TestBed.createComponent(App);
etectChanges();
r, hostDir} = fixture.componentInstance;r.buttonColor).toBe('red');
stDir.color).toBe('red');omponentInstance.color = 'green';
etectChanges();r.buttonColor).toBe('green');
stDir.color).toBe('green');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(fixture.componentInstance.hostDir.color).toBe('red');fixture.componentInstance.color = 'green';
fixture.detectChanges();expect(fixture.componentInstance.hostDir.color).toBe('green');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();
const {exposedHostDir, unExposedHostDir} = fixture.componentInstance;expect(exposedHostDir.color).toBe('red');
expect(unExposedHostDir.color).toBe(undefined);fixture.componentInstance.color = 'green';
fixture.detectChanges();expect(exposedHostDir.color).toBe('green');
...
e({standalone: true})
stHostDir {
@Input() firstColor?: string;e({standalone: true})
ondHostDir {
@Input() secondColor?: string;e({
selector: '[dir]',
hostDirectives: [
  {directive: FirstHostDir, inputs: ['firstColor: buttonColor']},
  {directive: SecondHostDir, inputs: ['secondColor: buttonColor']}
] {t({template: '<button dir [buttonColor]="color"></button>'})
 {
...
ture = TestBed.createComponent(App);
etectChanges();
rstHostDir, secondHostDir} = fixture.componentInstance;rstHostDir.firstColor).toBe('red');
condHostDir.secondColor).toBe('red');omponentInstance.color = 'green';
etectChanges();rstHostDir.firstColor).toBe('green');
condHostDir.secondColor).toBe('green');
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(fixture.componentInstance.hostDir.color).toBe(undefined);
...
e({standalone: true})
tDir {
@Input('colorAlias') color?: string;e({
selector: '[dir]',
hostDirectives: [{directive: HostDir, inputs: ['colorAlias: buttonColor']}] {t({template: '<button dir buttonColor="red"></button>'})
 {
@ViewChild(HostDir) hostDir!: HostDir;onfigureTestingModule({declarations: [App, Dir]});
ture = TestBed.createComponent(App);
etectChanges();
xture.componentInstance.hostDir.color).toBe('red');
 });    it('should set a static input that was exposed from one host directive, but not another',
 () => {
e({standalone: true})
osedHostDir {
@Input() color?: string;e({standalone: true})
xposedHostDir {
@Input() color?: string;e({
selector: '[dir]',
hostDirectives: [{directive: ExposedHostDir, inputs: ['color']}, UnExposedHostDir] {t({template: '<button dir color="red"></button>'})
 {
@ViewChild(ExposedHostDir) exposedHostDir!: ExposedHostDir;
@ViewChild(UnExposedHostDir) unExposedHostDir!: UnExposedHostDir;onfigureTestingModule({declarations: [App, Dir]});
ture = TestBed.createComponent(App);
etectChanges();xture.componentInstance.exposedHostDir.color).toBe('red');
xture.componentInstance.unExposedHostDir.color).toBe(undefined);
...
hostDirectives: [
  {directive: FirstHostDir, inputs: ['firstColor: buttonColor']},
  {directive: SecondHostDir, inputs: ['secondColor: buttonColor']}
] {t({template: '<button dir buttonColor="red"></button>'})
 {
@ViewChild(FirstHostDir) firstHostDir!: FirstHostDir;
@ViewChild(SecondHostDir) secondHostDir!: SecondHostDir;
color = 'red';onfigureTestingModule({declarations: [App, Dir]});
ture = TestBed.createComponent(App);
etectChanges();xture.componentInstance.firstHostDir.firstColor).toBe('red');
xture.componentInstance.secondHostDir.secondColor).toBe('red');
 });    it('should not expose an input under its host directive alias if a host directive is not applied',
 () => {
s: string[] = [];e({selector: '[host-dir]', standalone: true})
tDir implements OnChanges {
@Input('colorAlias') color?: string;ngOnChanges(changes: SimpleChanges) {
  logs.push(changes['color'].currentValue);
}e({
selector: '[dir]',
standalone: true,
hostDirectives: [{directive: HostDir, inputs: ['colorAlias: buttonColor']}] {t({
standalone: true,
imports: [Dir, HostDir],
...
template: `
 ...
 ` {
spanValue = 'spanValue';
buttonValue = 'buttonValue';onfigureTestingModule({errorOnUnknownProperties: true}); => {
const fixture = TestBed.createComponent(App);
fixture.detectChanges();
wError(/Can't bind to 'buttonColor' since it isn't a known property of 'button'/);put on the button instance should not have been written to.
gs).toEqual(['spanValue']);
 });
  });  describe('ngOnChanges', () => {
    it('should invoke ngOnChanges when an input is set on a host directive', () => {
let firstDirChangeEvent: SimpleChanges|undefined;
let secondDirChangeEvent: SimpleChanges|undefined;@Directive({standalone: true})
class FirstHostDir implements OnChanges {
  @Input() color?: string;  ngOnChanges(changes: SimpleChanges) {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(firstDirChangeEvent).toEqual(jasmine.objectContaining({
...
let secondDirChangeEvent: SimpleChanges|undefined;@Directive({standalone: true})
class FirstHostDir implements OnChanges {
  @Input('firstAlias') color?: string;  ngOnChanges(changes: SimpleChanges) {
firstDirChangeEvent = changes;
  }
}@Directive({standalone: true})
class SecondHostDir implements OnChanges {
  @Input('secondAlias') color?: string;  ngOnChanges(changes: SimpleChanges) {
secondDirChangeEvent = changes;
  }
}@Directive({
  selector: '[dir]',
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(firstDirChangeEvent).toBe(undefined);
expect(secondDirChangeEvent).toEqual(jasmine.objectContaining({
  color: jasmine.objectContaining({
firstChange: true,
previousValue: undefined,
...
let latestChangeEvent: SimpleChanges|undefined;@Directive({standalone: true})
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();expect(latestChangeEvent).toEqual(jasmine.objectContaining({
  color: jasmine.objectContaining({
firstChange: true,
previousValue: undefined,
currentValue: 'red',
...
let plainDirInstance!: PlainDir;@Directive({standalone: true})
class HostDir {
  constructor() {
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();
const componentHost = fixture.nativeElement.querySelector('comp');expect(hostDirInstance instanceof HostDir).toBe(true);
...
expect(getDirectives(componentHost)).toEqual([
 ...
 () => {
nstance!: Comp;e({standalone: true})
tDir {t({
selector: 'comp',
template: '',
hostDirectives: [HostDir],p {
constructor() {
  compInstance = this;
}t({template: '<comp></comp>'})
 {onfigureTestingModule({declarations: [App, Comp]});
ture = TestBed.createComponent(App);
etectChanges();
ponentHost = fixture.nativeElement.querySelector('comp');mpInstance instanceof Comp).toBe(true);
tComponent(componentHost)).toBe(compInstance);
...
ture = TestBed.createComponent(App);
etectChanges();
e = fixture.debugElement.query(By.css('comp'));mpInstance instanceof Comp).toBe(true);
de.componentInstance).toBe(compInstance);
...
const result = fixture.debugElement.query(By.directive(HostDir)).nativeElement;expect(result).toBe(expected);
    });
...
fixture.detectChanges();
const ref = fixture.componentInstance.insertionPoint.createComponent(componentType);return {ref, fixture};
...
}const {fixture} = createRootComponent(HostComp);expect(logs).toEqual(['HostDir', 'HostComp']);
expect(fixture.nativeElement.innerHTML)
.toContain(
...
otherHostDir = inject(OtherHostDir);
 ...
 'other-host-dir-only="other-host-dir"></host-comp>');  ref.instance.hostDir.value = 'host-dir-changed';
 ...
 'other-host-dir-only="other-host-dir-changed"></host-comp>');
  ...
  const logs: string[] = [];  @Directive({standalone: true, host: {'(click)': 'handleClick()'}})
  class HostDir {
handleClick() {
 logs.push('HostDir');
}
...
handleClick() {
 logs.push('HostComp');
  ...
  }  const {fixture, ref} = createRootComponent(HostComp);  ref.location.nativeElement.click();
  fixture.detectChanges();  expect(logs).toEqual(['HostDir', 'OtherHostDir', 'HostComp']);
});it('should have the host bindings of the root component take precedence over the ones from the host directives',@Directive({standalone: true, host: {'id': 'host-dir'}})
class HostDir {
...
fixture.detectChanges();
expect(ref.location.nativeElement.getAttribute('id')).toBe('host');    });    describe('dependency injection', () => {
it('should allow the host directive to inject the root component', () => {
  let hostDirInstance!: HostDir;  @Directive({standalone: true})
  class HostDir {
host = inject(HostComp);constructor() {
 hostDirInstance = this;
}
  }  @Component({hostDirectives: [HostDir], template: ''})
  class HostComp {
  }  const {ref} = createRootComponent(HostComp);  expect(hostDirInstance instanceof HostDir).toBe(true);
...
});it('should give precedence to the DI tokens from the root component over the host directive tokens',const token = new InjectionToken<string>('token');
let hostInstance!: HostComp;
let firstHostDirInstance!: FirstHostDir;
let secondHostDirInstance!: SecondHostDir;@Directive({standalone: true, providers: [{provide: token, useValue: 'SecondDir'}]})
class SecondHostDir {
  tokenValue = inject(token);  constructor() {
secondHostDirInstance = this;
  }
...
}createRootComponent(HostComp);expect(hostInstance instanceof HostComp).toBe(true);
expect(firstHostDirInstance instanceof FirstHostDir).toBe(true);
expect(secondHostDirInstance instanceof SecondHostDir).toBe(true);expect(hostInstance.tokenValue).toBe('HostDir');
expect(firstHostDirInstance.tokenValue).toBe('HostDir');
expect(secondHostDirInstance.tokenValue).toBe('HostDir');it('should allow the root component to inject tokens from the host directives', () => {
  const firstToken = new InjectionToken<string>('firstToken');
  const secondToken = new InjectionToken<string>('secondToken');  @Directive({standalone: true, providers: [{provide: secondToken, useValue: 'SecondDir'}]})
  class SecondHostDir {
  }  @Directive({
...
it('should set inputs on root component and all host directive instances using `setInput`',let hostDirInstance!: HostDir;
let otherHostDirInstance!: OtherHostDir;@Directive({standalone: true})
class HostDir {
  @Input() color?: string;  constructor() {
hostDirInstance = this;
  }
}@Directive({standalone: true})
class OtherHostDir {
...
fixture.detectChanges();expect(hostDirInstance.color).toBe(undefined);
expect(otherHostDirInstance.color).toBe(undefined);
expect(ref.instance.color).toBe(undefined);ref.setInput('color', 'green');expect(hostDirInstance.color).toBe('green');
expect(otherHostDirInstance.color).toBe('green');
expect(ref.instance.color).toBe('green');it('should set inputs that only exist on a host directive when using `setInput`', () => {
  let hostDirInstance!: HostDir;  @Directive({standalone: true})
  class HostDir {
@Input() color?: string;constructor() {
 hostDirInstance = this;
...
color?: string;  // Note: intentionally not marked as @Input.
  }  const {ref} = createRootComponent(HostComp);  expect(hostDirInstance.color).toBe(undefined);
  expect(ref.instance.color).toBe(undefined);  ref.setInput('color', 'color');  expect(hostDirInstance.color).toBe('color');
...
@Input() color?: string;
  ...
  fixture.detectChanges();  expect(hostDirInstance.color).toBe(undefined);
  expect(ref.instance.color).toBe(undefined);  ref.setInput('color', 'green');  expect(hostDirInstance.color).toBe(undefined);
  expect(ref.instance.color).toBe('green');
});it('should use the input name alias in `setInput`', () => {
  let hostDirInstance!: HostDir;  @Directive({standalone: true})
  class HostDir {
@Input('alias') color?: string;constructor() {
 hostDirInstance = this;
}
  }  @Component({
selector: 'host-comp',
hostDirectives: [{
 ...
 inputs: ['alias: customAlias'],
  ...
  fixture.detectChanges();  expect(hostDirInstance.color).toBe(undefined);  // Check that the old alias or the original name aren't available first.
 ...
 .toThrowError(/NG0303: Can't set value of the 'alias' input/);
  expect(hostDirInstance.color).toBe(undefined);  // Check the alias.
  ...
  let latestChanges: SimpleChanges|undefined;  @Directive({standalone: true})
  class HostDir implements OnChanges {
@Input('alias') color?: string;ngOnChanges(changes: SimpleChanges) {
...
hostDirectives: [{directive: HostDir, inputs: ['alias: customAlias']}],
  ...
  class HostComp {
  }  const {ref, fixture} = createRootComponent(HostComp);  expect(latestChanges).toBe(undefined);  ref.setInput('customAlias', 'red');
  fixture.detectChanges();  expect(latestChanges).toEqual(jasmine.objectContaining({
color: jasmine.objectContaining({
 previousValue: undefined,
 currentValue: 'red',
 firstChange: true,
...
class HostDir {}@Directive({selector: '[dir]', hostDirectives: [HostDir]})
class Dir {
}@Component({template: '<div dir></div>'})
class App {
}TestBed.configureTestingModule({declarations: [App, Dir]});expect(() => TestBed.createComponent(App))
.toThrowError(
...
class App {
}TestBed.configureTestingModule({declarations: [App, Dir]});expect(() => {
  const fixture = TestBed.createComponent(App);
  fixture.detectChanges();
})
.toThrowError(
