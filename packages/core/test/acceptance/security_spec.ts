...
import {DomSanitizer} from '@angular/platform-browser';describe('comment node text escaping', () => {
 ...
 () => {
t({template: `<div><span *ngIf="xssValue"></span><div>`})
Comp {
// ngIf serializes the `xssValue` into a comment for debugging purposes.
xssValue: string = xssValue + '<script>"evil"</script>';onfigureTestingModule({declarations: [XSSComp]});
ture = TestBed.createComponent(XSSComp);
etectChanges();
 = fixture.nativeElement.querySelector('div') as HTMLElement;
ize into a string to mimic SSR serialization.
l = div.innerHTML;
ust be escaped or we have XSS.
ml).not.toContain('--><script');
rse it back into DOM (from string)
HTML = html;
 that we did not accidentally deserialize the `<script>`
ipt = div.querySelector('script');
ript).toBeFalsy();
 });
  });
...
fixture.detectChanges();
    }).toThrowError(getErrorMessageRegexp());    ensureNoIframePresent(fixture);
    return fixture!;
...
fixture = TestBed.createComponent(component);
fixture.detectChanges();
    }).not.toThrow();    const iframe = fixture!.nativeElement.querySelector('iframe');
...
['sandbox', 'allow', 'allowFullscreen', 'referrerPolicy', 'csp', 'fetchPriority'];  const TEST_IFRAME_URL = 'https://angular.io/assets/images/logos/angular/angular.png';  let oldNgDevMode!: typeof ngDevMode;  beforeAll(() => {
    oldNgDevMode = ngDevMode;
  });  afterAll(() => {
    global['ngDevMode'] = oldNgDevMode;
  });  [true, false].forEach(devModeFlag => {
    beforeAll(() => {
global['ngDevMode'] = devModeFlag;// TestBed and JIT compilation have some dependencies on the ngDevMode state, so we need to
...
class IframeComp {
 ...
 class IframeComp {
   ...
   src = this.sanitizeFn(TEST_IFRAME_URL);   get sanitizeFn() {
...
});it('should work when a directive sets a security-sensitive attribute as a static attribute',@Directive({
  standalone: true,
  selector: '[dir]',
  host: {
'src': TEST_IFRAME_URL,
...
class IframeComp {
}expectIframeToBeCreated(IframeComp, {src: TEST_IFRAME_URL});it('should work when a directive sets a security-sensitive host attribute on a non-iframe element',@Directive({
  standalone: true,
  selector: '[dir]',
  host: {
'src': TEST_IFRAME_URL,
...
fixture.detectChanges();expect(fixture.nativeElement.firstChild.src).toEqual(TEST_IFRAME_URL);it('should work when a security-sensitive attribute on an <iframe> ' +
  'which also has a structural directive (*ngIf)',@Component({
  standalone: true,
  imports: [NgIf],
  selector: 'my-comp',
...
class IframeComp {
  visible = true;
}expectIframeToBeCreated(IframeComp, {src: TEST_IFRAME_URL});it('should work when a security-sensitive attribute is set between `src` and `srcdoc`',@Component({
  standalone: true,
  selector: 'my-comp',
  template: `<iframe src="${TEST_IFRAME_URL}" sandbox srcdoc="Hi!"></iframe>`,
})
class IframeComp {
}expectIframeToBeCreated(IframeComp, {src: TEST_IFRAME_URL});it('should work when a directive sets a security-sensitive attribute before setting `src`',@Directive({
  standalone: true,
  selector: '[dir]',
  host: {
'sandbox': '',
...
class IframeComp {
}expectIframeToBeCreated(IframeComp, {src: TEST_IFRAME_URL});it('should work when a directive sets an `src` and ' +
  'there was a security-sensitive attribute set in a template' +
  '(directive attribute after `sandbox`)',@Directive({
  standalone: true,
  selector: '[dir]',
  host: {
'src': TEST_IFRAME_URL,
...
class IframeComp {
  ...
  'as an attribute binding (checking that it\'s case-insensitive)',@Directive({
  standalone: true,
  selector: '[dir]',
  host: {
'[attr.SANDBOX]': '\'\'',
...
class IframeComp {
}expectIframeCreationToFail(IframeComp);it('should work when a directive sets an `src` and ' +
  'there was a security-sensitive attribute set in a template' +
  '(directive attribute before `sandbox`)',@Directive({
  standalone: true,
  selector: '[dir]',
  host: {
'src': TEST_IFRAME_URL,
...
class IframeComp {
}expectIframeToBeCreated(IframeComp, {src: TEST_IFRAME_URL});it('should work when a directive sets a security-sensitive attribute and ' +
  'there was an `src` attribute set in a template' +
  '(directive attribute after `src`)',@Directive({
  standalone: true,
  selector: '[dir]',
  host: {
'sandbox': '',
...
class IframeComp {
}expectIframeToBeCreated(IframeComp, {src: TEST_IFRAME_URL});it('should work when a security-sensitive attribute is set as a static attribute', () => {
  @Component({
standalone: true,
selector: 'my-comp',
template: `
...
referrerPolicy: 'no-referrer',
  ...
  'as a property binding and an <iframe> is wrapped into another element',@Component({
  standalone: true,
  selector: 'my-comp',
  template: `
 <section>
...
class IframeComp {
}expectIframeCreationToFail(IframeComp);it('should work when a directive sets a security-sensitive attribute and ' +
  'there was an `src` attribute set in a template' +
  '(directive attribute before `src`)',@Directive({
  standalone: true,
  selector: '[dir]',
  host: {
'sandbox': '',
...
class IframeComp {
}expectIframeToBeCreated(IframeComp, {src: TEST_IFRAME_URL});it('should work when a directive that sets a security-sensitive attribute goes ' +
...
class IframeComp {
}expectIframeToBeCreated(IframeComp, {src: TEST_IFRAME_URL});it('should work when a directive that sets a security-sensitive attribute has ' +
...
class IframeComp {
}expectIframeToBeCreated(IframeComp, {src: TEST_IFRAME_URL});it('should work when a directive that sets an `src` has ' +
  'a host directive that sets a security-sensitive attribute value',@Directive({
  standalone: true,
...
class IframeComp {
}expectIframeToBeCreated(IframeComp, {src: TEST_IFRAME_URL});it('should error when creating a view that contains an <iframe> ' +
  'with security-sensitive attributes set via property bindings',@Component({
  standalone: true,
  selector: 'my-comp',
  template: `
...
class IframeComp {
  ...
  @ViewChild('template') template!: TemplateRef<unknown>;  createEmbeddedView() {
...
fixture.detectChanges();expect(() => {
  ...
  fixture.detectChanges();
}).toThrowError(getErrorMessageRegexp());ensureNoIframePresent(fixture);describe('i18n', () => {
