...
import {ComponentFixture, TestBed} from '@angular/core/testing';describe('change detection for transplanted views', () => {
...
constructor() {
  appComp = this;
    ...
    let onPushDeclareComp!: OnPushDeclareComp;    beforeEach(() => {
...
beforeEach(() => {
 ...
 'Expect transplanted LView to be CD because the declaration is CD.');  insertComp.greeting = 'Hi';
  fixture.detectChanges(false);
  expect(log).toEqual(['Insert']);
  log.length = 0;
  expect(trim(fixture.nativeElement.textContent))
 .toEqual(
 'DeclareComp(Angular) InsertComp(Hello) Hello Angular!',
 'expect no change because it is on push.');  insertComp.changeDetectorRef.markForCheck();
  fixture.detectChanges(false);
  expect(log).toEqual(['Insert']);
  log.length = 0;
  expect(trim(fixture.nativeElement.textContent))
 .toEqual('DeclareComp(Angular) InsertComp(Hi) Hi Angular!');  // Destroy insertion should also destroy declaration
  appComp.showInsert = false;
  fixture.detectChanges(false);
  expect(log).toEqual([]);
  log.length = 0;
  expect(trim(fixture.nativeElement.textContent)).toEqual('DeclareComp(Angular)');  // Restore both
  appComp.showInsert = true;
  fixture.detectChanges(false);
  expect(log).toEqual(['Insert']);
  log.length = 0;
  expect(trim(fixture.nativeElement.textContent))
 .toEqual('DeclareComp(Angular) InsertComp(Hello) Hello Angular!');  // Destroy declaration, But we should still be able to see updates in insertion
...
beforeEach(() => {
 ...
 .toEqual('OnPushDeclareComp(world) InsertComp(Hello) Hello world!');
  ...
  log.length = 0;
  expect(trim(fixture.nativeElement.textContent))
 .toEqual('OnPushDeclareComp(Angular) InsertComp(Hello) Hello Angular!');  // mark insertion point dirty
  insertForOnPushDeclareComp.changeDetectorRef.markForCheck();
  fixture.detectChanges(false);
  expect(log).toEqual(['Insert']);
  log.length = 0;
  expect(trim(fixture.nativeElement.textContent))
 .toEqual('OnPushDeclareComp(Angular) InsertComp(Hi) Hi Angular!');  // mark both insertion and declaration point dirty
  insertForOnPushDeclareComp.changeDetectorRef.markForCheck();
  onPushDeclareComp.changeDetector.markForCheck();
  fixture.detectChanges(false);
  expect(log).toEqual(['Insert']);
...
xit('is checked as part of CheckNoChanges pass', () => {
  ...
  log.length = 0;  // mark insertion point dirty
  insertForOnPushDeclareComp.changeDetectorRef.markForCheck();
  fixture.detectChanges(false);
  expect(log).toEqual(['Insert', null]);
  log.length = 0;  // mark both insertion and declaration point dirty
  insertForOnPushDeclareComp.changeDetectorRef.markForCheck();
  onPushDeclareComp.changeDetector.markForCheck();
  fixture.detectChanges(false);
  expect(log).toEqual(['Insert', null]);
...
expect(appComponent.declaration.transplantedViewRefreshCount).toEqual(0);
 ...
 () => {
ent.declaration.name = 'new name';
ent.insertion.changeDetectorRef.markForCheck();
etectChanges(false);
xture.nativeElement.textContent)
  .toEqual(
...
pComponent.declaration.transplantedViewRefreshCount).toEqual(1);
 });    it('updates only the declaration view when there is a change to declaration and declaration is marked dirty',
 () => {
ent.declaration.name = 'new name';
ent.declaration.changeDetectorRef.markForCheck();
etectChanges(false);ectedContent =
  'Insertion(initial)TemplateDeclaration(initial)TemplateContext(initial)Declaration(new name)';
xture.nativeElement.textContent).toEqual(expectedContent);
pComponent.declaration.transplantedViewRefreshCount).toEqual(0);ere that this second change detection should not be necessary, but is because of
ckwards reference not being fully supported. The assertions below should be true
the first CD.
etectChanges(false);
xture.nativeElement.textContent)
  .toEqual(
...
pComponent.declaration.transplantedViewRefreshCount).toEqual(1);
 ...
 () => {
ent.insertion.name = 'new name';
ent.declaration.changeDetectorRef.markForCheck();
etectChanges(false);
hould not update in insertion view because only declaration was marked dirty
t name also does not update in the template because the insertion view needs to be
d to update the `ngTemplateOutletContext` input.
xture.nativeElement.textContent)
  .toEqual(
  'Insertion(initial)TemplateDeclaration(initial)TemplateContext(initial)Declaration(initial)',
...
ere that if backwards references were better supported, we would be able to
h the transplanted view in the first `detectChanges` call but because the
ion point was already checked, we need to call detectChanges again to refresh it.
pComponent.declaration.transplantedViewRefreshCount).toEqual(0);etectChanges(false);
xture.nativeElement.textContent)
  .toEqual(
  'Insertion(initial)TemplateDeclaration(initial)TemplateContext(initial)Declaration(initial)',
...
value = 'initial';constructor(readonly cdr: ChangeDetectorRef) {}
    }
    @Component({
template: `
<ng-template #template>{{value}}</ng-template>
...
name = 'Penny';
 ...
 'Expected transplanted view to be refreshed even when insertion is not dirty');
    ...
    class AppComponent {
@ViewChild(TemplateRef) templateRef!: TemplateRef<{}>;constructor(readonly rootVref: ViewContainerRef, readonly cdr: ChangeDetectorRef) {}checks = 0;
incrementChecks() {
  this.checks++;
}
    }    const fixture = TestBed.configureTestingModule({declarations: [AppComponent]})
mponent(AppComponent);
    const component = fixture.componentInstance;
    fixture.detectChanges();    const viewRef = component.templateRef.createEmbeddedView({});
    ...
    expect(component.checks).toEqual(0);    // Detach view, manually call `detectChanges`, and verify the template was refreshed
...
@Input() template!: TemplateRef<{}>;createTemplate() {
...
@Input() template!: TemplateRef<{}>;createTemplate() {
  return this.viewContainer.createEmbeddedView(this.template);
    ...
    let checkAlwaysComponent: CheckAlwaysComponent;    @Component({
template: `
