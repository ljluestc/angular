...
import {withBody} from '@angular/private/testing';describe('NgModule scopes', () => {
  beforeEach(destroyPlatform);
  afterEach(destroyPlatform);  it('should apply NgModule scope to a component that extends another component class',
 ...
 // scopes are applied, preventing the issue from occurring. @Pipe({name: 'multiply'})
 class MultiplyPipe implements PipeTransform {
(value: number, factor: number): number {
return value * factor; } @Component({template: '...'})
 class BaseComponent {
 } @Component({selector: 'my-app', template: 'App - {{ 3 | multiply:2 }}'})
 class App extends BaseComponent {
 } @NgModule({
[BrowserModule],
ons: [App, BaseComponent, MultiplyPipe],
: [App],
 })
 class Mod {
