...
import {TestBed} from '@angular/core/testing';describe('environment injector', () => {
  it('should create and destroy an environment injector', () => {
    class Service {}    let destroyed = false;
    ...
    envInjector.onDestroy(() => destroyed = true);    const service = envInjector.get(Service);
    expect(service).toBeInstanceOf(Service);    envInjector.destroy();
    expect(destroyed).toBeTrue();
  });  it('should allow unregistration while destroying', () => {
    const destroyedLog: string[] = [];    const parentEnvInjector = TestBed.inject(EnvironmentInjector);
    const envInjector = createEnvironmentInjector([], parentEnvInjector);
    const destroyRef = envInjector.get(DestroyRef);    const unregister = destroyRef.onDestroy(() => {
destroyedLog.push('first');
unregister();
    });
    destroyRef.onDestroy(() => {
destroyedLog.push('second');
    });    expect(destroyedLog).toEqual([]);    envInjector.destroy();
    expect(destroyedLog).toEqual(['first', 'second']);
  });  it('should see providers from a parent EnvInjector', () => {
    class Service {}    const parentEnvInjector = TestBed.inject(EnvironmentInjector);
    const envInjector =
  createEnvironmentInjector([], createEnvironmentInjector([Service], parentEnvInjector));
    ...
    const token = new InjectionToken('token');    const parentEnvInjector = TestBed.inject(EnvironmentInjector);
  ...
  createEnvironmentInjector([{provide: token, useValue: 'parent'}], parentEnvInjector));
    ...
    class Service {}
    const parentEnvInjector = TestBed.inject(EnvironmentInjector);
    const envInjector = createEnvironmentInjector([Service], parentEnvInjector);    const ngModuleRef = envInjector.get(NgModuleRef);    expect(ngModuleRef).toBeInstanceOf(NgModuleRef);
    ...
    expect(ngModuleRef.instance).toBeNull();
     ...
     () => {
 class Service {} @Component({selector: 'test-cmp'})
 class TestComponent {
or(readonly service: Service) {}
 } const parentEnvInjector = TestBed.inject(EnvironmentInjector);
 const environmentInjector = createEnvironmentInjector([Service], parentEnvInjector);
 const cRef = createComponent(TestComponent, {environmentInjector}); expect(cRef.instance.service).toBeInstanceOf(Service);
...
multi: true,
  ...
  parentEnvInjector);    expect(initialized).toBeTrue();
...
[{provide: TOKEN, useValue: 'from injector'}], TestBed.inject(EnvironmentInjector));const result = injector.runInContext(() => inject(TOKEN));
...
class TestCmp {
  ...
  tokenFromEnvContext = this.envInjector.runInContext(() => inject(TOKEN));  // Attempt to inject ViewContainerRef within the environment injector's context. This should
...
const childInjector =
createEnvironmentInjector([{provide: TOKEN, useValue: 'from child'}], parentInjector);const results = parentInjector.runInContext(() => {
