...
import {By} from '@angular/platform-browser';describe('directives', () => {
  describe('matching', () => {
...
class TestDir {
  testValue: boolean|undefined;  /** Setter to assert that a binding is not invoked with stringified attribute value */
  @Input()
  set test(value: any) {
...
TestBed.overrideTemplate(TestComponent, `<span class="fade" [test]="false"></span>`);const fixture = TestBed.createComponent(TestComponent);
...
fixture.detectChanges();// the "test" attribute should not be reflected in the DOM as it is here only
...
}onfigureTestingModule({declarations: [TestComponent, TestDir]});
verrideTemplate(
  TestComponent,
  `<span class="fade" [prop1]="true" [test]="false" [prop2]="true"></span>`);ture = TestBed.createComponent(TestComponent);
tDir = fixture.debugElement.query(By.directive(TestDir)).injector.get(TestDir);
nEl = fixture.nativeElement.children[0];
etectChanges();est" attribute should not be reflected in the DOM as it is here only
rective matching purposes
anEl.hasAttribute('test')).toBe(false);
anEl.hasAttribute('prop1')).toBe(false);
anEl.hasAttribute('prop2')).toBe(false);
anEl.getAttribute('class')).toBe('fade');
stDir.testValue).toBe(false);
 });    it('should match directives on ng-template', () => {
TestBed.configureTestingModule({declarations: [TestComponent, TestDirective]});
TestBed.overrideTemplate(TestComponent, `<ng-template test></ng-template>`);const fixture = TestBed.createComponent(TestComponent);
const nodesWithDirective = fixture.debugElement.queryAllNodes(By.directive(TestDirective));expect(nodesWithDirective.length).toBe(1);
...
TestBed.configureTestingModule({declarations: [TestComponent, TestDirective]});
TestBed.overrideTemplate(TestComponent, `<div *test></div>`);const fixture = TestBed.createComponent(TestComponent);
const nodesWithDirective = fixture.debugElement.queryAllNodes(By.directive(TestDirective));expect(nodesWithDirective.length).toBe(1);
...
const directiveA = fixture.debugElement.query(By.css('span')).injector.get(DirectiveA);expect(directiveA.viewContainerRef).toBeTruthy();
...
const nodesWithDirective = fixture.debugElement.queryAllNodes(By.directive(TitleDirective));expect(nodesWithDirective.length).toBe(1);
...
fixture.debugElement.queryAllNodes(By.directive(TitleClassDirective));expect(nodesWithDirective.length).toBe(1);
    });    it('should NOT match classes to directive selectors', () => {
TestBed.configureTestingModule({declarations: [TestComponent, TitleDirective]});
TestBed.overrideTemplate(TestComponent, `
  <div class="title" [id]="someId"></div>
`);const fixture = TestBed.createComponent(TestComponent);
const nodesWithDirective = fixture.debugElement.queryAllNodes(By.directive(TitleDirective));expect(nodesWithDirective.length).toBe(0);
...
`);const fixture = TestBed.createComponent(TestComponent);
const nodesWithDirective =
fixture.debugElement.queryAllNodes(By.directive(TitleAttributeDirective));expect(nodesWithDirective.length).toBe(1);
    });    it('should match directives with attribute selectors on outputs', () => {
@Directive({selector: '[out]'})
class TestDir {
  @Output() out = new EventEmitter();
}TestBed.configureTestingModule({declarations: [TestComponent, TestDir]});
TestBed.overrideTemplate(TestComponent, `<span class="span" (out)="someVar = true"></span>`);const fixture = TestBed.createComponent(TestComponent);
const spanEl = fixture.nativeElement.children[0];// "out" should not be part of reflected attributes
...
const calls: string[] = [];@Directive({selector: '[dir]'})
class MyDir {
  ngOnInit() {
...
fixture.detectChanges();// Expect only one directive to be instantiated.
expect(calls).toEqual(['MyDir.ngOnInit']);
    });    it('should match directives on elements with namespace', () => {
const calls: string[] = [];@Directive({selector: 'svg[dir]'})
class MyDir {
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();expect(calls).toEqual(['MyDir.ngOnInit: svg']);
    });    it('should match directives on descendant elements with namespace', () => {
const calls: string[] = [];@Directive({selector: 'text[dir]'})
class MyDir {
  constructor(private el: ElementRef) {}
  ngOnInit() {
calls.push(`MyDir.ngOnInit: ${this.el.nativeElement.tagName}`);
...
const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();expect(calls).toEqual(['MyDir.ngOnInit: text']);
...
logs.push('MyDir.constructor');
  }  @Input('test') myInput = '';  @Input('disabled') myInput2 = '';
}@Component({
...
class MyComp {
  test = '';
}TestBed.configureTestingModule({declarations: [MyComp, MyDir]});const fixture = TestBed.createComponent(MyComp);
fixture.detectChanges();expect(logs).toEqual(['MyDir.constructor']);
    });
  });  describe('inputs', () => {
...
fixture.detectChanges();expect(dirInstance!.dir).toBe('Hello');
...
@Input() dir: string = '';t({
selector: 'my-app',
template: '<ng-template *ngIf="true" dir="{{ message }}"></ng-template>',tComp {
message = 'Hello';onfigureTestingModule({declarations: [TestComp, WithInput]});
ture = TestBed.createComponent(TestComp);
etectChanges();rInstance!.dir).toBe('Hello');
 });    it('should not set structural directive inputs from static element attrs', () => {
const dirInstances: StructuralDir[] = [];@Directive({selector: '[dir]'})
class StructuralDir {
  constructor() {
...
<div *dir="let item of items" dirUnboundInput>Some content</div><!-- De-sugared version of the same structural directive -->
...
fixture.detectChanges();const [regularDir, desugaredDir] = dirInstances;// When directive is used as a structural one, the `dirUnboundInput` should not be treated as
// an input.
expect(regularDir.dirUnboundInput).toBe(undefined);// In de-sugared version the `dirUnboundInput` acts as a regular input, so it should be set
// to an empty string.
...
dirInstances.push(this);
  ...
  template: `
<!-- Regular form of structural directive -->
<div *dir="let item of items" [title]="title">Some content</div><!-- De-sugared version of the same structural directive -->
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();const [regularDir, desugaredDir] = dirInstances;// When directive is used as a structural one, the `title` should not be treated as an input.
expect(regularDir.title).toBe(undefined);// In de-sugared version the `title` acts as a regular input, so it should be set.
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();
const {dirInstance, plainValue, aliasedValue} = fixture.componentInstance;expect(dirInstance.plainInput).toBe(plainValue);
...
aliasedInput: number|undefined;t({template: '<div dir [plainInput]="plainValue" [alias]="aliasedValue"></div>'})
 {
@ViewChild(Dir) dirInstance!: Dir;
plainValue = 123;
aliasedValue = 321;onfigureTestingModule({declarations: [App, Dir]});
ture = TestBed.createComponent(App);
etectChanges();
rInstance, plainValue, aliasedValue} = fixture.componentInstance;rInstance.plainInput).toBe(plainValue);
rInstance.aliasedInput).toBe(aliasedValue);
...
const fixture = TestBed.createComponent(TestComp);
fixture.detectChanges();expect(fixture.componentInstance.dir.value).toBe(0);fixture.componentInstance.assignedValue = 'hello';
fixture.detectChanges();expect(fixture.componentInstance.dir.value).toBe(1);
...
const fixture = TestBed.createComponent(TestComp);
fixture.detectChanges();expect(fixture.componentInstance.hostDir.value).toBe(0);fixture.componentInstance.assignedValue = 'hello';
fixture.detectChanges();expect(fixture.componentInstance.hostDir.value).toBe(1);
    });    it('should use the transformed input values in ngOnChanges', () => {
const trackedChanges: SimpleChange[] = [];@Directive({selector: '[dir]'})
class Dir implements OnChanges {
  @Input({transform: (value: string) => value ? 1 : 0}) value = -1;  ngOnChanges(changes: SimpleChanges): void {
if (changes['value']) {
...
const fixture = TestBed.createComponent(TestComp);
fixture.detectChanges();expect(trackedChanges).toEqual([jasmine.objectContaining(
{previousValue: undefined, currentValue: 0})]);fixture.componentInstance.assignedValue = 'hello';
fixture.detectChanges();expect(trackedChanges).toEqual([
...
let instance: Dir|undefined;function transform(this: Dir, _value: string) {
  instance = this;
  return 0;
}@Directive({selector: '[dir]'})
class Dir {
...
const fixture = TestBed.createComponent(TestComp);
fixture.detectChanges();expect(instance).toBe(fixture.componentInstance.dir);
...
const fixture = TestBed.createComponent(TestComp);
fixture.detectChanges();const ref = fixture.componentInstance.vcr.createComponent(Comp);ref.setInput('value', '');
expect(ref.instance.value).toBe(0);ref.setInput('value', 'hello');
...
const fixture = TestBed.createComponent(TestComp);
fixture.detectChanges();expect(fixture.componentInstance.testDir).toBeTruthy();
expect(fixture.componentInstance.value).toBe(false);fixture.componentInstance.testDir!.out.emit();
fixture.detectChanges();
...
const fixture = TestBed.createComponent(TestComp);
const testDir = fixture.debugElement.query(By.css('span')).injector.get(TestDir);expect(fixture.componentInstance.value).toBe(false);testDir.out.emit();
fixture.detectChanges();
...
const fixture = TestBed.createComponent(App);
fixture.detectChanges();const dirWithTitle =
...
expect(div.getAttribute('title')).toBe('a');
 ...
 () => {
t({template: `<div dir-with-title [title]="value"></div>`})
 {
value = 'a';onfigureTestingModule({
declarations: [App, DirWithTitle],ture = TestBed.createComponent(App);
etectChanges();WithTitle =
  fixture.debugElement.query(By.directive(DirWithTitle)).injector.get(DirWithTitle);
 = fixture.nativeElement.querySelector('div');
 checking the property here, not the attribute, because in the case of
"value" we are always setting the property of the instance, and actually setting
tribute is just a side-effect of the DOM implementation.
rWithTitle.title).toBe('a');
v.title).toBe('');
 });    it('should allow setting directive `title` input with `[title]="value"` and a "attr.title" attribute with `attr.title="test"`',
 () => {
t({template: `<div dir-with-title [title]="value" attr.title="test"></div>`})
 {
value = 'a';onfigureTestingModule({
declarations: [App, DirWithTitle],ture = TestBed.createComponent(App);
etectChanges();WithTitle =
 ...
 = fixture.nativeElement.querySelector('div');
rWithTitle.title).toBe('a');
v.getAttribute('attr.title')).toBe('test');
v.title).toBe('');
 });    it('should allow setting directive `title` input with `[title]="value1"` and attribute with `[attr.title]="value2"`',
 () => {
t({template: `<div dir-with-title [title]="value1" [attr.title]="value2"></div>`})
 {
value1 = 'a';
value2 = 'b';onfigureTestingModule({
declarations: [App, DirWithTitle],ture = TestBed.createComponent(App);
etectChanges();WithTitle =
  fixture.debugElement.query(By.directive(DirWithTitle)).injector.get(DirWithTitle);
 = fixture.nativeElement.querySelector('div');
rWithTitle.title).toBe('a');
v.getAttribute('title')).toBe('b');
 });    it('should allow setting directive `title` input with `[title]="value1"` and attribute with `attr.title="{{value2}}"`',
 () => {
t(
...
v.title).toBe('');
 ...
 () => {
t({template: `<div dir-with-title attr.title="{{value}}"></div>`})
 {
value = 'a';onfigureTestingModule({
declarations: [App, DirWithTitle],ture = TestBed.createComponent(App);
etectChanges();WithTitle =
  fixture.debugElement.query(By.directive(DirWithTitle)).injector.get(DirWithTitle);
 = fixture.nativeElement.querySelector('div');
rWithTitle.title).toBe('');
v.getAttribute('title')).toBe('a');
 });    it('should set the title attribute only, not directive input, for `[attr.title]="value"`',
 () => {
t({template: `<div dir-with-title [attr.title]="value"></div>`})
 {
