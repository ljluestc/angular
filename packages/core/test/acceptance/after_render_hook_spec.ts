...
import {TestBed} from '@angular/core/testing';describe('after render hooks', () => {
  describe('browser', () => {
...
it('should run with the correct timing', () => {
  ...
  class DynamicComp {
afterRenderCount = 0;constructor() {
 afterRender(() => {
...
viewContainerRef = inject(ViewContainerRef);constructor() {
 afterRender(() => {
   this.afterRenderCount++;
 });
}
...
...COMMON_CONFIGURATION,
  ...
  const dynamicCompRef = viewContainerRef.createComponent(DynamicComp);  // It hasn't run at all
  expect(dynamicCompRef.instance.afterRenderCount).toBe(0);
  expect(compInstance.afterRenderCount).toBe(0);  // Running change detection at the dynamicCompRef level
  dynamicCompRef.changeDetectorRef.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(1);
  expect(compInstance.afterRenderCount).toBe(1);  // Running change detection at the compInstance level
  compInstance.changeDetectorRef.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(2);
  expect(compInstance.afterRenderCount).toBe(2);  // Running change detection at the fixture level (first time)
  fixture.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(3);
  expect(compInstance.afterRenderCount).toBe(3);  // Running change detection at the fixture level (second time)
  fixture.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(4);
  expect(compInstance.afterRenderCount).toBe(4);  // Running change detection at the fixture level (third time)
  fixture.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(5);
  expect(compInstance.afterRenderCount).toBe(5);  // Running change detection after removing view.
  viewContainerRef.remove();
  fixture.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(5);
  expect(compInstance.afterRenderCount).toBe(6);
});it('should run all hooks after outer change detection', () => {
  let log: string[] = [];  @Component({selector: 'child-comp'})
  class ChildComp {
constructor() {
 afterRender(() => {
   log.push('child-comp');
...
template: `<child-comp></child-comp>`,
  ...
  class ParentComp {
changeDetectorRef = inject(ChangeDetectorRef);constructor() {
 afterRender(() => {
...
...COMMON_CONFIGURATION,
  ...
  let afterRenderCount = 0;  @Component({selector: 'comp'})
  class Comp {
constructor() {
 hookRef = afterRender(() => {
   afterRenderCount++;
...
...COMMON_CONFIGURATION,
  ...
  const fixture = TestBed.createComponent(Comp);
  expect(afterRenderCount).toBe(0);  fixture.detectChanges();
  expect(afterRenderCount).toBe(1);  fixture.detectChanges();
  expect(afterRenderCount).toBe(2);
  hookRef!.destroy();  fixture.detectChanges();
  expect(afterRenderCount).toBe(2);
...
...COMMON_CONFIGURATION,
 ...
 .toThrowError(/A new render operation began before the previous operation ended./);
  ...
  let innerHookCount = 0;  @Component({selector: 'comp'})
  class Comp {
injector = inject(Injector);constructor() {
 afterRender(() => {
   outerHookCount++;
   afterNextRender(() => {
 innerHookCount++;
...
...COMMON_CONFIGURATION,
  ...
  const fixture = TestBed.createComponent(Comp);  // It hasn't run at all
  expect(outerHookCount).toBe(0);
  expect(innerHookCount).toBe(0);  // Running change detection (first time)
  fixture.detectChanges();
  expect(outerHookCount).toBe(1);
  expect(innerHookCount).toBe(0);  // Running change detection (second time)
  fixture.detectChanges();
  expect(outerHookCount).toBe(2);
  expect(innerHookCount).toBe(1);  // Running change detection (third time)
  fixture.detectChanges();
  expect(outerHookCount).toBe(3);
  expect(innerHookCount).toBe(2);
});it('should run outside of the Angular zone', () => {
  const zoneLog: boolean[] = [];  @Component({selector: 'comp'})
  class Comp {
constructor() {
 afterRender(() => {
...
...COMMON_CONFIGURATION,
  ...
  const fixture = TestBed.createComponent(Comp);  expect(zoneLog).toEqual([]);
  fixture.detectChanges();
  expect(zoneLog).toEqual([false]);
});it('should propagate errors to the ErrorHandler', () => {
  const log: string[] = [];  class FakeErrorHandler extends ErrorHandler {
override handleError(error: any): void {
...
...COMMON_CONFIGURATION,
  ...
  const fixture = TestBed.createComponent(Comp);  expect(log).toEqual([]);
  fixture.detectChanges();
...
...COMMON_CONFIGURATION,
  ...
  compInstance.changeDetectorRef.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(1);
  expect(compInstance.afterRenderCount).toBe(1);  // Running change detection at the fixture level (first time)
  fixture.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(1);
  expect(compInstance.afterRenderCount).toBe(1);  // Running change detection at the fixture level (second time)
  fixture.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(1);
  expect(compInstance.afterRenderCount).toBe(1);  // Running change detection at the fixture level (third time)
  fixture.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(1);
  expect(compInstance.afterRenderCount).toBe(1);  // Running change detection after removing view.
  viewContainerRef.remove();
  fixture.detectChanges();
  expect(dynamicCompRef.instance.afterRenderCount).toBe(1);
...
...COMMON_CONFIGURATION,
  ...
  const fixture = TestBed.createComponent(Comp);
  expect(afterRenderCount).toBe(0);  hookRef!.destroy();
  fixture.detectChanges();
...
injector = inject(Injector);constructor() {
 afterNextRender(() => {
   outerHookCount++;   afterNextRender(() => {
 innerHookCount++;
   }, {injector: this.injector});
 });
}
