...
import {TestBed} from '@angular/core/testing';import {internalImportProvidersFrom} from '../../src/di/provider_collection';describe('environment injector and standalone components', () => {
  it('should see providers from modules imported by standalone components', () => {
    class ModuleService {}    @NgModule({providers: [ModuleService]})
    class Module {
  ...
  createEnvironmentInjector([importProvidersFrom(AppModule)], parentEnvInjector);
    ...
    class ModuleService {}    @NgModule({providers: [{provide: ModuleService, useClass: ModuleService, multi: true}]})
    class Module {
    }    @Component({standalone: true, imports: [Module]})
...
exports: [StandaloneComponent1, StandaloneComponent2]
    ...
    const envInjector =
  createEnvironmentInjector([importProvidersFrom(AppModule)], parentEnvInjector);
    const services = envInjector.get(ModuleService) as ModuleService[];    expect(services.length).toBe(1);
  });  it('should support nested arrays of providers', () => {
    const A = new InjectionToken('A');
    const B = new InjectionToken('B');
    const C = new InjectionToken('C');
    const MULTI = new InjectionToken('D');    const providers = [
