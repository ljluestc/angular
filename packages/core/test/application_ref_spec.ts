...
import type {ServerModule} from '@angular/platform-server';import {ApplicationRef} from '../src/application_ref';
import {NoopNgZone} from '../src/zone/ng_zone';
import {ComponentFixtureNoNgZone, inject, TestBed, waitForAsync, withModule} from '../testing';let serverPlatformModule: Promise<Type<ServerModule>>|null = null;
if (isNode) {
...
class SomeComponent {
  ...
  describe('bootstrap', () => {
    let mockConsole: MockConsole;    beforeEach(() => {
...
doc.body.appendChild(rootEl);
  ...
  {providers?: any[], ngDoBootstrap?: any, bootstrap?: any[], component?: Type<any>};    async function createModule(providers?: any[]): Promise<Type<any>>;
...
(errorHandler as any)._console = mockConsole as any;const platformModule =
getDOM().supportsDOMEvents ? BrowserModule : await serverPlatformModule!;@NgModule({
...
template: '',
  ...
  class SomeComponent {
  }  const helloToken = new InjectionToken<string>('hello');  @NgModule({
...
declarations: [SomeComponent],
  ...
  const component = app.bootstrap(cmpFactory);  // The component should see the child module providers
...
declarations: [SomeComponent],
  ...
  const cmpFactory =
  module.componentFactoryResolver.resolveComponentFactory(SomeComponent);
  const component = app.bootstrap(cmpFactory, 'custom-selector');  // The component should see the child module providers
  expect(component.injector.get(helloToken)).toEqual('component');
...
reenterCount = 1;
reenterErr: any;constructor(private appRef: ApplicationRef) {}reenter() {
...
function createApplicationRefInjector(parentInjector: EnvironmentInjector) {
  const extraProviders = [{provide: ApplicationRef, useClass: ApplicationRef}];  return createEnvironmentInjector(extraProviders, parentInjector);
...
e(
  ...
  (parentInjector: EnvironmentInjector, doc: Document) => {
    createRootEl();    const appRef = createApplicationRef(parentInjector);
    appRef.bootstrap(SomeComponent);    // The component template content (`hello`) is present in the document body.
    expect(doc.body.textContent!.indexOf('hello') > -1).toBeTrue();    appRef.destroy();    // The component template content (`hello`) is *not* present in the document
...
appRef.destroy();expect(() => appRef.destroy())
    .toThrowError(
...
createRootEl();const appRef = createApplicationRef(parentInjector) as unknown as ApplicationRef &
    {onDestroy: Function};
appRef.bootstrap(SomeComponent);
appRef.onDestroy(onDestroyA);
appRef.onDestroy(onDestroyB);
appRef.destroy();expect(onDestroyA).toHaveBeenCalledTimes(1);
...
createRootEl();const appRef = createApplicationRef(parentInjector) as unknown as ApplicationRef &
    {onDestroy: Function};
appRef.bootstrap(SomeComponent);const onDestroyA = jasmine.createSpy('onDestroyA');
const onDestroyB = jasmine.createSpy('onDestroyB');
const unsubscribeOnDestroyA = appRef.onDestroy(onDestroyA);
const unsubscribeOnDestroyB = appRef.onDestroy(onDestroyB);// Unsubscribe registered listeners.
unsubscribeOnDestroyA();
unsubscribeOnDestroyB();appRef.destroy();expect(onDestroyA).not.toHaveBeenCalled();
...
e(
  ...
  waitForAsync(inject([EnvironmentInjector], (parentInjector: EnvironmentInjector) => {
createRootEl();const appRef = createApplicationRef(parentInjector);
appRef.bootstrap(SomeComponent);expect(appRef.destroyed).toBeFalse();appRef.destroy();expect(appRef.destroyed).toBeTrue();
...
// type is available.
type DestroyableInjector = EnvironmentInjector&{destroyed?: boolean};createRootEl();const injector = createApplicationRefInjector(parentInjector) as DestroyableInjector;const appRef = injector.get(ApplicationRef);
appRef.bootstrap(SomeComponent);expect(appRef.destroyed).toBeFalse();
expect(injector.destroyed).toBeFalse();appRef.destroy();expect(appRef.destroyed).toBeTrue();
...
defaultPlatform.bootstrapModule(await createModule({bootstrap: [SomeComponent]}))
.then(module => expect((<any>defaultPlatform)._modules).toContain(module));it('should bootstrap with NoopNgZone', waitForAsync(async () => {
defaultPlatform
...
templateUrl: '/test-template.html',
  ...
  const testModule = await createModule({component: WithTemplateUrlComponent});  await defaultPlatform.bootstrapModule(testModule, {
providers: [
...
templateUrl: '',
 ...
 {component: I18nComponent, providers: [{provide: LOCALE_ID, useValue: 'ro'}]});
  await defaultPlatform.bootstrapModule(testModule);  expect(getLocaleId()).toEqual('ro');
});it('should wait for APP_INITIALIZER to set providers for `LOCALE_ID`', async () => {
  let locale: string = '';  const testModule = await createModule({
providers: [
...
setTimeout(() => {
  resolve(true);
  initializerDone = true;
}, 1);const compilerFactory: CompilerFactory =
...
// Error rethrown will be seen by the exception handler since it's after
// construction.
expect(mockConsole.res[0].join('#')).toEqual('ERROR#Test');it('should rethrow promise errors even if the exceptionHandler is not rethrowing',
ync(async () => {
const compilerFactory: CompilerFactory =
defaultPlatform.injector.get(CompilerFactory, null)!;
const moduleFactory =
...
providers: [{provide: ComponentFixtureNoNgZone, useValue: true}]
  ...
  expect(appRef.viewCount).toBe(0);  appRef.tick();
  expect(comp.nativeElement).toHaveText('');  appRef.attachView(comp.componentRef.hostView);
  appRef.tick();
  expect(appRef.viewCount).toBe(1);
  expect(comp.nativeElement).toHaveText('Initial');
});it('should not dirty check detached views', () => {
  const comp = TestBed.createComponent(MyComp);
  const appRef: ApplicationRef = TestBed.inject(ApplicationRef);  appRef.attachView(comp.componentRef.hostView);
  appRef.tick();
  expect(comp.nativeElement).toHaveText('Initial');  appRef.detachView(comp.componentRef.hostView);
  ...
  const comp = TestBed.createComponent(MyComp);
  const appRef: ApplicationRef = TestBed.inject(ApplicationRef);  appRef.attachView(comp.componentRef.hostView);
  comp.destroy();  expect(appRef.viewCount).toBe(0);
});it('should detach attached embedded views if they are destroyed', () => {
  const comp = TestBed.createComponent(EmbeddedViewComp);
  const appRef: ApplicationRef = TestBed.inject(ApplicationRef);  const embeddedViewRef = comp.componentInstance.tplRef.createEmbeddedView({});  appRef.attachView(embeddedViewRef);
  embeddedViewRef.destroy();  expect(appRef.viewCount).toBe(0);
});it('should not allow to attach a view to both, a view container and the ApplicationRef',const comp = TestBed.createComponent(MyComp);
...
const appRef: ApplicationRef = TestBed.inject(ApplicationRef);vc.insert(hostView);
expect(() => appRef.attachView(hostView))
.toThrowError('NG0902: This view is already attached to a ViewContainer!');
hostView = vc.detach(0)!;appRef.attachView(hostView);
...
Promise.resolve(null).then((_: any) => {
 this.text += '1';
    ...
    }    let stableCalled = false;    beforeEach(() => {
stableCalled = false;
TestBed.configureTestingModule({
  declarations: [
...
const zone: NgZone = TestBed.inject(NgZone);
appRef.attachView(fixture.componentRef.hostView);
zone.run(() => appRef.tick());fixture.whenStable().then(() => {
...
const appRef = TestBed.inject(ApplicationRef);expect(appRef.injector).toBe(fixture.componentInstance.envInjector);
    });
  });
}class MockConsole {
  res: any[][] = [];
