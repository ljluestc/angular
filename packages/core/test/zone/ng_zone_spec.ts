...
import {Log} from '@angular/core/testing/src/testing_internal';import {global} from '../../src/util/global';
import {getNativeRequestAnimationFrame} from '../../src/util/raf';
import {NoopNgZone} from '../../src/zone/ng_zone';const resultTimer = 1000;
...
let _errors: any[];
let _traces: any[];
let _zone: NgZone;const resolvedPromise = Promise.resolve(null);function logOnError() {
  ...
  describe('NgZone', () => {
    function createZone(enableLongStackTrace: boolean) {
      ...
      beforeEach(() => {
        ...
        logOnStable();
        logOnError();
      });      commonTests();      it('should produce long stack traces', done => {
        macroTask(() => {
...
const promise: Promise<any> = new Promise((res) => {
 ...
 expect(_traces[0].length).toBeGreaterThan(1);
      ...
      beforeEach(() => {
        ...
        logOnStable();
        logOnError();
      });      commonTests();      it('should disable long stack traces', done => {
        macroTask(() => {
let resolve: (result: any) => void;
const promise: Promise<any> = new Promise((res) => {
 resolve = res;
 ...
 if (_traces[0] != null) {
   ...
   expect(_traces[0].indexOf('---')).toEqual(-1);
    ...
    const ngZone = new NoopNgZone();    it('should run', () => {
      let runs = false;
      ngZone.run(() => {
        ngZone.runGuarded(() => {
 ...
 runs = true;
  ...
  describe('hasPendingMicrotasks', () => {
    ...
    it('should return the body return value from run', done => {
      ...
      // then verified that onStable is only called once at the end      runNgZoneNoLog(() => macroTask(_log.fn('run')));      let times = 0;
...
if (times < 2) {
 ...
 'onMicrotaskEmpty; onMicrotaskEmpty 2; onStable');
      ...
      runNgZoneNoLog(() => macroTask(_log.fn('run')));      macroTask(() => {
        expect(_log.result()).toEqual('onUnstable; run; onMicrotaskEmpty; onStable');
        done();
      }, resultTimer);
      ...
      // then verifies that those microtasks do not cause additional digests.      let turnStart = false;
...
eventDone = true;
      ...
      runNgZoneNoLog(() => macroTask(_log.fn('run')));      // the only practical use-case to run a callback inside the zone is
...
turnDone = true;
 ...
 'onMicrotaskEmpty; onMyMicrotaskEmpty; onStable');
      ...
      runNgZoneNoLog(() => macroTask(_log.fn('run')));      _zone.onStable.subscribe({
        next: () => {
NgZone.assertNotInAngularZone();
...
NoLog(() => macroTask(_log.fn('start run')));icrotaskEmpty.subscribe({
next: () => {
...
Timer);
      ...
      runNgZoneNoLog(() => macroTask(_log.fn('run2')));      macroTask(() => {
        expect(_log.result())
 .toEqual(
 'onUnstable; run1; onMicrotaskEmpty; onStable; onUnstable; run2; onMicrotaskEmpty; onStable');
      ...
      let bPromise: Promise<string>;      runNgZoneNoLog(() => {
        macroTask(() => {
...
ve: (result: string|null) => void;
se: Promise<string|null>;(() => {
NgZone.assertNotInAngularZone();
...
NoLog(() => macroTask(_log.fn('run'))); false;
icrotaskEmpty.subscribe({
next: () => {
  _log.add('onMicrotaskEmpty(begin)');  if (!ran) {
_zone.run(() => {
  queueMicrotask(() => {
    ran = true;
...
_log.add('run end');
      ...
      let startPromiseRan = false;      _zone.onUnstable.subscribe({
        next: () => {
...
lve: (result: string|null) => void;
ise: Promise<string|null>;
lve: (result: string|null) => void;
ise: Promise<string|null>;NoLog(() => {
macroTask(() => {
...
Timer);
    ...
    it('should call the on error callback when it is invoked via zone.runGuarded', done => {
      macroTask(() => {
        const exception = new Error('sync');        _zone.runGuarded(() => {
throw exception;
...
expect(() => _zone.run(() => {
  throw exception;
})).toThrowError('sync');expect(_errors.length).toBe(0);
done();       });    it('should call onError for errors from microtasks', done => {
      const exception = new Error('async');      macroTask(() => {
        _zone.run(() => {
queueMicrotask(() => {
 throw exception;
});
    ...
    describe('#10503', () => {
      let ngZone: NgZone;      beforeEach(inject([NgZone], (_ngZone: NgZone) => {
...
expect(result).toEqual('works');      describe('async', () => {
        let asyncResult: string;
        const waitLongerThenTestFrameworkAsyncTimeout = 5;        beforeEach(() => {
asyncResult = null!;
...
expect(logs).toEqual(['microTask empty', 'myEvent', 'microTask empty']);
      ...
      });      it('should only emit onMicroTaskEmpty once within the same event loop for multiple event tasks',const tasks: Task[] = [];
coalesceZone.run(() => {
  tasks.push(Zone.current.scheduleEventTask('myEvent', () => {
logs.push('eventTask1');
  }, undefined, () => {}));
...
});      it('should only emit onMicroTaskEmpty once within the same event loop for ngZone.run in onMicrotaskEmpty subscription',const tasks: Task[] = [];
coalesceZone.onMicrotaskEmpty.subscribe(() => {
  coalesceZone.run(() => {});
});
coalesceZone.run(() => {
...
}, 100);      it('should emit onMicroTaskEmpty once within the same event loop for not only event tasks, but event tasks are before other tasks',const tasks: Task[] = [];
coalesceZone.run(() => {
  tasks.push(Zone.current.scheduleEventTask('myEvent', () => {
logs.push('eventTask1');
  }, undefined, () => {}));
...
});      it('should emit multiple onMicroTaskEmpty within the same event loop for not only event tasks, but event tasks are after other tasks',const tasks: Task[] = [];
coalesceZone.run(() => {
  tasks.push(Zone.current.scheduleMacroTask('myMacro', () => {
logs.push('macroTask');
  }, undefined, () => {}));
...
expect(logs).toEqual(['microTask empty']);
      ...
      });      it('should only emit onMicroTaskEmpty once within the same event loop for multiple ngZone.run',coalesceZone.run(() => {});
coalesceZone.run(() => {});
expect(logs).toEqual([]);
nativeRequestAnimationFrame(() => {
  expect(logs).toEqual(['microTask empty']);
  done();
});      it('should only emit onMicroTaskEmpty once within the same event loop for ngZone.run in onMicrotaskEmpty subscription',coalesceZone.onMicrotaskEmpty.subscribe(() => {
  coalesceZone.run(() => {});
});
coalesceZone.run(() => {});
coalesceZone.run(() => {});
...
}, 100);      it('should only emit onMicroTaskEmpty once within the same event loop for multiple tasks',const tasks: Task[] = [];
coalesceZone.run(() => {
  tasks.push(Zone.current.scheduleMacroTask('myMacro', () => {
logs.push('macroTask');
  }, undefined, () => {}));
