 ...
 */import {Component, Directive, NgModule, Pipe, Type, ɵReflectionCapabilities as ReflectionCapabilities} from '@angular/core';import {MetadataOverride} from './metadata_override';
import {MetadataOverrider} from './metadata_overrider';const reflection = new ReflectionCapabilities();/**
...
abstract class OverrideResolver<T> implements Resolver<T> {
  ...
  private resolved = new Map<Type<any>, T|null>();  abstract get type(): any;  addOverride(type: Type<any>, override: MetadataOverride<T>) {
...
if (isKnownType) {
  return annotation instanceof this.type ? annotation as unknown as T : null;
    ...
    let resolved: T|null = this.resolved.get(type) || null;    if (!resolved) {
