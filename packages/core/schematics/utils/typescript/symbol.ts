 ...
 */import ts from 'typescript';export function getValueSymbolOfDeclaration(node: ts.Node, typeChecker: ts.TypeChecker): ts.Symbol|
    undefined {
  let symbol = typeChecker.getSymbolAtLocation(node);  while (symbol && symbol.flags & ts.SymbolFlags.Alias) {
...
export function isNullableType(typeChecker: ts.TypeChecker, node: ts.Node) {
  ...
  let hasSeenNullableType = false;  // Trace the type of the node back to a type node, walk
