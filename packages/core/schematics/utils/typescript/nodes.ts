 ...
 */import ts from 'typescript';/** Checks whether the given TypeScript node has the specified modifier set. */
...
export function closestNode<T extends ts.Node>(node: ts.Node, predicate: (n: ts.Node) => n is T): T|
    null {
  let current = node.parent;  while (current && !ts.isSourceFile(current)) {
