 ...
 */import ts from 'typescript';import {ImportManager} from './import_manager';/** Function that can be used to remap a generated import. */
export type ImportRemapper = (moduleName: string, inFile: string) => string;/** Mapping between a source file and the changes that have to be applied to it. */
export type ChangesByFile = ReadonlyMap<ts.SourceFile, PendingChange[]>;/** Change that needs to be applied to a file. */
...
export class ChangeTracker {
  ...
  private readonly _importManager: ImportManager;  constructor(private _printer: ts.Printer, private _importRemapper?: ImportRemapper) {
...
moduleName = this._importRemapper(moduleName, sourceFile.fileName);
    ...
    moduleName = normalizePath(moduleName);    return this._importManager.addImportToSourceFile(sourceFile, symbolName, moduleName, alias);
  }  /**
  ...
  private _trackChange(file: ts.SourceFile, change: PendingChange): void {
    const changes = this._changes.get(file);    if (changes) {
...
const insertIndex = changes.findIndex(current => current.start <= change.start);if (insertIndex === -1) {
  changes.push(change);
} else {
