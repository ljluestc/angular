...
import ts from 'typescript';/** Update recorder for managing imports. */
...
alias: string|null = null, typeImport = false): ts.Expression {
    ...
    let existingImport: ts.ImportDeclaration|null = null;    // In case the given import has been already generated previously, we just return
...
const statement = sourceFile.statements[i];if (!ts.isImportDeclaration(statement) || !ts.isStringLiteral(statement.moduleSpecifier) ||
!statement.importClause) {
  continue;
}if (importStartIndex === 0) {
  importStartIndex = this._getEndPositionOfNode(statement);
}const moduleSpecifier = statement.moduleSpecifier.text;if (moduleSpecifier.startsWith('.') &&
   resolve(sourceDir, moduleSpecifier) !== resolve(sourceDir, moduleName) ||
moduleSpecifier !== moduleName) {
  continue;
}if (statement.importClause.namedBindings) {
  const namedBindings = statement.importClause.namedBindings;  // In case a "Type" symbol is imported, we can't use namespace imports
...
const {propertyName, name} = this._getImportParts(sourceFile, symbolName!, alias);// Since it can happen that multiple classes need to be imported within the
...
existingImport,
(this.updatedImports.get(existingImport) || []).concat({propertyName, importName: name}));// Keep track of all updated imports so that we don't generate duplicate
// similar imports as these can't be statically analyzed in the source-file yet.
this.importCache.push({sourceFile, moduleName, symbolName, alias, identifier: name});return name;
    }    let identifier: ts.Identifier|null = null;    if (!this.newImports.has(sourceFile)) {
...
identifier = name;if (!importMap.has(moduleName)) {
...
importMap.set(moduleName, identifier);
    ...
    // similar imports as these can't be statically analyzed in the source-file yet.
    this.importCache.push({sourceFile, symbolName, moduleName, alias, identifier});    return identifier;
  }  /**
...
namedBindings.elements.concat(expressions.map(
   ...
   ts.factory.createImportSpecifier(false, propertyName, importName))));const newNamedBindingsText =
...
const recorder = this.getUpdateRecorder(sourceFile);
const useSingleQuotes = this._getQuoteStyle(sourceFile) === QuoteStyle.Single;defaultImports.forEach((identifier, moduleName) => {
  const newImport = ts.factory.createImportDeclaration(
 undefined, ts.factory.createImportClause(false, identifier, undefined),
 ts.factory.createStringLiteral(moduleName, useSingleQuotes));  recorder.addNewImport(
...
name = `${baseName}_${counter++}`;
    } while (!this.isUniqueIdentifierName(sourceFile, name));    this._recordUsedIdentifier(sourceFile, name!);
...
sourceFile: ts.SourceFile): string {
    const text = this.printer.printNode(ts.EmitHint.Unspecified, newImport, sourceFile);    // If the import is generated at the start of the source file, we want to add
  ...
  private _getImportParts(sourceFile: ts.SourceFile, symbolName: string, alias: string|null) {
    ...
    let name: ts.Identifier;    if (needsGeneratedUniqueName) {
...
let quoteStyle: QuoteStyle|undefined;// Walk through the top-level imports and try to infer the quotes.
