...
import ts from 'typescript';import {extractAngularClassMetadata} from './extract_metadata';
import {computeLineStartsMap, getLineAndCharacterFromPosition} from './line_mappings';
import {getPropertyNameText} from './typescript/property_name';export interface ResolvedTemplate {
...
export class NgComponentTemplateVisitor {
  resolvedTemplates: ResolvedTemplate[] = [];  constructor(public typeChecker: ts.TypeChecker, private _basePath: string, private _tree: Tree) {}  visitNode(node: ts.Node) {
...
return;
    ...
    const sourceFileName = sourceFile.fileName;    // Walk through all component metadata properties and determine the referenced
    // HTML templates (either external or inline)
    metadata.node.properties.forEach(property => {
if (!ts.isPropertyAssignment(property)) {
  return;
}const propertyName = getPropertyNameText(property.name);// In case there is an inline template specified, ensure that the value is statically
...
if (propertyName === 'templateUrl' && ts.isStringLiteralLike(property.initializer)) {
  ...
  const templateDevkitPath = relative(this._basePath, templateDiskPath);  // In case the template does not exist in the file system, skip this
  // external template.
  if (!this._tree.exists(templateDevkitPath)) {
return;
  }  const fileContent = this._tree.read(templateDevkitPath)!.toString();
  const lineStartsMap = computeLineStartsMap(fileContent);  this.resolvedTemplates.push({
