...
import {Configuration, Linter} from 'tslint';describe('Google3 waitForAsync TSLint rule', () => {
  const rulesDirectory =
dirname(runfiles.resolvePackageRelative('../../migrations/google3/waitForAsyncCjsRule.js'));  let tmpDir: string;  beforeEach(() => {
    tmpDir = join(process.env['TEST_TMPDIR']!, 'google3-test');
    shx.mkdir('-p', tmpDir);    // We need to declare the Angular symbols we're testing for, otherwise type checking won't work.
...
'@angular/core/testing': ['testing.d.ts'],
  ...
  });  afterEach(() => shx.rm('-r', tmpDir));  function runTSLint(fix: boolean) {
    const program = Linter.createProgram(join(tmpDir, 'tsconfig.json'));
    const linter = new Linter({fix, rulesDirectory: [rulesDirectory]}, program);
    const config = Configuration.parseConfigFile({rules: {'wait-for-async-cjs': true}});    program.getRootFileNames().forEach(fileName => {
linter.lint(fileName, program.getSourceFile(fileName)!.getFullText(), config);
    ...
    writeFile('/index.ts', `
import { async, inject } from '@angular/core/testing';it('should work', async(() => {
...
import { async, inject } from '@angular/core/testing';it('should work', async(() => {
  ...
  .toContain(`import { inject, waitForAsync } from '@angular/core/testing';`);
    ...
    writeFile('/index.ts', `
import { async as renamedAsync, inject } from '@angular/core/testing';it('should work', renamedAsync(() => {
  expect(inject('foo')).toBe('foo');
}));
    `);    runTSLint(true);
    expect(getFile('/index.ts'))
...
import { async } from './my-test-library';it('should work', async(() => {
  expect(inject('foo')).toBe('foo');
}));
    `);    runTSLint(true);
    ...
    writeFile('/index.ts', `
import { async, inject, waitForAsync } from '@angular/core/testing';it('should work', async(() => {
  expect(inject('foo')).toBe('foo');
...
import { async, inject } from '@angular/core/testing';it('should work', async(() => {
  ...
  expect(inject('bar')).toBe('bar');
    ...
    `);    runTSLint(true);    const content = getFile('/index.ts');
