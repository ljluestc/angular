...
import shx from 'shelljs';describe('Guard and Resolve interfaces migration', () => {
...
export declare interface CanMatch { }`);    previousWorkingDir = shx.pwd();
...
`);    await runMigration();
    ...
    writeFile('/index.ts', `
  import {Resolve} from '@angular/router';  class A implements X, Resolve {}
`);    await runMigration();
    const content = tree.readContent('/index.ts');
    expect(content).toContain(`implements X {}`);
  });  it('should be able to remove last implements', async () => {
    writeFile('/index.ts', `
  import {Resolve} from '@angular/router';  class A implements Resolve {}
`);    await runMigration();
    const content = tree.readContent('/index.ts');
    ...
    writeFile('/index.ts', `
  import {Resolve, CanActivate, CanActivateChild, CanDeactivate, CanMatch, CanLoad} from '@angular/router';  class A implements Resolve {}
...
`);    await runMigration();
    ...
    writeFile('/index.ts', `
  import {Resolve, CanActivate, CanActivateChild, CanDeactivate, CanMatch, CanLoad} from '@angular/router';  function runResolver<T>(resolver: Resolve<T>): T {}
...
`);    await runMigration();
    ...
    writeFile('/index.ts', `
  import {Resolve} from '@angular/router';  export interface Something  {
...
`);    await runMigration();
  ...
  content, `resolve?: {someData?: Type<{resolve: ResolveFn<any>;}>}}`);
    ...
    writeFile('/index.ts', `
  import {Resolve} from '@angular/router';  export interface IssueDetailViewParams {
issueId: string;
taskId?: string;
  }
