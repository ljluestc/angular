 ...
 */import ts from 'typescript';import {ChangeTracker} from '../../utils/change_tracker';
...
import {closestNode} from '../../utils/typescript/nodes';export const deprecatedInterfaces =
    new Set(['CanLoad', 'CanMatch', 'CanActivate', 'CanDeactivate', 'CanActivateChild', 'Resolve']);
export const routerModule = '@angular/router';export type RewriteFn = (startPos: number, width: number, text: string) => void;export function migrateFile(
...
getImportSpecifiers(sourceFile, routerModule, Array.from(deprecatedInterfaces));
  ...
  const updatedImplements = new Map<ts.HeritageClause, ts.HeritageClause>();  findUsages(
sourceFile, typeChecker, updatedImplements, updatedImports, changeTracker, deprecatedImports);
  findImports(sourceFile, updatedImports);  for (const [originalNode, rewrittenNode] of updatedImports.entries()) {
...
rewriteFn(change.start, change.removeLength ?? 0, change.text);
  ...
  for (const deprecatedInterface of deprecatedInterfaces) {
    const importSpecifier = getImportSpecifier(sourceFile, routerModule, deprecatedInterface);    // No `specifier` found, nothing to migrate, exit early.
    if (importSpecifier === null) continue;    const namedImports = closestNode(importSpecifier, ts.isNamedImports);
...
deprecatedInterfaces.has(importIdentifier.name)) {
 ...
 ts.isTypeReferenceNode(node.parent) ? node.parent.typeArguments : undefined,
  ...
  const importSpecifier = getImportSpecifier(sourceFile, routerModule, interfaceName);  // No `specifier` found, nothing to migrate, exit early.
  if (importSpecifier === null) return;  const namedImports = closestNode(importSpecifier, ts.isNamedImports);
  if (namedImports !== null) {
const importToUpdate = updatedImports.get(namedImports) ?? namedImports;
