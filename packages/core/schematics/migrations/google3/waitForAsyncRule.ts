...
import ts from 'typescript';import {getImportSpecifier, replaceImport} from '../../utils/typescript/imports';
import {closestNode} from '../../utils/typescript/nodes';
import {isReferenceToImport} from '../../utils/typescript/symbol';// This rule is also used inside of Google by Typescript linting./** Name of the deprecated function that we're removing. */
const deprecatedFunction = 'async';/** Name of the function that will replace the deprecated one. */
const newFunction = 'waitForAsync';/** TSLint rule that migrates from `async` to `waitForAsync`. */
export class Rule extends Rules.TypedRule {
  ...
  getImportSpecifier(sourceFile, '@angular/core/testing', deprecatedFunction);
    const asyncImport =
  asyncImportSpecifier ? closestNode(asyncImportSpecifier, ts.isNamedImports) : null;    // If there are no imports of `async`, we can exit early.
...
node: ts.NamedImports, sourceFile: ts.SourceFile, printer: ts.Printer): RuleFailure {
  ...
  ts.EmitHint.Unspecified, replaceImport(node, deprecatedFunction, newFunction), sourceFile);    return new RuleFailure(
